...
export interface DrainProgressSample {
  /** The owned session ids still tracked as running on THIS runtime's loop. */
  running: string[];
  /**
   * The most recent progress marker (ms epoch) across the still-running owned
   * sessions: `max(last_checkpoint_at, updated_at)`. 0/undefined when unknown.
   */
  lastProgressMs: number;
}

export type DrainAction =
  | { kind: "keep-waiting"; reason: "loops-still-running" | "loop-still-progressing" }
  | { kind: "drain-complete" }
  | { kind: "force-teardown"; reason: "loops-wedged-stale" };

export interface DrainDecisionInput {
  /** Current wall clock (ms). */
  nowMs: number;
  /** Absolute deadline (ms) = teardownStartMs + teardown_drain_seconds*1000. */
  deadlineMs: number;
  /** Latest sample of owned-running sessions + their freshest progress marker. */
  sample: DrainProgressSample;
  /**
   * The last progress marker we observed in a PRIOR poll (ms). Used to decide
   * whether the loop advanced since we last looked. Pass 0 on the first poll.
   */
  prevProgressMs: number;
  /** `stuck_loop_seconds` * 1000 -- staleness threshold for a wedged loop. */
 ...
 */
export function decideDrainAction(input: DrainDecisionInput): DrainAction {
  const { nowMs, deadlineMs, sample, prevProgressMs, stuckThresholdMs } = input;
  if (sample.running.length === 0) return { kind: "drain-complete" };
  if (nowMs < deadlineMs) return { kind: "keep-waiting", reason: "loops-still-running" };
