...
export type WorktreesPreflightResult =
  | { ok: true; created: boolean; worktreesRoot: string }
  | {
      ok: false;
      created: false;
      worktreesRoot: string;
 ...
 */
export function ensureWorktreesRootWritable(deps: WorktreesPreflightDeps): WorktreesPreflightResult {
  ...
  return {
    ...
    chownCommand,
    message:
      `worktrees root ${root} exists but is NOT writable by this process` +
      (uid !== null ? ` (uid ${uid})` : "") +
