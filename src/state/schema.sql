...
-- control_plane_schema_version=1; control_plane_contract_version=control-plane-contract/v1
-- Migration is additive and idempotent. legacy_rc13 session rows are historical
-- evidence only and are never converted into executable control authority.
-- Contract spelling: CREATE TABLE control_changes
CREATE TABLE IF NOT EXISTS control_changes (
  change_id TEXT PRIMARY KEY,
  state TEXT NOT NULL CONSTRAINT control_change_state CHECK (state IN ('prepared','accepted','running','pr_ready','failed','merged','merge_failed')),
  ...
  credential_route TEXT NOT NULL,
...
-- Contract spelling: CREATE TABLE control_attestations
CREATE TABLE IF NOT EXISTS control_attestations (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  change_id TEXT NOT NULL REFERENCES control_changes(change_id) ON DELETE CASCADE,
  ...
  UNIQUE(host_event_id),
  UNIQUE(nonce)
);

-- Contract spelling: CREATE TABLE control_execution_intents
CREATE TABLE IF NOT EXISTS control_execution_intents (
  change_id TEXT NOT NULL REFERENCES control_changes(change_id) ON DELETE CASCADE,
...
-- Contract spelling: CREATE TABLE control_merge_intents
CREATE TABLE IF NOT EXISTS control_merge_intents (
  change_id TEXT NOT NULL REFERENCES control_changes(change_id) ON DELETE CASCADE,
  pr_head_sha TEXT NOT NULL,
  readiness_digest TEXT NOT NULL,
  provider_idempotency_key TEXT NOT NULL UNIQUE,
  merge_provider_idempotency TEXT GENERATED ALWAYS AS (provider_idempotency_key) VIRTUAL,
  created_at INTEGER NOT NULL,
  completed_at INTEGER,
  UNIQUE(change_id)
);
