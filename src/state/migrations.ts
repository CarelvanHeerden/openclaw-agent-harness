...
function terminaliseLegacyControlChanges(db: DatabaseSync): void {
  ...
  if (!present) return;
  const rows = db.prepare("SELECT * FROM control_changes").all() as Array<Record<string, unknown>>;
  for (const row of rows) {
    const id = String(row.change_id);
    const scope = (() => { try { return JSON.parse(String(row.scope_json)) as string[]; } catch { return ["**/*"]; } })();
    ...
    db.prepare(`INSERT OR IGNORE INTO control_runs
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id, state, Number(row.generation ?? 1), String(row.actor_identity),
        String(row.conversation_identity), String(row.repository_identity), String(row.base_ref), String(row.brief_digest),
        String(row.policy_digest), JSON.stringify(authority), row.pr_url == null ? null : String(row.pr_url),
        state === "failed" ? String(row.terminal_code ?? "legacy_terminalised") : null, createdAt, Number(row.updated_at ?? createdAt));
    db.prepare(`INSERT OR IGNORE INTO control_proposals
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id, Number(row.generation ?? 1), 0,
        String(row.base_revision), String(row.brief_json), String(row.scope_json), String(row.excluded_scope_json),
        String(row.credential_route), String(row.security_class), String(row.assumptions_json),
        Number(row.proposal_expires_at), row.pr_number == null ? null : Number(row.pr_number), row.pr_url == null ? null : String(row.pr_url), row.published_sha == null ? null : String(row.published_sha),
        row.readiness_digest == null ? null : String(row.readiness_digest), row.spend_usd === null ? null : Number(row.spend_usd),
        String(row.terminal_summary ?? "Legacy control record terminalised during migration."),
        "legacy-rc13/terminal-only", "2.0.0-rc.13", createdAt, Number(row.updated_at ?? createdAt));
  }
  ...
  for (const migration of migrations) {
    ...
    try {
      ...
      if (!applied) {
        ...
        if (migration.id === "20260924_003_canonical_control_plane") terminaliseLegacyControlChanges(db);
