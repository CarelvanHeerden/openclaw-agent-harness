...
export interface DispatcherDeps {
  ...
  slackReply: (channel: string, threadTs: string, text: string) => Promise<{ ts: string }>;
...
export class Dispatcher {
   ...
   */
  async startNewSession(evt: SlackMessageEvent): Promise<void> {
    ...
    try {
      this.deps.state.db
        ...
        .run(
          ...
          evt.channel,
          evt.user,
          this.deps.config.pat_routing.overrides[evt.user]?.["_gh_user"] ?? evt.user,
          "",           // repo — resolved by lead planner
          "",           // branch — same
          "",           // worktree — same
          Date.now(),
          Date.now(),
          this.deps.config.budgets.session_default_usd,
        );
    } catch (err) {
  ...
  private async runSession(sessionId: string, evt: SlackMessageEvent): Promise<void> {
    // 1. Crystallise
    ...
    if (cResult.kind === "reject") {
      ...
      await this.deps.slackReply(
        evt.channel,
        evt.ts,
        cResult.intent === "unsafe"
    ...
    switch (outcome.status) {
      ...
      case "failed":
        await this.deps.slackReply(
          evt.channel,
          evt.ts,
          `:x: Session failed after ${outcome.cycles} cycle(s): ${outcome.reason}\nCost so far: $${outcome.totalCostUsd.toFixed(2)}`,
        );
        break;
      case "aborted":
        await this.deps.slackReply(
          evt.channel,
          evt.ts,
          `:octagonal_sign: Session aborted: ${outcome.reason}\nCycles: ${outcome.cycles} — Cost: $${outcome.totalCostUsd.toFixed(2)}`,
        );
        break;
    }
  }
   ...
   */
  async continueSession(sessionId: string, evt: SlackMessageEvent): Promise<void> {
