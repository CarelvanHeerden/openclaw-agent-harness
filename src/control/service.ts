...
export function confirmationAttestationDigest(reviewDigest: string, att: ConfirmationAttestation): string {
...
export class ControlPlaneService {
  ...
  attestationTargetForEvent(operation:ControlOperation,actorIdentity:string,conversationIdentity:string,issuedAt:number,requestedChangeId?:string):AttestationTarget {
    if(requestedChangeId)return this.attestationTarget(operation,actorIdentity,conversationIdentity,requestedChangeId);
    if(!Number.isSafeInteger(issuedAt)||issuedAt<=0)throw new ControlError(operation==="merge_change"?"merge_attestation_required":"confirmation_attestation_required","The human intent does not identify a valid pending change.");
    const state=operation==="confirm_change"?"awaiting_confirmation":"pr_ready",now=this.now();
    const rows=operation==="confirm_change"
      ? this.deps.db.prepare(`SELECT r.id,r.updated_at FROM control_runs r JOIN control_proposals p ON p.run_id=r.id WHERE r.requester_id=? AND r.conversation_id=? AND r.state=? AND r.updated_at<? AND p.confirmable=1 AND p.proposal_expires_at>=? ORDER BY r.updated_at DESC LIMIT 2`).all(actorIdentity,conversationIdentity,state,issuedAt,now) as Array<{id:string;updated_at:number}>
      : this.deps.db.prepare(`SELECT id,updated_at FROM control_runs WHERE requester_id=? AND conversation_id=? AND state=? AND updated_at<? ORDER BY updated_at DESC LIMIT 2`).all(actorIdentity,conversationIdentity,state,issuedAt) as Array<{id:string;updated_at:number}>;
    if(rows.length===0||rows.length>1&&rows[0]!.updated_at===rows[1]!.updated_at)throw new ControlError(operation==="merge_change"?"merge_attestation_required":"confirmation_attestation_required","The human intent does not identify one latest pending change.");
