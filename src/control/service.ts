...
export class ControlPlaneService {
  ...
  constructor(private readonly deps: ControlServiceDeps) { this.now=deps.now??Date.now; this.ttl=deps.confirmationTtlMs??900_000; this.dispatchLeaseMs=deps.dispatchLeaseMs??300_000; queueMicrotask(()=>{void this.recoverDispatches();void this.deps.mergeService.recoverPending();}); this.recoveryTimer=setInterval(()=>void this.recoverDispatches(),Math.max(10,Math.floor(this.dispatchLeaseMs/3))); this.recoveryTimer.unref?.(); }
