...
export class InternalMergeService {
  ...
  async recoverPending():Promise<void>{
    const rows=this.db.prepare(`SELECT authorization_id FROM control_engine_merge_intents WHERE status IN ('authorized','merging') ORDER BY created_at`).all() as Array<{authorization_id:string}>;
  ...
  async merge(id:string):Promise<MergeServiceResult>{
    ...
    if(intent.status==="merged"&&intent.provider_merge_sha&&EXACT_PROVIDER_SHA.test(intent.provider_merge_sha)&&await this.provider.verifyMerged({repository,prNumber,mergeSha:intent.provider_merge_sha})) return Object.freeze({status:"already_merged",mergeSha:intent.provider_merge_sha});
    if(intent.status!=="authorized") return this.reconcileClaimedMerge(run.id,intent.id,repository,prNumber,intent.status,intent.provider_merge_sha);
    ...
    if(inspection.merged){const mergeSha=inspection.mergeSha??intent.provider_merge_sha??undefined;if(mergeSha&&EXACT_PROVIDER_SHA.test(mergeSha)&&await this.provider.verifyMerged({repository,prNumber,mergeSha})){this.completeRun(run.id,intent.id,mergeSha);return Object.freeze({status:"already_merged",mergeSha});}this.failRun(run.id,intent.id,"verification_failed");return Object.freeze({status:"merge_failed",code:"verification_failed"});}
    ...
    try {
      ...
      this.db.exec("COMMIT");
    }catch(error){try{this.db.exec("ROLLBACK");}catch{}if(error instanceof Error&&error.message==="merge_claim_lost")return this.reconcileClaimedMerge(run.id,intent.id,repository,prNumber);return Object.freeze({status:"refused",code:"authorization_replayed"});}
    try{
    ...
    }catch{return this.reconcileClaimedMerge(run.id,intent.id,repository,prNumber,"merging");}
  }

  private async reconcileClaimedMerge(runId:string,intentId:string,repository:string,prNumber:number,knownStatus?:string,knownSha?:string|null):Promise<MergeServiceResult>{
    ...
    while(true){
      ...
      if(status==="merged"){
        if(providerMergeSha&&EXACT_PROVIDER_SHA.test(providerMergeSha)&&await this.provider.verifyMerged({repository,prNumber,mergeSha:providerMergeSha}))return Object.freeze({status:"already_merged",mergeSha:providerMergeSha});
      ...
      try{inspection=await this.provider.inspect({repository,prNumber});}catch{}
      if(inspection?.merged){
        const exactSha=inspection.mergeSha;
        if(exactSha&&EXACT_PROVIDER_SHA.test(exactSha)&&await this.provider.verifyMerged({repository,prNumber,mergeSha:exactSha})){this.completeRun(runId,intentId,exactSha);return Object.freeze({status:"already_merged",mergeSha:exactSha});}
