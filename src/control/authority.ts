...
export function createAuthorityEnvelope(input: AuthorityEnvelope): AuthorityEnvelope {
  if (input.version !== 1) throw new Error("Unsupported authority envelope version");
  for (const [name, value] of Object.entries({ requesterId: input.requesterId, conversationId: input.conversationId, repository: input.repository, baseRef: input.baseRef, nonce: input.nonce })) {
    if (!value.trim()) throw new Error(`Authority envelope ${name} is required`);
  }
...
export function evaluateAuthority(envelope: AuthorityEnvelope, request: AuthorityRequest, nonceStore?: AuthorityNonceStore): AuthorityDecision {
  if (request.requesterId !== envelope.requesterId) return terminate("identity_mismatch");
  if (request.conversationId !== envelope.conversationId) return terminate("conversation_mismatch");
  if (request.repository !== envelope.repository) return terminate("repository_mismatch");
  if (request.baseRef !== envelope.baseRef) return terminate("base_ref_mismatch");
  if (request.briefDigest !== envelope.briefDigest) return terminate("brief_digest_mismatch");
  if (request.policyDigest !== envelope.policyDigest) return terminate("policy_digest_mismatch");
  if (request.nonce !== envelope.nonce) return terminate("replay");
  if (request.now > envelope.expiresAt) return terminate("expired");
  if (request.securityBypass) return terminate("security_expansion");
  ...
  if (request.projectedActiveTimeMs > envelope.limits.activeTimeMs) return terminate("time_expansion");
  if (request.projectedCycles > envelope.limits.cycles) return terminate("cycle_expansion");
  if (request.projectedRetries > envelope.limits.retries) return terminate("retry_expansion");
  if (nonceStore && !nonceStore.consume(envelope.nonce, authorityEnvelopeDigest(envelope), request.now)) return terminate("replay");
