...
export class OrchestratorLoop {
  ...
  private async runInner(sessionId: string, brief: CrystallisedBrief): Promise<LoopOutcome> {
    ...
    while (cycle < this.deps.config.loop.max_cycles) {
      ...
      const clarify = { question: null as string | null, seq: -1 };
      ...
      const runOne = async (st: LeadPlanSubTask): Promise<void> => {
        ...
        try {
          ...
          result = await withTimeout(
            this.deps.runWorker({ brief, subTask: st, plan, requester: row.requester, dispatchHint: reviseHint }),
        ...
        if (contract.length > 0 && this.deps.buildVerifyProbes) {
          ...
          try {
            verification = await verifySubTaskOutput(
              contract,
              { defaultBranch: branchHint, subTaskStartMs: subTaskStartedAtMs, baseSha: subTaskBaseSha },
              probes,
            );
          } catch (err) {
    ...
    this.deps.state.audit(
      "loop.shipped",
      { sessionId, prUrl, prNumber, mergeRecommendation: rec.recommendation, reason: rec.reason },
      sessionId,
    );
  ...
  private finaliseAwaitingClarification(
    ...
    this.deps.state.db.prepare(
      `UPDATE sessions SET clarification_question = ?, clarification_seq = ?, clarification_answer = NULL, updated_at = ? WHERE id = ?`,
    ).run(question, seq, Date.now(), sessionId);
    this.deps.state.audit(
