...
export class OrchestratorLoop {
        ...
        if (contract.length > 0 && this.deps.buildVerifyProbes) {
          ...
          try {
            verification = await verifySubTaskOutput(
              contract,
              { defaultBranch: branchHint, subTaskStartMs: subTaskStartedAtMs, baseSha: subTaskBaseSha, branchBaseSha: planBaseShaForVerify },
              probes,
            );
          } catch (err) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        result = await withTimeout(
          this.deps.runWorker({ brief, subTask: st, plan, requester, dispatchHint }),
          this.deps.config.loop.worker_timeout_seconds,
        );
      } catch (err) {
        if (err instanceof WorkerTimeoutError) {
  ...
  private async finaliseReviewCrash(
    ...
    this.deps.state.audit(
      "loop.shipped",
      { sessionId, prUrl, prNumber, mergeRecommendation: "needs_human_review", reason: recReason, viaReviewCrashRecovery: true },
      sessionId,
    );
