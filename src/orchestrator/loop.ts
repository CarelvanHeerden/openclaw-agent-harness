        ...
        try {
          ...
          if (ledger.length > 0) {
            const headSha = this.deps.worktreeHeadSha
              ...
              : "";
            const bad = await this.deps.unreachableCommits(
              plan.worktreePath,
              headSha || "HEAD",
              ledger.map((e) => e.commitSha),
            );
            ...
            this.deps.state.audit(
              ...
              { sessionId, cycle, headSha, checked: integrity.checked, unreachableCount: integrity.unreachable.length, ok: integrity.ok },
              sessionId,
            );
            if (!integrity.ok) {
