...
export function deriveMergeRecommendation(input: RecommendationInput): RecommendationResult {
  ...
  if (!review) return { recommendation: "do_not_merge", reason: "No completed adversary review exists for the published head." };
  const passed = review.verdict === "pass";
  if (!passed) return { recommendation: "do_not_merge", reason: `The final adversary verdict was "${review.verdict}", not a pass.` };
  if (!reachedCleanPass) return { recommendation: "do_not_merge", reason: "The review did not finish with a clean final pass." };
