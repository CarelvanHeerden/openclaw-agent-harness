...
function ciNote(ciStatus: RecommendationInput["ciStatus"]): string {
  ...
  if (review.verdict !== "pass") {
    ...
    if (blockingCount === 0) {
      ...
      return {
        ...
        reason:
          `The adversary's final verdict was "${review.verdict}", but nothing blocking remains: ` +
    ...
    return {
      recommendation: "do_not_merge",
      reason:
        `The adversary's final verdict was "${review.verdict}", not "pass". The review loop did not sign off on this change.` +
