 ...
 */
export function mergeLedgerCommits(...sources: LedgerCommit[][]): LedgerCommit[] {
  const seen = new Set<string>();
  const out: LedgerCommit[] = [];
 ...
 * abbreviated.
 */
export function buildLedgerIntegrityReport(
  ledger: LedgerCommit[],
 ...
 */
export function describeLedgerIntegrityFailure(report: LedgerIntegrityReport, headSha?: string): string {
  const lost = report.unreachable
    .map((e) => `seq ${e.seq} (${e.commitSha.slice(0, 7)}${e.title ? `: ${e.title}` : ""})`)
    .join(", ");
  return (
    `branch integrity check FAILED: ${report.unreachable.length} of ${report.checked} recorded sub-task commit(s) ` +
