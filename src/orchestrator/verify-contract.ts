 ...
 */
export function inferVerifyContract(
  ...
  effectiveTaskMode?: LeadPlanSubTask["taskMode"],
): SubTaskVerify[] {
  ...
  if (subTask.verify && subTask.verify.length > 0) {
    if (demotedToObserve) return subTask.verify.filter((c) => !isMutationScopeKind(c.kind));
