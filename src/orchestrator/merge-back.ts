...
export type MergeBackResult =
  ...
  | { ok: false; reason: "conflict" | "error"; conflictedPaths: string[]; detail: string };
...
export async function commitsToReplay(git: MergeBackGit, req: MergeBackRequest): Promise<string[]> {
...
export async function conflictedPaths(git: MergeBackGit, cwd: string): Promise<string[]> {
  try {
 ...
 */
export async function mergeBackSubTask(git: MergeBackGit, req: MergeBackRequest): Promise<MergeBackResult> {
  let landed: string[];
  try {
