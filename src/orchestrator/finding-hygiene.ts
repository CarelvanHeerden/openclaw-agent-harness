 ...
 */
export const CONDITIONAL_FINDING_RE =
  /\b(if\s+no\b|if\s+there\s+(is|are)\s+no\b|if\s+.*\bdoes\s+not\s+exist|assuming\b|unless\b|provided\s+that\b|so\s+long\s+as\b|only\s+if\b|when\s+no\b|if\s+.*\bexist)/i;

/** Extract the human-readable text of a stored finding (schema is loose). */
export function findingText(f: unknown): string {
...
export function isConditionalFinding(f: unknown): boolean {
  ...
  for (const raw of (s ?? "").toLowerCase().replace(/[^a-z0-9/_.-]+/g, " ").split(/\s+/)) {
    ...
    if (/[/.]/.test(raw)) {
      for (const seg of raw.split(/[/.]+/)) {
        if (seg.length >= 3 && !STOPWORDS.has(seg)) out.add(seg);
      }
    }
  }
  return out;
 ...
 */
export function removeOwningFindingLines(
  lines: string[],
  pausedTitle: string,
  pausedIntent: string,
): { kept: string[]; dropped: string[] } {
  const want = new Set<string>([...contentTokens(pausedTitle), ...contentTokens(pausedIntent)]);
  if (want.size === 0) return { kept: lines, dropped: [] };
  const kept: string[] = [];
  ...
  for (const line of lines) {
    ...
    let hits = 0;
    for (const t of lineToks) if (want.has(t)) hits++;
    ...
    if (hits >= 2 && ratio >= 0.3) dropped.push(line);
    else kept.push(line);
  }
  return { kept, dropped };
}
