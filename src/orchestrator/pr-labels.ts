...
export function prLabelsFor(review: LabelableReview): string[] {
  const labels: string[] = [];
  if (review.verdict !== "pass") labels.push(LABEL_DO_NOT_MERGE);
  if (review.verdictDowngraded) labels.push(LABEL_DOWNGRADED_PASS);
