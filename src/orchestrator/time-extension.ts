 ...
 * the whole run.
 */
export function listenerLooksAlive(heartbeatAt: number | null | undefined, nowMs = Date.now()): boolean {
  if (typeof heartbeatAt !== "number" || !Number.isFinite(heartbeatAt) || heartbeatAt <= 0) return false;
