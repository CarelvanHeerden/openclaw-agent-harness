...
export function parseTscErrors(output: string): TscError[] {
  ...
  for (const raw of (output ?? "").split("\n")) {
    ...
    const g = m.groups as unknown as { file: string; line: string; col: string; code: string; msg: string };
 ...
 */
export function diagnosticsFrom(result: { output?: string; outputTail?: string }): TscError[] {
 ...
 */
export function errorsInChangedFiles(errors: TscError[], changedFiles: string[]): TscError[] {
  if (errors.length === 0 || changedFiles.length === 0) return [];
 ...
 */
export function buildTypecheckFinding(errors: TscError[], script: string): ReviewFinding {
  ...
  const FILES_MAX = 12;
  const perFile =
    files.slice(0, FILES_MAX).map((f) => `${f} (${byFile.get(f)?.length ?? 0})`).join(", ") +
  ...
  return {
    ...
    line: errors[0]?.line,
    title:
      `Branch does not typecheck: ${errors.length} error(s) from \`${script}\` across ` +
      `${files.length} file(s) this branch changed`,
    detail:
      `\`${script}\` reported ${errors.length} error(s) in ${files.length} file(s) this branch touched. These ` +
