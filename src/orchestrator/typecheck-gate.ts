...
export function buildTypecheckFinding(errors: TscError[], script: string): ReviewFinding {
  ...
  return {
    ...
    line: errors[0]?.line,
    title:
      `Branch does not typecheck: ${errors.length} error(s) from \`${script}\` in file(s) this branch changed`,
    detail:
      `\`${script}\` reported ${errors.length} error(s) in files this branch touched. These were introduced or ` +
