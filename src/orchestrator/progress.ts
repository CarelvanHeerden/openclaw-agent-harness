...
export function buildProgressSnapshot(db: DatabaseSync, sessionId: string, limit = 12, stallSeconds = 1800, sdkActivityStallSeconds = 90): ProgressSnapshot {
  ...
  if (status === "failed" || status === "failed_verification") {
    ...
    for (let i = recentEvents.length - 1; i >= 0; i--) {
      ...
      if (PATH_FAIL.has(ev.event)) {
        const d = (ev.detail ?? {}) as { detail?: string };
        if (typeof d.detail === "string") failureDetail = `verifier path check: ${d.detail}`;
        break;
      }
    }
    ...
    if (!failureDetail) {
      try {
        ...
        if (fr?.payload) {
          ...
          if (typeof raw === "string" && raw.trim()) failureDetail = raw.slice(0, 300);
        }
      } catch {
  ...
  if (status === "aborted") {
    try {
      ...
      for (const ev of ar) {
        if (ev.event === "loop.abort_worktree_preserved") worktreePreserved = true;
        if (!failureDetail && ev.payload) {
          const p = JSON.parse(ev.payload) as { reason?: string };
          if (typeof p.reason === "string" && p.reason.trim()) failureDetail = p.reason.slice(0, 300);
        }
      }
    } catch {
      /* best-effort */
...
function countFiles(filesTouched?: string | null): number {
  if (!filesTouched) return 0;
  try {
    const parsed: unknown = JSON.parse(filesTouched);
 ...
 */
export function mergeAdvice(recommendation?: string | null, reason?: string | null): string {
  const rec = (recommendation ?? "").trim().toLowerCase();
  if (!rec) return "";
  if (rec === "merge") return " Ready to merge.";
