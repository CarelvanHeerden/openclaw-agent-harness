...
export function diagnoseCheckEnv(
  worktree: string,
  deps: { spawn?: typeof spawnSync; env?: NodeJS.ProcessEnv } = {},
): CheckEnvDiagnosis {
  ...
  let nodeModulesEntries: number | undefined;
  ...
  let tsc: CheckEnvDiagnosis["tsc"] = "missing";
 ...
 */
export function runTypecheckDirect(
  worktree: string,
  timeoutMs: number,
  deps: { spawn?: typeof spawnSync; env?: NodeJS.ProcessEnv } = {},
): DirectRun | null {
  const spawn = deps.spawn ?? spawnSync;
  const env = deps.env ?? process.env;
  const opts = { cwd: worktree, timeout: timeoutMs, encoding: "utf8" as const, maxBuffer: 8 * 1024 * 1024, env };

  const local = join(worktree, "node_modules", ".bin", "tsc");
  const attempts: Array<{ via: DirectRun["via"]; cmd: string; args: string[] }> = [];
  if (existsSync(local)) attempts.push({ via: "node_modules_bin", cmd: local, args: ["--noEmit"] });
  ...
  for (const a of attempts) {
    ...
    if (res.status === 127 || res.status === 126) continue;
    if (res.error && !isTimeout(res)) continue;
...
function isTimeout(res: { error?: unknown; signal?: string | null }): boolean {
