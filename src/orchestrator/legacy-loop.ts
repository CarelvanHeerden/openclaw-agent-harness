...
export class OrchestratorLoop {
  ...
  private assertConfirmedControlAuthority(
    ...
    if (!guard) throw new ConfirmedControlAuthorityError("confirmed control authority guard is missing");
  ...
  private async run(sessionId: string, brief: CrystallisedBrief): Promise<LoopOutcome> {
      ...
      // rather than blowing the budget by ~$0.83 on a review we can't pay for.
      {
        ...
        if (dailyWouldExceed) {
          ...
          this.deps.state.audit(
            ...
            { sessionId, cycle, totalCost, reviewEstimate, dailySoFar, dailyMax, reason: "daily_max", extensionAvailable: false },
            sessionId,
          );
          this.warnDailyMaxHit(sessionId, row.requester, dailySoFar, dailyMax);
