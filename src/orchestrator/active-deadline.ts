...
interface DeadlineRow {
  ...
  human_pause_started_at: number | null;
}
...
function row(db: DatabaseSync, sessionId: string): DeadlineRow {
  const value = db
    .prepare(
      `SELECT hard_timeout_seconds, active_limit_ms, active_elapsed_ms,
              active_segment_started_at, human_pause_started_at
         FROM sessions WHERE id = ?`,
    )
    .get(sessionId) as DeadlineRow | undefined;
  if (!value) throw new Error(`session ${sessionId} not found`);
  return value;
}
 ...
 */
export function resumeActiveDeadline(
  db: DatabaseSync,
  sessionId: string,
  ...
  db.prepare(
    `UPDATE sessions
        SET active_limit_ms = ?, active_elapsed_ms = ?, active_segment_started_at = ?,
            ...
            minimum_runtime_version = COALESCE(minimum_runtime_version, '2.0.0-rc.11'),
            updated_at = ?
      WHERE id = ?`,
...
export function pauseActiveDeadline(
  db: DatabaseSync,
  sessionId: string,
  now = Date.now(),
): ActiveDeadlineSnapshot {
  ...
  const elapsedMs =
    Math.max(0, current.active_elapsed_ms ?? 0) +
...
export function closeActiveDeadline(
  db: DatabaseSync,
  sessionId: string,
  now = Date.now(),
): ActiveDeadlineSnapshot {
  ...
  db.prepare(
    `UPDATE sessions
      ...
      WHERE id = ?`,
  ).run(limitMs, elapsedMs, DEADLINE_POLICY_VERSION, now, sessionId);
  return {
    limitMs,
    elapsedMs,
    remainingMs: Math.max(0, limitMs - elapsedMs),
...
export function activeDeadlineSnapshot(
  db: DatabaseSync,
  sessionId: string,
  now = Date.now(),
): ActiveDeadlineSnapshot {
...
export function extendActiveDeadline(
  db: DatabaseSync,
  sessionId: string,
  seconds: number,
  now = Date.now(),
  ...
  db.prepare(
    `UPDATE sessions SET active_limit_ms = ?, hard_timeout_seconds = CAST(? / 1000 AS INTEGER),
                         updated_at = ? WHERE id = ?`,
  ).run(limitMs, limitMs, now, sessionId);
