...
function normSet(files: string[] | undefined): Set<string> {
...
export function fileScopesOverlap(a: string[] | undefined, b: string[] | undefined): boolean {
  const sa = normSet(a);
  const sb = normSet(b);
 ...
 *  - otherwise -> true.
 */
export function canDispatchConcurrently(
  candidate: ParallelSubTask,
  inFlight: ParallelSubTask[],
): boolean {
  if (inFlight.length === 0) return true; // first one always dispatches
  const cFiles = (candidate.filesLikelyTouched ?? []).filter(Boolean);
  if (cFiles.length === 0) return false; // unknown candidate scope
  for (const other of inFlight) {
    const oFiles = (other.filesLikelyTouched ?? []).filter(Boolean);
    if (oFiles.length === 0) return false; // unknown in-flight scope
    if (fileScopesOverlap(cFiles, oFiles)) return false;
  }
  return true;
}
...
export function resolveEffectiveConcurrency(opts: {
  ...
  const c = Math.max(1, Math.floor(opts.subtaskConcurrency || 1));
  if (!opts.parallelEnabled) return 1;
  return c;
}
