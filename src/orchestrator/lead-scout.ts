 ...
 */
export const SCOUT_ALLOWED_TOOLS = ["Read", "Glob", "Grep"] as const;
...
export function buildScoutUserMessage(brief: {
  ...
  if (guess.length > 0) {
    lines.push(
      ``,
      `The upstream request guessed these paths. Treat them as UNVERIFIED and check each one -- confirm it, or find the real location and say what it actually is:`,
      ...guess.map((f) => `  - ${f}`),
    );
  }
 ...
 */
export function renderScoutForPrompt(report: string | undefined): string {
  const r = (report ?? "").trim();
  if (!r) return "";
  return [
    "",
