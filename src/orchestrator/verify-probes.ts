...
export function createVerifyProbes(
  ...
  return ({ plan, requester, worktreePath, baseSha }: BuildVerifyProbesArgs): VerifyProbes => {
    ...
    return {
       ...
       */
      fileCommittedInCommits: async (path: string, shas: readonly string[]) => {
        if (shas.length === 0) return { committed: false, detail: "no earlier attempt recorded a commit" };
       ...
       */
      changedFilesSince: async (branchBaseSha: string) => {
        try {
