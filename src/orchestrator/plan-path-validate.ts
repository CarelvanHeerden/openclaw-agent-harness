 ...
 */
export function findSuspectPlanPaths(planPaths: string[], repoFiles: string[]): SuspectPlanPath[] {
  ...
  if (files.size === 0) return []; // No repo listing -> no opinion. Fail open.

  const seen = new Set<string>();
  const out: SuspectPlanPath[] = [];
  for (const raw of planPaths) {
    ...
    seen.add(p);
    if (files.has(p) || dirs.has(p)) continue;
    const dir = parentDir(p);
    if (!dir) continue; // repo root always exists
    if (dirs.has(dir)) continue; // plausible new file in a real directory
 ...
 */
export function describeSuspectPlanPaths(suspects: SuspectPlanPath[]): string {
  ...
  if (detached.length > 0) {
    parts.push(
      `PLAN PATH WARNING: the plan names ${detached.length} path(s) in directories that do not exist, and the ` +
  ...
  if (shallow.length > 0) {
    parts.push(
      `NEW DIRECTORY NOTE: ${shallow.length} path(s) in the plan sit in a directory that does not exist yet, so ` +
