 ...
 */
export function evidenceCoversCandidate(
  evidence: PublicationEvidence | null | undefined,
  candidateSha: string,
  branch?: string,
): boolean {
  if (!evidence) return false;
  if (branch !== undefined && evidence.branch !== branch) return false;
...
export function describeUnpublished(input: {
  ...
  switch (input.kind) {
    case "remote_mismatch":
      lines.push(
        `The branch tip is a commit the harness did not publish. It may be concurrent work.`,
        `The harness will NEVER force-push over it. Inspect both commits and reconcile by hand.`,
      );
      break;
    case "remote_missing":
    ...
    case "verification_unavailable":
      lines.push(
        `The remote could not be read, so publication is UNKNOWN, not assumed.`,
        `Check credentials/network, then read the branch before deciding whether to push.`,
      );
      break;
  ...
  lines.push(
    ``,
    `The commits are preserved. Recover them with:`,
    `  git -C ${input.worktreePath ?? "<worktree>"} log --oneline origin/${input.branch}..HEAD`,
    `  git -C ${input.worktreePath ?? "<worktree>"} push origin ${input.branch}`,
  );
  return lines.join("\n");
}
