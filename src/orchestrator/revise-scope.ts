...
function isMetaDimension(f: ScopeFinding): boolean {
  return META_DIMENSIONS.has(normaliseDimension(f.dimension as string | undefined));
}
...
export interface ReviseScopeResult {
  ...
  reason?:
    | "not_revise_cycle"
    | "no_findings"
    | "unscopable_findings" // >=1 actionable finding has no resolvable file
    | "all_relevant" // every sub-task intersects a finding (nothing to skip)
    | "no_subtask_owns_the_findings"; // beta.113: scoping selected nobody
...
export function computeReviseScope(
  ...
  const anyFindingUnfiled = (findings ?? []).some(
    (f) => !(f.file ?? "").trim() && !isMetaDimension(f) && !isBelowActionable(f),
  );
