 ...
 * on a response.
 */
const EXECUTION_SIGNALS: Array<{ re: RegExp; label: string }> = [
 ...
 */
const CODE_WORK_GUARD =
  /\b(add|write|create|implement|refactor|update|fix|test(s)?\s+(that|asserting|for)|assert(ing)?\s+the\s+(handler|route|endpoint|response)|the\s+handler\s+(returns|responds)|unit\s*test|integration\s*test|route\s*handler|in\s+the\s+(codebase|repo|file))\b/i;

function criterionSignals(text: string): string[] {
  const labels: string[] = [];
 ...
 */
export function detectApiExecutionBrief(
  brief: Pick<CrystallisedBrief, "acceptanceCriteria" | "title" | "motivation">,
  opts: ApiExecutionDetectOptions = {},
): ApiExecutionDetectionResult {
  ...
  for (const ac of acs) {
    ...
    const strong = sigs.some((s) =>
      s === "live-api-url" || s === "vercel-host" || s === "return-ok-true" || s === "ok-true-literal" || s === "against-live-system" || s === "file-upload-call",
    );
    if (!strong && CODE_WORK_GUARD.test(text)) continue;
    matched.push(text);
  }
 ...
 */
export function buildApiExecutionClarification(result: ApiExecutionDetectionResult): string {
