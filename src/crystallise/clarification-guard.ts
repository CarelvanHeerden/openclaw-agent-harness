...
export type GuardVerdict =
  | { action: "ask"; question: string; reason: ClarificationReason }
  | { action: "withhold"; question: string; suppressed: SuppressionReason[] };
...
const ABSOLUTE_PATH_RE =
  /(?:^|[\s"'`([<])(?:~\/|[A-Za-z]:\\|\/(?:home|Users|root|var|tmp|opt|mnt|srv|data|app|usr|private|workspace|Volumes)\/)[^\s"'`)\]>,;]*/g;
...
const BASE_PHRASE_RE =
  /\b(?:based?\s+(?:this\s+|it\s+)?on|start(?:ing)?\s+from|branch(?:ing)?\s+(?:off|from)|check\s?out)\s+(?:the\s+)?(?:latest\s+)?`?(?:origin\/)?([A-Za-z0-9._\-/]+)/i;
 ...
 */
const PR_TARGET_RE =
  /(?:(?:PR|pull\s+request)\s+(?:against|into|to)|merged?\s+in?to|target(?:ing)?\s+branch)\s+(?:the\s+)?`?(?:origin\/)?([A-Za-z0-9._\-/]+)/i;
...
export function unverifiedPathsIn(text: string, g: ClarificationGrounding): string[] {
 ...
 * exists.
 */
export function claimsUnverifiedState(text: string, g: ClarificationGrounding): boolean {
  if (g.continuation) return false;
 ...
 */
export function guardClarification(
  question: string,
  grounding: ClarificationGrounding,
  reason: ClarificationReason,
): GuardVerdict {
  const text = (question ?? "").trim();
  if (!text) return { action: "withhold", question: text, suppressed: ["harness_owned_checkout"] };

  const suppressed: SuppressionReason[] = [];
  if (unverifiedPathsIn(text, grounding).length > 0) suppressed.push("invented_filesystem_path");
  if (claimsUnverifiedState(text, grounding)) suppressed.push("unverified_worktree_state");
  if (isFalseBaseConflict(text)) suppressed.push("harness_owned_checkout");

  if (suppressed.length > 0) return { action: "withhold", question: text, suppressed };
 ...
 */
export function renderGroundingBlock(g: ClarificationGrounding): string {
  const lines: string[] = [
    "",
    "VERIFIED CONTEXT (this is everything the harness knows; you have no other source):",
  ];
  lines.push(
    g.allowedRepos.length > 0
      ? `- Repositories this harness may touch: ${g.allowedRepos.join(", ")}`
      : "- The operator has not restricted which repositories may be touched.",
  );
  if (g.continuation) {
    lines.push(
      `- This continues session ${g.continuation.sessionId}, on branch ${g.continuation.branch} of ${g.continuation.repo}, in a worktree that has been checked and still exists. You may refer to it.`,
    );
  } else {
    lines.push(
      "- There is NO active session, NO worktree, NO checkout and NO uncommitted work associated with this request. None has been created yet.",
    );
  }
  lines.push(
    "",
    "CHECKOUT IS HARNESS-OWNED. Every new run fetches the remote, creates a fresh isolated worktree on a new namespaced branch based on the latest " +
      `origin/${g.defaultBaseBranch || "<default branch>"}, and opens its PR against ${g.defaultBaseBranch || "the default branch"}. Consequences you must respect:`,
    "- \"checkout latest main\", \"start from latest main\" and \"base this on origin/main\" all DESCRIBE THAT DEFAULT. They are confirmations, not new instructions, and never a reason to clarify.",
    "- Basing on the latest main and opening a PR against main are THE SAME normal workflow. Never present them as conflicting or ask the user to choose between them.",
    "- You have no filesystem knowledge. NEVER write an absolute path, name a directory, or refer to a worktree, checkout, clone, uncommitted change or local edit. You cannot know whether any exists, and asserting one is a defect.",
    "- Never ask the user to decide checkout mechanics (which directory, whether to reuse a worktree, whether to fetch, what to do with local changes). The harness owns all of it.",
    "",
  );
  return lines.join("\n");
}
