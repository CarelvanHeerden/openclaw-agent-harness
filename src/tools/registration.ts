  ...
  disposers.push(
    toDispose(
      api.registerTool({
        ...
        execute: async (_callId: unknown, input: unknown) => {
          const { requester, action, token, provider, promptTs, dmChannel } = input as {
            requester: string; action: "start" | "submit"; token?: string; provider?: string; promptTs?: string; dmChannel?: string;
          };
          if (!liveConfig().slack.authorised_users.includes(requester)) {
