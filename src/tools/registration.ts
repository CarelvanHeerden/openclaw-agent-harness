...
import { makeBriefProposal, renderBriefProposal, verifyBriefProposal, type BriefProposal, type BriefState } from "./brief-proposal.js";
...
export function registerHarnessTools(api: HarnessPluginApi, runtime: HarnessRuntime): () => void {
  ...
  function startSessionFromBrief(params: {
    ...
    auditEvent: string;
    /**
     * beta.120: skip the confirm-before-spend gate. Set for continuations of
     * already-confirmed work (a revise re-runs a brief the human accepted, off
     * findings the harness itself produced).
     */
    confirmWaived?: boolean;
    /** beta.120: set when the request was read verbatim from this path. */
    sourcePath?: string;
  }):
    | { ok: true; sessionId: string; budgetNote?: string; effectiveBudget: number; recommendedBudget: number; estimatedUsd: number; awaitingConfirmation?: false }
    | { ok: true; sessionId: string; awaitingConfirmation: true; question: string; effectiveBudget: number; recommendedBudget: number; estimatedUsd: number; budgetNote?: string }
    ...
    if (slackThread) {
      const prior = liveDb()
        ...
        .all(slackChannel, slackThread) as Array<{ id: string; status: string }>;
      ...
      if (prior.length > 0) {
        ...
        const retire = liveDb().prepare(
          `UPDATE sessions SET slack_thread = 'retired:' || id || ':' || slack_thread, updated_at = ?
    ...
    const decision = decideBriefConfirmation({
      ...
      mode: (briefCfg?.confirm_before_spend ?? "always") as ConfirmMode,
      riskLevel: (params.brief as { riskLevel?: string }).riskLevel,
      minRisk: (briefCfg?.confirm_min_risk ?? "high") as RiskLevel,
      waived: params.confirmWaived,
    });
    const confirmationQuestion = decision.confirm
      ? renderBriefConfirmation({
          brief: params.brief as unknown as CrystallisedBrief,
    ...
    try {
      liveDb()
        ...
        .run(
          ...
          decision.confirm ? "awaiting_clarification" : "planning",
          ...
          rec.recommended,
          decision.confirm ? confirmationQuestion : null,
          decision.confirm ? BRIEF_CONFIRMATION_SEQ : null,
          decision.confirm ? JSON.stringify({ kind: BRIEF_CONFIRMATION_KIND }) : null,
        );
    } catch (err) {
    ...
    if (decision.confirm) {
      liveState().audit(
        "tool.run.awaiting_brief_confirmation",
        { sessionId, requester: params.requester, reason: decision.reason, riskLevel: (params.brief as { riskLevel?: string }).riskLevel ?? null, effectiveBudget, fromFile: params.sourcePath ?? null },
        sessionId,
      );
  ...
  function recommendBudget(
    user: string,
    requested?: number,
  ): { recommended: number; note?: string; dailySoFar: number; dailyMax: number; remainingDaily: number } {
    ...
    if (typeof ceiling === "number" && ceiling > 0) recommended = Math.min(recommended, ceiling);
    if (Number.isFinite(remainingDaily)) recommended = Math.min(recommended, remainingDaily);
        ...
        parameters: {
          ...
          properties: {
            topic: {
              ...
              description:
                "Narrow the answer: starting a change, controlling a run in flight, what to do once a PR exists, or how budgets/caps work. Default 'all'.",
            },
          },
          additionalProperties: false,
        },
        execute: (_callId: unknown, input: unknown) => {
          const topic = String((input as { topic?: string } | undefined)?.topic ?? "all");
  ...
  disposers.push(
    toDispose(
      api.registerTool({
        ...
        description:
          "Merge the pull request a completed harness session opened, then verify the deployment. HARD SAFETY GATE: the harness only merges when its post-ship recommendation is 'merge'. If the recommendation is 'do_not_merge' (or CI is failing), it REFUSES and the user must merge from the GitHub UI — the harness cannot be told to override. Use after a session reaches 'done' with a PR, when the user has approved the merge. On a Vercel-enabled repo it polls the deployment for the merge commit and reports READY/ERROR (with build logs on error).",
        parameters: {
          type: "object",
          properties: {
          ...
          if (res.awaitingConfirmation === true) {
            return {
              ...
              details: {
                ...
                feedback: {
                  instruction:
                    "STOP and show the question verbatim. Wait for the authenticated requester to reply naturally, then call harness_answer with answeredBy:'human', invokedBy matching the host requester, and the current clarification identity. Do not approve on their behalf.",
                  answerWith: "natural_language_reply",
                },
              },
            };
          }
  ...
  disposers.push(
    toDispose(
      api.registerTool({
        ...
        parameters: {
          ...
          properties: {
            ...
            request: {
              ...
              minLength: 10,
              description:
                "The user's coding request, VERBATIM AND COMPLETE. Copy their text exactly -- full spec, all sections, all field names. Never summarise, condense into acceptance criteria, or reword: the harness crystallises it for you, and a paraphrase silently builds a different feature. Optional only when `requestPath` is given.",
            },
            requestPath: {
              type: "string",
              minLength: 1,
              description:
                "beta.120. Optional but PREFERRED when the request came from a file: an absolute path to the user's specification. The harness reads the bytes itself, which removes any chance of the text being paraphrased in transit. The file must sit inside a configured `brief.request_file_roots` directory. If both this and `request` are supplied, the file wins.",
            },
            slackChannel: { type: "string", minLength: 1, description: "Optional. Slack channel to post progress into." },
        ...
        execute: async (_callId: unknown, input: unknown) => {
          const { requester, request, requestPath, slackChannel, slackThread, budgetUsd, relevantConcepts } = input as {
            requester: string; request?: string; requestPath?: string; slackChannel?: string; slackThread?: string; budgetUsd?: number;
            relevantConcepts?: Array<{ id: string; path?: string; summary?: string; tags?: string[]; content?: string }>;
          };
          ...
          let requestSourcePath: string | undefined;
          ...
          if (res.awaitingConfirmation === true) {
            return {
              ...
              details: {
                ...
                feedback: {
                  instruction:
                    "STOP and show the question verbatim. Wait for the authenticated requester to reply naturally, then call harness_answer with answeredBy:'human', invokedBy matching the host requester, and the current clarification identity. Do not approve on their behalf or start another run.",
                  answerWith: "natural_language_reply",
                },
              },
            };
  ...
  disposers.push(
    toDispose(
      api.registerTool({
        name: "harness_health",
        description:
          "Return a health snapshot: DB reachable, schema OK, config well-formed, effective model routes/auth resolvable, credentials configured. Pass { deep: true } to probe the configured backends.",
        parameters: {
          type: "object",
          properties: {
            deep: {
        ...
        execute: async (_callId: unknown, input: unknown) => {
          ...
          // when the real fault is the key. Fatal, same rationale as git auth.
          {
            ...
            if (!vaultErr) {
              ...
              } catch (err) { detail = `open, but listing failed: ${String(err)}`; }
            }
      ...
      api.registerTool({
        ...
        execute: async (_callId: unknown, input: unknown) => {
          ...
          if (row.status === "awaiting_clarification" || (row.clarification_question && row.clarification_answer == null)) {
            return { content: [{ type: "text", text: `Cannot resume ${sessionId}: a pending approval/clarification cannot be bypassed with force. Relay the current question and wait for the authenticated requester to answer it.` }],
  ...
  const buildAnswer = (toolContext: HarnessToolContext): HarnessToolDefinition => ({
        name: "harness_answer",
      description:
        "Answer a paused harness clarification after interpreting the authenticated requester's natural-language reply, " +
        "or as delegated automation where policy permits it. Human answers are accepted only when OpenClaw supplies a " +
        "trusted requesterSenderId that exactly matches invokedBy and is listed in slack.authorised_users. Brief approval, " +
        "revision, and budget grants cannot be delegated. Include the current clarification identity; automation also requires evidence.",
      parameters: {
        type: "object",
        properties: {
          sessionId: { type: "string", minLength: 1 },
          ...
          answeredBy: {
            ...
            description:
              "Use 'human' only when interpreting the current host-authenticated requester's natural-language answer. " +
              "The tool verifies requesterSenderId against invokedBy and slack.authorised_users. Use 'automation' only for a delegated agent decision.",
          },
          evidence: {
            type: "string",
            description:
        ...
        required: ["sessionId", "answer", "invokedBy", "answeredBy"],
        additionalProperties: false,
      },
      execute: async (_callId: unknown, input: unknown) => {
        ...
        // rc.13: model-supplied identity is only an audit assertion. The tool
        // factory context is host-authenticated, so human authority exists only
        // when requesterSenderId is present, exactly matches invokedBy, and is
        // in the configured allow-list. OpenClaw may interpret that requester's
        // natural-language answer and call this tool; users never call the
        // harness directly. Missing or mismatched host provenance fails closed.
        ...
        if (
          ...
          return {
            content: [{
              type: "text",
              text:
                `Authenticated requester ${trustedSender || "(missing)"} is not authorised for harness_answer ` +
                `(claimed invokedBy=${invokedBy ?? "(missing)"}).`,
            }],
        ...
        const row = liveDb()
          .prepare(
            `SELECT status, crystallised_prompt, lead_plan_json, clarification_question, clarification_seq, clarification_id,
                    ...
                    requester_gh, plan_revision, cycles_ran
          ...
          .get(sessionId) as {
            ...
            requester_gh?: string | null; plan_revision?: number | null; cycles_ran?: number | null;
          } | undefined;
        if (!row) return { content: [{ type: "text", text: `No session ${sessionId}` }], details: { ok: false, notFound: true } };
          ...
          if (
            ...
            if (proposal.completeAmendment && proposal.proposalHash) {
              const observedHash = createHash("sha256")
                .update(JSON.stringify(proposal.completeAmendment))
                .digest("hex");
        ...
        let resumedPendingAmendmentId: string | null = null;
        if (clarificationId) {
          try {
            const prior = liveDb().prepare(
              `SELECT id, answer_hash, status FROM task_contract_amendments
                WHERE session_id = ? AND clarification_id = ?`,
            ).get(sessionId, clarificationId) as { id: string; answer_hash: string; status: string } | undefined;
            if (prior) {
        ...
        const automated = effectiveAnsweredBy === "automation";
        ...
        if (typeof clarificationSeq === "number" && Number.isFinite(clarificationSeq) && clarificationSeq !== seq) {
          ...
          return {
            content: [{
              type: "text",
              text:
                `Not answering: you addressed clarification ${clarificationSeq}, but ${sessionId} is now paused on ` +
        ...
        //
        // Human provenance was already established from the host-authenticated
        // tool factory context. Automation remains separately constrained by
        // delegation policy and evidence below.
        ...
        if (automated && isBudgetExtensionPause(row.clarification_subtask)) {
          ...
          return {
            content: [{
              type: "text",
              text:
                `Not answering: a BUDGET extension cannot be granted by an agent. This question asks for more ` +
        ...
        if (automated && liveConfig().loop.clarification_auto_accept_delegated !== true) {
          ...
          return {
            content: [{
              type: "text",
              text:
                `Not answering: this deployment has not delegated clarification answers to an agent ` +
        ...
        if (
          ...
          try {
            const paused = JSON.parse(row.clarification_subtask) as { task?: LeadPlanSubTask };
            if (paused.task) {
              ...
              if (conflicts.length > 0) {
                ...
                if (candidate.ok) precomputedAmendment = candidate.amendment;
              }
            }
          } catch {
          ...
          try {
            ...
            if (resumedPendingAmendmentId) {
              ...
              const claimed = liveDb()
                .prepare(
                  `UPDATE sessions SET clarification_answer = ?, updated_at = ?
            ...
            if (claimedChanges === 1 && precomputedAmendment && !resumedPendingAmendmentId) {
              liveDb().prepare(
                ...
                precomputedAmendment.id,
                sessionId,
                row.clarification_id ?? "",
                Math.max(1, row.cycles_ran ?? 1),
                seq,
                precomputedAmendment.version,
                precomputedAmendment.basePlanHash,
                precomputedAmendment.baseTaskHash,
                createHash("sha256").update(trimmed).digest("hex"),
                invokedBy,
                JSON.stringify(precomputedAmendment.originalTask),
                JSON.stringify(precomputedAmendment.revisedTask),
                JSON.stringify(precomputedAmendment.substitution),
                JSON.stringify(precomputedAmendment.changedFields),
                Date.now(),
              );
            }
            liveDb().exec("COMMIT");
        ...
        if (isTimeExtensionPause(row.clarification_subtask) || budgetPause) {
          ...
          if (prUrl) {
            try { closeActiveDeadline(liveDb(), sessionId); } catch { /* legacy/unmigrated test row */ }
            liveDb()
              .prepare(
                `UPDATE sessions SET status = 'done', merge_recommendation = 'needs_human_review',
                        ...
                        clarification_subtask = NULL, clarification_heartbeat_at = NULL,
                        terminal_cause = 'time_extension_listener_lost',
                        terminal_classification = 'recovery_degraded', updated_at = ?
                  WHERE id = ?`,
              )
              .run(
          ...
          liveDb()
            .prepare(
              `UPDATE sessions SET status = 'aborted', worktree_preserved = 1, clarification_question = NULL,
                      ...
                      terminal_cause = 'time_extension_listener_lost',
                      terminal_classification = 'recovery_degraded',
                      updated_at = ? WHERE id = ?`,
            )
            .run(Date.now(), sessionId);
          return {
        ...
        if (/^(abort|cancel)\b/i.test(trimmed)) {
          try { closeActiveDeadline(liveDb(), sessionId); } catch { /* legacy/unmigrated test row */ }
          liveDb().prepare(
            `UPDATE sessions SET status = 'aborted', terminal_cause = 'user_cancel',
                                 terminal_classification = 'operator_cancelled', updated_at = ? WHERE id = ?`,
          ).run(Date.now(), sessionId);
        ...
        if (isBriefConfirmationPause(row.clarification_subtask)) {
          const stateLimits = liveDb().prepare(
            "SELECT budget_usd, hard_timeout_seconds FROM sessions WHERE id = ?",
          ).get(sessionId) as { budget_usd: number; hard_timeout_seconds: number | null };
          const base: BriefState = {
            sessionId,
            brief,
          ...
          const pause = JSON.parse(row.clarification_subtask!) as { kind: string; briefProposal?: BriefProposal };
          const revision = /^revise brief:\s*([\s\S]*)$/i.exec(trimmed);
          const proposalConfirmation = /^confirm brief ([a-f0-9]{64})$/i.exec(trimmed);
          const keepPaused = (text: string, details: Record<string, unknown> = {}) => {
            liveDb().prepare(
              "UPDATE sessions SET clarification_answer = NULL, updated_at = ? WHERE id = ?",
            ).run(Date.now(), sessionId);
            return { content: [{ type: "text" as const, text }], details: { ok: true, sessionId, started: false, awaitingConfirmation: true, ...details } };
          };
          if (revision) {
            ...
            try {
              ...
              staging = true;
              const staged = proposalDb.prepare(
                "UPDATE sessions SET clarification_subtask = ?, clarification_question = ?, clarification_answer = NULL, minimum_runtime_version = '2.0.0-rc.13', updated_at = ? WHERE id = ? AND status = 'awaiting_clarification' AND clarification_subtask = ? AND crystallised_prompt = ? AND budget_usd = ? AND COALESCE(hard_timeout_seconds, ?) = ?",
              ).run(JSON.stringify({ kind: BRIEF_CONFIRMATION_KIND, briefProposal: proposal }), question, Date.now(), sessionId,
                row.clarification_subtask ?? null, row.crystallised_prompt, base.budgetUsd, base.hardTimeoutSeconds, base.hardTimeoutSeconds);
              if (staged.changes !== 1) throw new Error("Brief changed while staging proposal");
              ...
              if (/A brief revision must contain 1–8,000 characters/.test(String(err))) {
                return keepPaused("No changes applied: the brief revision is too long (maximum 8,000 characters).", { ok: false, proposalRejected: true });
              }
          ...
          if (pause.briefProposal !== undefined || proposalConfirmation) {
            if (!pause.briefProposal || !proposalConfirmation || !verifyBriefProposal(
              pause.briefProposal, proposalConfirmation[1]!.toLowerCase(), base, liveConfig().budgets?.session_hard_ceiling_usd,
            )) {
          ...
          try {
            const current = confirmationDb.prepare(
              "SELECT status,clarification_subtask,crystallised_prompt,budget_usd,hard_timeout_seconds FROM sessions WHERE id = ?",
            ).get(sessionId) as { status: string; clarification_subtask: string; crystallised_prompt: string; budget_usd: number; hard_timeout_seconds: number | null };
          ...
          if (unpersisted.length > 0) {
            ...
            if (activated.changes !== 1) throw new Error("Brief changed before activation");
          ...
          liveState().audit(
            "tool.answer_brief_confirmed",
            { sessionId, answerLen: trimmed.length, invokedBy: invokedBy ?? null, proposalHash: proposalConfirmation?.[1] ?? null, budgetApplied: budgetApplied ?? null, timeoutApplied: timeoutApplied ?? null, effectiveLimits },
            sessionId,
          );
          void liveRuntime().loop.run(sessionId, brief).catch((err) => {
        ...
        if (acceptsCommittedWork) {
          ...
          const ledger = liveDb()
            .prepare(
              `SELECT commit_sha FROM sub_tasks
                 WHERE session_id = ? AND seq = ?
                   AND cycle = (SELECT MAX(cycle) FROM sub_tasks WHERE session_id = ? AND seq = ?)`,
            )
            .get(sessionId, seq, sessionId, seq) as { commit_sha?: string | null } | undefined;
          const hasCommittedWork =
            !!(ledger?.commit_sha ?? "").trim() || expectedPaths.size > 0 || actualPaths.length > 0;
          if (!hasCommittedWork) {
            ...
            return {
              content: [{
                type: "text",
                text:
                  `Not accepting: sub-task ${seq} has no commit, so there is no committed work to accept. ` +
          ...
          brief.outOfScope.push(
            pausedTitle || pausedIntent
            ...
            if (paused.task) {
              ...
              if (blockedPaths.length > 0) {
                const amendment = precomputedAmendment
                  ? ({ ok: true as const, amendment: precomputedAmendment })
                  : buildArtifactSubstitutionAmendment({
                      plan: storedPlan,
                      task: paused.task,
                      answer: amendmentAnswer,
                ...
                if (!amendment.ok) {
                  ...
                  liveDb().prepare(
                    `UPDATE sessions SET clarification_question = ?, clarification_id = ?,
                                         clarification_answer = NULL, clarification_subtask = ?,
                                         updated_at = ? WHERE id = ?`,
                  ).run(
                    `I could not safely activate that answer automatically: ${amendment.reason}. ` +
                ...
                if (residual.length > 0) {
                  try {
                    liveDb().prepare(
                      `UPDATE task_contract_amendments SET status = 'rejected', rejection_reason = ?
                        WHERE id = ? AND status = 'pending'`,
                    ).run("revised contract still conflicts with safety policy", amendment.amendment.id);
                ...
                try {
                  ...
                  const activated = liveDb().prepare(
                    `UPDATE task_contract_amendments
  ...
  disposers.push(
    toDispose(
      api.registerTool({
        ...
        description:
          "Per-user git credential onboarding and management (DM flow). One person can hold DIFFERENT tokens for different orgs, so credentials are keyed by provider+org: " +
        ...
        execute: async (_callId: unknown, input: unknown) => {
          ...
          const providerApiBase = (p: GitProvider): string =>
            liveConfig().pat_routing?.providers?.[p]?.api_base ??
           ...
           */
          const configRouteFor = (p: GitProvider, org: string) => {
            const orgs = (liveConfig().pat_routing as unknown as Record<string, unknown> | undefined)?.[p] as
              | Record<string, Record<string, { slack_user_id?: string }>>
              | undefined;
            const node = orgs?.[org] ?? orgs?.[org.toLowerCase()];
            if (!node) return undefined;
            for (const [person, entry] of Object.entries(node)) {
              if (entry?.slack_user_id === requester) return { person };
            }
            return undefined;
          };

          /** A concrete allowed repo in this org, so reach can actually be tested. */
          const allowedRepoIn = (org: string): string | undefined =>
            liveConfig().repos.allowed.find(
              (r) => !r.includes("*") && r.split("/")[0]?.toLowerCase() === org.toLowerCase(),
            );

          /** Slack is best-effort for the management actions; absent, they still work. */
          const tryOnboardSlack = async (): Promise<OnboardingSlack | null> => {
          ...
          const describeRoute = (r: { provider: string; org: string; providerLogin?: string; tokenExpiresAt?: number }) => {
            const who = r.providerLogin ? ` as \`${r.providerLogin}\`` : "";
            if (!r.tokenExpiresAt) return `• \`${r.provider}\` / \`${r.org}\`${who}`;
          ...
          if (action === "remove") {
            ...
            if (slack && dmChannel) await slack.postDm(dmChannel, `:wastebasket: Removed your \`${p}\` credential for \`${org}\`.`);
            return {
          ...
          if (action === "add" || action === "replace") {
            ...
            if (!valid.ok) {
              ...
              const slack = await tryOnboardSlack();
              if (slack && dmChannel) await slack.postDm(dmChannel, `:x: That token didn't validate (${valid.error ?? "unknown"}). Please try again with a valid token.`);
            ...
            const email =
              commitEmail ?? existing?.commitEmail ?? valid.email ??
            ...
            try {
              ...
              // metadata.
              if (action === "add") {
                try { liveRuntime().vault.delete(vaultService); } catch { /* leave the orphan; nothing points at it */ }
              }
            ...
            const slack = await tryOnboardSlack();
            if (slack && dmChannel && promptTs) await slack.deleteOwnMessage(dmChannel, promptTs);
            const expiryNote = valid.expiresAt ? ` It expires on ${new Date(valid.expiresAt).toISOString().slice(0, 10)}.` : "";
            if (slack && dmChannel) {
              await slack.postDm(
                dmChannel,
                `:white_check_mark: Stored your \`${p}\` token for \`${org}\`${valid.login ? ` (validated as \`${valid.login}\`)` : ""}.${expiryNote}` +
            ...
            return {
              content: [{
                type: "text",
                text:
                  `${action === "add" ? "Added" : "Replaced"} your \`${p}\` credential for \`${org}\`` +
          ...
          if (!valid.ok) {
            ...
            if (dmChannel) await onboard.postDm(dmChannel, `:x: That token didn't validate (${valid.error ?? "unknown"}). Please try again with a valid token.`);
            return { content: [{ type: "text", text: `Token failed validation (${valid.error ?? "unknown"}); NOT stored.` }], details: { ok: false, invalidToken: true, error: valid.error } };
          }
          ...
          if (dmChannel && promptTs) await onboard.deleteOwnMessage(dmChannel, promptTs);
          if (dmChannel) {
            await onboard.postDm(
              dmChannel,
              `:white_check_mark: Stored your ${provider ?? "github"} token as \`${vaultService}\`` +
  ...
  function buildReviseBrief(
    ...
    allFindings.forEach((f, i) => {
      ...
      if (isConditionalFinding(f)) {
        ...
        findingLines.push(
          `${base} -- CONDITIONAL PREMISE: this finding's action depends on an unverified claim about repo state. ` +
