...
const CAPABILITIES: Record<string, HelpCapability[]> = {
  starting: [
    {
  ...
  after: [
    ...
    {
      what:
        "Reconnect a run that failed AFTER it had already opened a pull request, so that pull request can be revised " +
...
export function buildHarnessHelp(topic = "all"): HarnessHelp {
  const wanted = topic === "all" || !(topic in CAPABILITIES) ? Object.keys(CAPABILITIES) : [topic];
  const capabilities: Record<string, HelpCapability[]> = {};
  for (const k of wanted) capabilities[k] = CAPABILITIES[k]!;
  return {
    summary:
      "I can take a change you describe in plain English, plan it against the actual repository, write it, review it adversarially, and open a pull request — then keep working on that pull request until the review signs off.",
    capabilities,
    limits: LIMITS,
    tools: ALL_TOOLS,
  };
