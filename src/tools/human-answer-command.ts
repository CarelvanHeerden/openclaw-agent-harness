...
export function pendingAnswerState(db: DatabaseSync, sessionId: string) {
  const row = db.prepare(`SELECT id, status, requester, requester_gh, crystallised_prompt,
    ...
    branch, cost_usd FROM sessions WHERE id = ?`).get(sessionId) as any;
...
export function answerStateHash(value: unknown): string {
 ...
 */
export function pendingAnswerReview(state: any): string {
  const plan = parseJson(state.lead_plan_json) as { subTasks?: unknown[]; [key: string]: unknown } | null;
  const subTasks = plan && typeof plan === "object" && Array.isArray(plan.subTasks) ? plan.subTasks : [];
  const seq = state.clarification_seq;
  const matchingTasks = subTasks.filter((task: any) => task?.seq === seq);
...
export function reviewPages(state: unknown): string[] {
...
export function renderHumanAnswerReview(
  state: any,
  sessionId: string,
  challenge: string,
  page: number,
...
export function issueHumanAnswer(db: DatabaseSync, sessionId: string, sender: string): { text: string } {
  ...
  db.prepare(`INSERT INTO human_answer_challenges
    ...
    VALUES (?, ?, ?, ?, ?, ?, 1)`).run(
      challenge, sessionId, sender, answerStateHash(state), now + 10 * 60_000, pages.length,
    );
