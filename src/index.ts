...
export function bootstrapHarnessSync(api: HarnessPluginApi): HarnessRuntime {
  ...
  const controlMergeProvider = {
    inspect: async ({ repository, prNumber }: { repository: string; prNumber: number }) => {
      ...
      const proposal = state.db.prepare(`SELECT p.*, r.base_ref, r.policy_digest, r.authority_envelope_json
        FROM control_proposals p JOIN control_runs r ON r.id=p.run_id WHERE p.pr_number=? AND r.repository=?`).get(prNumber, repository) as Record<string, unknown>;
  ...
  runtime.controlPlane = new ControlPlaneService({
    ...
    executeEngine: async (change) => {
      ...
      change.assertCurrent();
      const existingSession = state.db.prepare(`SELECT status,pr_number,final_pr_url,published_sha FROM sessions WHERE id=?`).get(change.changeId) as
      ...
      ) SELECT verification_status,commit_sha FROM ranked WHERE rank=1`).all(change.changeId) as Array<{ verification_status: string | null; commit_sha: string | null }>;
