...
export function bootstrapHarnessSync(api: HarnessPluginApi): HarnessRuntime {
  ...
  const loop = new OrchestratorLoop({
    ...
    deliverProgress: (sessionId, status) => {
      ...
      if (!poster) return; // no token -> poll-model fallback
      if (config.slack.native_progress_delivery === false) return;
      let bind: { slack_channel?: string; slack_thread?: string } | undefined;
      try {
        bind = state.db
    ...
    postWarning: (sessionId, text) => {
      ...
      const channel = bind?.slack_channel ?? "";
      const thread = bind?.slack_thread ?? "";
      if (!hasRealSlackBinding(channel, thread)) return;
      if (!text) return;
  ...
  const runtime: HarnessRuntime = {
    ...
    mergePr: async ({ sessionId, invokedBy, repairBudgetUsd }) => {
      ...
      const rec = (row.merge_recommendation ?? "do_not_merge") as "merge" | "do_not_merge" | "needs_human_review";
