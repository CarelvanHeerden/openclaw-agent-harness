 ...
 * is found.
 */
export function parseOkfBlocksFromContext(text: string | undefined): ParsedOkfConcept[] {
  ...
  const results: ParsedOkfConcept[] = [];
  ...
  for (const block of blocks) {
    ...
    for (const raw of lines.slice(1)) {
      ...
      if (summary === undefined) summary = line;
    }

    if (!id) continue; // No id => not a real OKF block.
    const concept: ParsedOkfConcept = { id };
    if (summary) concept.summary = summary;
    else if (h3Line) concept.summary = h3Line;
    if (tags && tags.length > 0) concept.tags = tags;
...
export class OkfConceptCache {
  private readonly entries = new Map<string, { concepts: ParsedOkfConcept[]; updatedAt: number }>();
...
export function decideAutoForward(input: {
...
}): { inject: false } | { inject: true; concepts: ParsedOkfConcept[]; injectionSite: "root" | "brief" } {
  const { toolName, params, cached } = input;
  if (toolName !== "harness_run" && toolName !== "harness_start_session") {
    return { inject: false };
  }
  if (!cached || cached.length === 0) return { inject: false };
  ...
  const injectionSite: "root" | "brief" = toolName === "harness_run" ? "root" : "brief";
  const p = (params ?? {}) as Record<string, unknown>;
  if (injectionSite === "root") {
    const existing = p.relevantConcepts;
    if (Array.isArray(existing) && existing.length > 0) return { inject: false };
  } else {
 ...
 */
export function buildRewrittenParams(
  toolName: string,
  params: unknown,
 ...
 */
export function cacheKeyForCtx(ctx: unknown): string {
  const c = (ctx ?? {}) as { sessionKey?: unknown; sessionId?: unknown };
  if (typeof c.sessionKey === "string" && c.sessionKey.length > 0) return c.sessionKey;
