...
export const CAPTURED_OPENCODE_VERSIONS = ["1.18.11"] as const;
 ...
 */
export function assessOpenCodeVersion(
  reported: string | undefined,
  expected: string = PINNED_OPENCODE_VERSION,
): VersionAssessment {
  ...
  if (!reported || !reported.trim()) {
    return {
      ...
      warn: true,
      message:
        `the agent did not report a version; expected ${expected}. ` +
        `Behaviour is unverified against this build — the startup permission probe is what actually gates it.`,
    };
  }
  if (reported.trim() === expected) return { ...base, relation: "exact", warn: false };
  ...
  const relation: VersionRelation = a[0] !== b[0] ? "major" : a[1] !== b[1] ? "minor" : "patch";
  const severity =
    relation === "major"
  ...
  return {
    ...
    warn: true,
    message:
      `running opencode ${reported}, pinned at ${expected}. ${severity} ` +
