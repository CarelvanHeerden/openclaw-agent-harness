...
async function structuredCall<T>(params: {
  ...
  streamOpenTimeoutSeconds?: number;
  /**
   * v2.0.0: return the raw reply WITHOUT extracting or parsing it.
   *
   * For callers driving `runStructuredLadder`, which owns extraction,
   * validation and repair. Parsing here as well would throw away the raw text
...
export async function runLeadSdk(params: {
  ...
  const report = (info: LeadAttemptInfo): void => {
    try {
  ...
  try {
    ...
    report({
      ...
      costUsd: (err as StructuredCallError).costUsd ?? 0,
      outputChars: (err as StructuredCallError).rawText?.length ?? 0,
    ...
    try {
      ...
      report({
        ...
        costUsd: (err2 as StructuredCallError).costUsd ?? 0,
        outputChars: (err2 as StructuredCallError).rawText?.length ?? 0,
        rung,
        error: String((err2 as Error)?.message ?? err2).slice(0, 300),
      });
      ...
      let terminal: unknown = err2;
      const fault = describeJsonSyntaxFault(err2);
      if (fault && params.leadSyntaxRetryEnabled !== false) {
        params.logger?.warn?.(
          "[lead] plan was COMPLETE but not valid JSON; retrying ONCE with the parse error quoted back (beta.128)",
          { fault: fault.slice(0, 300) },
        );
        const spentBeforeRepair = (err2 as StructuredCallError).costUsd ?? spentSoFar;
        try {
          const r3 = await call(
          ...
          (err3 as StructuredCallError).costUsd = spentBeforeRepair + ((err3 as StructuredCallError).costUsd ?? 0);
          report({
            attempt: 3,
            outcome: classifyAttempt(err3),
            costUsd: (err3 as StructuredCallError).costUsd ?? 0,
            outputChars: (err3 as StructuredCallError).rawText?.length ?? 0,
            rung: "syntax_repair",
            error: String((err3 as Error)?.message ?? err3).slice(0, 300),
          });
          terminal = err3;
        }
      }
      ...
      if (params.leadSalvageEnabled === false) throw terminal;
      ...
      const salvaged =
        salvageLeadPlan(terminal as StructuredCallError) ??
        salvageLeadPlan(err2 as StructuredCallError) ??
        salvageLeadPlan(err as StructuredCallError);
      if (salvaged) {
        params.logger?.warn?.(
        ...
        return {
          ...salvaged,
          costUsd: (terminal as StructuredCallError).costUsd ?? (err2 as StructuredCallError).costUsd ?? spentSoFar,
          tokensIn: 0,
          tokensOut: 0,
        };
      }
...
type ReviewDoc = { verdict: "pass" | "revise" | "block"; findings: unknown[]; summary: string };
...
async function reviewOnce(
  ...
  return runStructuredLadder<ReviewDoc>({
    role: "adversary",
 ...
 */
export const CLAUDE_CODE_CAPABILITIES: BackendCapabilities = {
  id: "claude-code",
  toolUse: true,
