...
function seal(key: Buffer, plaintext: string, aad: string): SealedRow {
...
function unseal(key: Buffer, row: SealedRow, aad: string): string {
 ...
 */
export function parseVaultKey(raw: string, source: string): Buffer {
  const s = raw.trim();
  if (!s) throw new VaultKeyError(`vault key from ${source} is empty`);
  ...
  if (!buf || buf.length !== KEY_BYTES) {
    throw new VaultKeyError(
      `vault key from ${source} must be ${KEY_BYTES} bytes as 64 hex chars or base64; got ${s.length} chars`,
    );
  }
  return buf;
}
...
function writeKeyFile(path: string, key: Buffer): void {
...
export class CredentialVault {
  ...
  static open(opts: CredentialVaultOptions): CredentialVault {
    ...
    const keyFilePath = resolve(
      opts.keyFile ?? process.env[VAULT_KEY_FILE_ENV] ?? join(dir, "vault.key"),
    );
    ...
    let key: Buffer;
    let keySource: "env" | "file" | "generated";
    ...
    if (fromEnv && fromEnv.trim()) {
      ...
      keySource = "generated";
      opts.logger.warn(
        "[vault] no key found; generated a new one. BACK THIS FILE UP -- without it every stored credential is unrecoverable.",
        { keyFilePath },
      );
    }
    ...
    if (existsSync(dbPath)) {
      try { chmodSync(dbPath, 0o600); } catch { /* best effort; WAL siblings inherit the dir mode */ }
    }
  ...
  private verifyKey(): void {
    const row = this.db
      ...
      .get(VERIFIER_KEY) as SealedRow | undefined;
    if (!row) {
    ...
    try {
      ...
      throw new VaultKeyError(
        `the vault key does not match ${this.dbPath}. The key came from ${this.describeKeySource()}. ` +
  ...
  private describeKeySource(): string {
    if (this.keySource === "env") return `$${this.opts.keyEnvVar ?? VAULT_KEY_ENV}`;
    return this.keyFilePath;
  }

  /** Resolve a secret. Returns undefined when absent; throws only on a broken vault. */
  get(service: string, type: "token" | "api_key" = "token"): string | undefined {
    const row = this.db
      .prepare("SELECT iv, tag, ciphertext FROM credentials WHERE service = ?")
      .get(service) as SealedRow | undefined;
    if (!row) {
    ...
    try {
      ...
      throw new VaultKeyError(
        `credential '${service}' failed authenticated decryption; the row is corrupt or was tampered with`,
      );
    }
  ...
  set(service: string, value: string, opts?: { type?: string; notes?: string }): void {
    if (!service.trim()) throw new Error("vault: service name is required");
    if (!value) throw new Error(`vault: refusing to store an empty value for '${service}'`);
    const sealed = seal(this.key, value, service);
    const now = Date.now();
    const existing = this.db.prepare("SELECT created_at FROM credentials WHERE service = ?").get(service) as
      | { created_at: number }
      | undefined;
    this.db
      .prepare(
        `INSERT OR REPLACE INTO credentials (service, type, iv, tag, ciphertext, notes, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        service,
  ...
  delete(service: string): boolean {
    ...
    if (removed) this.opts.audit?.("vault.delete", { service });
    return removed;
  }
  ...
  list(): CredentialRecord[] {
    const rows = this.db
      ...
      .all() as Array<{ service: string; type: string; notes: string | null; created_at: number; updated_at: number }>;
    return rows.map((r) => ({
      service: r.service,
      type: r.type,
   ...
   * next boot.
   */
  rotate(): { rotated: number; keyFilePath: string } {
    if (this.keySource === "env") {
      throw new VaultKeyError(
        `refusing to rotate: the active key comes from $${this.opts.keyEnvVar ?? VAULT_KEY_ENV}, which would keep ` +
    ...
    const rows = this.db
      ...
      .all() as unknown as Array<{ service: string } & SealedRow>;
    ...
    try {
      ...
      try { unlinkSync(staged); } catch { /* the staged key is now meaningless */ }
      throw err;
    }
