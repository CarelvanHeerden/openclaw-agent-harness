...
export async function listPullRequestCommits(input: {
  ...
  ghToken: string;
  apiBase?: string;
}): Promise<{ shas: string[]; truncated: boolean }> {
  const apiBase = input.apiBase ?? "https://api.github.com";
  const shas: string[] = [];
  let truncated = false;
  for (let page = 1; page <= 3; page += 1) {
    const res = await fetch(
      `${apiBase}/repos/${input.repoFullName}/pulls/${input.prNumber}/commits?per_page=100&page=${page}`,
      { headers: GH_HEADERS(input.ghToken) },
    );
    if (!res.ok) {
      throw new Error(`GitHub list PR #${input.prNumber} commits failed ${res.status}: ${(await res.text()).slice(0, 300)}`);
    }
    const page$ = (await res.json()) as Array<{ sha: string }>;
    for (const c of page$) shas.push(c.sha);
    if (page$.length < 100) return { shas, truncated: false };
...
export async function getMergeBase(input: {
  ...
  const apiBase = input.apiBase ?? "https://api.github.com";
  const res = await fetch(
    `${apiBase}/repos/${input.repoFullName}/compare/${encodeURIComponent(input.base)}...${encodeURIComponent(input.head)}`,
    { headers: GH_HEADERS(input.ghToken) },
  );
  ...
  const j = (await res.json()) as { merge_base_commit?: { sha?: string } };
