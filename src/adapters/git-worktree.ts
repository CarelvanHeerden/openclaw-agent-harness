...
export function isHarnessScratch(path: string): boolean {
...
export interface GitAdapterOptions {
  ...
  /**
   * beta.114: pathspecs whose changes are reverted rather than committed --
   * generated trees that belong to the repo's own tooling, not to any feature.
   * Configured per repo (`repos.never_commit_paths`), never inferred. Empty by
   * default. See revertNeverCommitPaths.
   */
...
`;
   ...
   */
  private freeDiskBytes(p: string): number | null {
    try {
  ...
  private async bootstrapWorktreeDeps(worktreePath: string): Promise<void> {
    try {
      ...
      if (floor > 0) {
        ...
        if (free !== null && free < floor) {
          this.opts.logger.error(
            `[git-worktree] BLOCKING: only ${(free / 1e6).toFixed(0)}MB free on the worktrees filesystem ` +
              ...
              `A test-authoring sub-task cannot execute its tests without a healthy node_modules.`,
            { worktreePath, freeBytes: free, floorBytes: floor, event: "harness.worktree_disk_low" },
          );
          return;
        }
      }
      ...
      if (existsSync(nm)) {
        try {
          if (readdirSync(nm).length > 0 && this.declaredCheckBinsPresent(worktreePath)) return;
      ...
      if (looksLikeDiskExhaustion(msg)) {
        this.opts.logger.error(
          `[git-worktree] BLOCKING: dep bootstrap hit DISK EXHAUSTION and node_modules may be CORRUPT ` +
            `in ${worktreePath}. Free disk on the harness host and re-run; a corrupt node_modules makes ` +
            `every inline install fail too, so a test sub-task will commit an UNRUN test. ${this.redactSafe(msg)}`,
          { worktreePath, event: "harness.worktree_bootstrap_disk_exhaustion" },
        );
      } else {
  ...
  private redactSafe(text: string): string {
   ...
   */
  private declaredCheckBinsPresent(worktreePath: string): boolean {
    try {
      ...
      for (const [name, cmd] of Object.entries(scripts)) {
        if (!/^(lint|typecheck|type-check|tsc|test|okf:check)$/i.test(name)) continue;
        const c = String(cmd);
        if (/\beslint\b/.test(c)) wanted.add("eslint");
        if (/\btsx\b/.test(c)) wanted.add("tsx");
        if (/\btsc\b/.test(c)) wanted.add("tsc");
        if (/\bvitest\b/.test(c)) wanted.add("vitest");
        if (/\bjest\b/.test(c)) wanted.add("jest");
      }
      if (wanted.size === 0) return true;
      for (const bin of wanted) {
        if (!existsSync(join(binDir, bin))) return false;
      }
      return true;
    } catch {
      return true;
  ...
  private async branchTipSha(bare: string, branch: string): Promise<string> {
    try {
   ...
   * insufficient.
   */
  private async reconcileBranchWorktrees(bare: string, branch: string, targetPath: string): Promise<void> {
    try {
      ...
      for (const g of groups) {
        ...
        if (existsSync(wtPath)) await this.robustRemoveDir(wtPath).catch(() => undefined);
      }
      await this.run(["-C", bare, "worktree", "prune"]).catch(() => "");
    } catch (err) {
   ...
   */
  async pathIntroducedSince(
    worktreePath: string,
    base: string,
    path: string,
  ): Promise<{ introduced: boolean; changeType: string; detail: string }> {
    if (!base) return { introduced: false, changeType: "", detail: "no base sha" };
    const out = await this.run([
   ...
   */
  async pathRenamedAwaySince(
    worktreePath: string,
    base: string,
    path: string,
  ): Promise<{ renamed: boolean; to: string; score: string; detail: string }> {
    const miss = { renamed: false, to: "", score: "", detail: "" };
    if (!base) return { ...miss, detail: "no base sha" };
    const out = await this.run([
      "-C", worktreePath, "log", `${base}..HEAD`, "--diff-filter=R", "--name-status", "-M", "--pretty=format:",
    ]).catch(() => "");
    const want = path.replace(/^\.?\//, "");
    for (const line of out.split("\n")) {
      const parts = line.trim().split("\t").filter(Boolean);
      // R<score> <old> <new> -- anything shorter is not a rename record.
      ...
      if (!from || !to) continue;
      if (from === want || from.endsWith(`/${want}`) || want.endsWith(`/${from}`)) {
        return {
          renamed: true,
          to,
          score: status,
          detail: `renamed to ${to} in ${base.slice(0, 7)}..HEAD (${status})`,
        };
      }
    }
    return { ...miss, detail: `not renamed away in ${base.slice(0, 7)}..HEAD` };
  }
   ...
   */
  async remoteBranchExistsByUrl(repoFullName: string, branch: string, ghToken: string): Promise<boolean> {
   ...
   */
  private async sweepCommitMsgScratch(worktreePath: string): Promise<string[]> {
    const swept: string[] = [];
    ...
    try {
      ...
      for (const path of tracked.split("\n").map((l) => l.trim()).filter(Boolean)) {
        if (!isCommitMsgNoise(path)) continue;
    ...
    try {
      ...
      for (const line of status.split("\n")) {
        ...
        if (!path || !isCommitMsgNoise(path) || swept.includes(path)) continue;
   ...
   */
  private async sweepScratchDir(worktreePath: string): Promise<boolean> {
  ...
  /**
   * beta.114: drop changes to paths this repo has declared off-limits.
   *
   * The b113 DR/BCP run shipped a PR of 154 files. Thirteen were the feature.
   * The other 141 were `okf/**`, ProjectThanos's generated documentation
   * bundle, regenerated by a worker as a side effect and swept in by the
   * unscoped `add -A`. The adversary noticed twice and filed it both times as
   * `codebase-fit`/`info`, so it never blocked, never got cleaned up, and left
   * the PR conflicting with main.
   *
   * b110's exclusions cannot reach this. They are written to
   * `.git/info/exclude`, which git consults only for UNTRACKED files, and
   * `okf/` is 1,496 tracked files at origin/main. Excluding a tracked path
   * there is a no-op.
   *
   * So this runs AFTER staging and reverts, rather than before and ignores.
   * Both halves are needed: unstaging alone leaves the files dirty in the
   * worktree, and the next sub-task's `add -A` stages them straight back.
   *
   * Configured per repo, never inferred. A generated tree is indistinguishable
   ...
   */
  private async revertNeverCommitPaths(
    worktreePath: string,
    authorizedPaths: readonly string[] = [],
  ): Promise<string[]> {
    const globs = (this.opts.neverCommitPaths ?? []).map((g) => g.trim()).filter(Boolean);
    if (globs.length === 0) return [];
    const isAuthorized = (file: string) =>
      authorizedPaths.some((p) => (p.endsWith("/") ? file.startsWith(p) : file === p));
    try {
      ...
      const out = await this.run([
        "-C", worktreePath, "diff", "--cached", "--name-only", "--", ...globs,
      ]);
      const matched = out.split("\n").map((l) => l.trim()).filter(Boolean);
      if (matched.length === 0) return [];
      // Belt and braces on the pathspec.
      //
      // `git diff --cached --name-only --` with NOTHING after the separator
      // lists every staged file, and `git restore --staged --` likewise means
      // "everything" to read but dies with "you must specify path(s)". So an
      // empty `globs` reaching here would compute `matched` = the entire
      // change and then be saved only by git refusing the restore and this
      // method's catch swallowing it. That is correct today by accident, and
      // the failure mode if it ever stopped being accidental is reverting the
      // whole commit. The early return above already prevents it; this makes
      ...
      if (spared.length > 0) {
        ...
        this.opts.logger.info(
          `[git] rc.7: kept ${spared.length} never-commit path(s) this sub-task is contracted to generate`,
          {
            worktreePath,
            count: spared.length,
        ...
        this.opts.logger.warn(
          `[git] beta.114: reverted ${toRevert.length} change(s) to never-commit path(s); they are not this change's work`,
          {
            worktreePath,
            globs,
      ...
      // A path that is untracked has nothing to restore FROM, and `git restore`
      // fails the whole invocation on one such pathspec. Tolerate that: the
      // unstage above already keeps it out of the commit.
      await this.run(["-C", worktreePath, "restore", "--worktree", "--", ...globs]).catch(() => undefined);
      this.opts.logger.warn(
        `[git] beta.114: reverted ${matched.length} change(s) to never-commit path(s); they are not this change's work`,
        {
          worktreePath,
          globs,
      ...
      // Housekeeping must never break a commit; a swept-in bundle is a bad PR,
      // a failed commit is a lost sub-task.
      this.opts.logger.warn("[git] beta.114: never-commit revert failed (non-fatal)", {
        worktreePath, err: String(err),
      });
      return [];
    }
  }
   ...
   */
  async diff(worktreePath: string, base: string, ghToken?: string): Promise<string> {
    const ask = ghToken ? await this.makeAskpass(ghToken) : undefined;
    try {
   ...
   */
  async fileDiffLineCount(worktreePath: string, base: string, exactPath: string): Promise<number> {
    if (!base || !exactPath) return 0;
    const out = await this.run([
...
`;
  ...
  private run(args: string[], _cwd?: string, askpassPath?: string, token?: string): Promise<string> {
    return new Promise((resolveP, rejectP) => {
      ...
      proc.on("close", (code) => {
        if (code === 0) return resolveP(out);
        const e = new Error(
          `git ${args.map((a) => redactSecrets(a, token)).join(" ")} failed (${code}): ${redactSecrets(err.trim(), token)}`,
