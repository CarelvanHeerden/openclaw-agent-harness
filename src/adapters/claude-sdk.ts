      ...
      if (message.type === "assistant") {
        ...
        if (Array.isArray(content)) {
          ...
          if (text.trim()) {
            ...
            if (opts.accumulateAllText) allText.push(text);
          }
        }
      }
...
export async function runLeadScoutSdk(params: {
  ...
  logger?: { warn: (m: string, meta?: unknown) => void };
}): Promise<{ report: string; sdkSessionId: string; costUsd: number; tokensIn: number; tokensOut: number; stopReason: string }> {
  const sdk = await loadSdk();
  const abort = new AbortController();
  const timer = setTimeout(() => abort.abort(), params.timeoutSeconds * 1000);
  ...
  if (params.validation) {
    try {
      ...
      if (err instanceof Error) {
        ...
        (err as StructuredCallError).truncated = stopReason === "max_tokens";
      }
      throw err;
    }
  } else {
...
export async function runLeadSdk(params: {
  ...
  try {
    ...
    if (!/extractJson failed|no JSON in output|validation failed|JSON\.parse|\[stream_open_timeout\]/i.test(msg)) throw err;
