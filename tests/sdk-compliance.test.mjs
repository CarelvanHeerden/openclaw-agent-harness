...
test("sdk: structuredCall disables tools via tools: [] (beta.28)", () => {
  const src = readFileSync(resolve(repoRoot, "src/adapters/claude-code.ts"), "utf8");
  assert.match(
    src,
    /\btools:\s*\[\s*\]/,
    "src/adapters/claude-code.ts must set `tools: []` on the structured sdk.query call to disable all built-in tools.",
  );
});
...
test("sdk: compiled structuredCall keeps tools: [] (beta.28)", { skip: !existsSync(resolve(repoRoot, "dist/adapters/claude-code.js")) }, () => {
  const src = readFileSync(resolve(repoRoot, "dist/adapters/claude-code.js"), "utf8");
