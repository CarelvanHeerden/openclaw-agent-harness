...
test("ambiguity is resolved internally into one confirmable proposal",()=>{assert.match(service,/crystallised\.kind===\"reject\"/);assert.match(service,/confirmable:true/);assert.match(service,/VALUES \(\?,\?,\?,/);assert.doesNotMatch(service,/No implementation may start until the unresolved decision is supplied/);assert.doesNotMatch(service,/proposal_not_confirmable/);});
