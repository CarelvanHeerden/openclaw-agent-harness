...
test("strict merge re-inspects repository, base, PR number, head and readiness",()=>{for(const token of ["provider.inspect","inspection.repository!==repository","inspection.baseRef!==baseRef","inspection.prNumber!==prNumber","inspection.headSha!==expectedHeadSha","evaluatePrReadiness","readiness_changed"])assert.match(merge,new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));});
