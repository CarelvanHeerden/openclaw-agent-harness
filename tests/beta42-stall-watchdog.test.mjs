...
try {
  ({ OrchestratorLoop, isSessionLoopRunning, clearStallWatchdog } = await import("../dist/orchestrator/legacy-loop.js"));
...
test("beta42: watchdog force-deregisters a STALLED skipped session and emits loop.wedge_detected", { skip: OrchestratorLoop === null }, async () => {
  ...
  const first = loop.runConfirmedControl("W1", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
  ...
  const out = await loop2.runConfirmedControl("W1", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
...
test("beta42: watchdog does NOT fire when the skipped session keeps making progress", { skip: OrchestratorLoop === null }, async () => {
  ...
  const first = loop.runConfirmedControl("W2", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
  await new Promise((r) => setTimeout(r, 10));

  const loop2 = makeLoop(state, { config: config({ loop: { stall_watchdog_seconds: 0.15 } }) });
  const out = await loop2.runConfirmedControl("W2", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
  assert.equal(out.status, "skipped_already_running");
