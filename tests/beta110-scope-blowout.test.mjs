...
test("beta110: readiness rejects a committed path outside confirmed scope",()=>{const h="d".repeat(40),r="e".repeat(64);const result=evaluatePrReadiness({finalVerdict:"pass",blockingFindings:0,reviewCompleted:true,verificationProbes:{completed:1,required:1,indeterminate:0},candidateSha:h,publication:{sha:h,observedAt:1},pullRequest:{repository:"o/r",baseRef:"main",headSha:h,open:true},expectedRepository:"o/r",expectedBaseRef:"main",requiredCi:{registered:true,requiredChecks:["test"],successfulChecks:["test"],sha:h,status:"success"},runtimeEvidence:{status:"not_required"},securityEvidence:{status:"pass",sha:h,observedAt:1},elapsedTimeMs:1,timeLimitMs:10,changedPaths:["src/x.ts",".npm-cache-tmp/blob"],allowedScope:["src/**"],excludedScope:[".npm-cache-tmp/**"],operationsPerformed:["commit"],operationReceipts:[{operation:"commit",observedAt:1,source:"test-fixture"}],allowedOperations:["commit"],credentialRouteDigest:r,expectedCredentialRouteDigest:r,secretExposure:{detected:false,evidence:"pass"},spendUsd:1,budgetUsd:2},2);assert.equal(result.ready,false);assert.ok(result.failures.includes("scope_exceeded"));});
