...
function config(overrides = {}) {
  return {
    ...
    // rc.6: `budget_extension_ask_enabled: false` because the money tests in
    // this file pin what the STOPS do, and with the ask on each one would sit
    // out a five-minute window before doing it. That the flag genuinely
    // restores the pre-rc.6 stop is part of what they now demonstrate; the ask
    // itself is covered in tests/rc6-budget-headroom.test.mjs.
    loop: { max_cycles: 3, adversarial_pass_ends_early: true, worker_timeout_seconds: 60, adversary_timeout_seconds: 60, session_hard_timeout_seconds: 3600, budget_extension_ask_enabled: false },
...
test("loop: happy path shipping on first cycle when adversary passes",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S1", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
...
test("loop: adversary revise once then pass",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S2", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
    assert.equal(outcome.status, "shipped");
...
test("loop: retired reaction input is ignored",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S3", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
    assert.equal(outcome.status, "shipped");
  });
...
test("loop: budget exhaustion aborts unless budget_bump",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S4", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
...
test("loop: adversary block ends immediately as failed",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S5", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
    assert.equal(outcome.status, "failed");
...
test("loop: threads the session requester into runLead/runWorker/pushBranchAndOpenPr (multi-user)",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S_MU", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
    assert.equal(outcome.status, "shipped");
...
test("loop: worker confabulates 'completed' push but branch is NOT on remote -> failed_verification (beta.8 fix #1)",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("SV1", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
...
test("loop: real push (branch on remote) passes harness verification and ships (beta.8 fix #1)",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("SV2", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
    assert.equal(outcome.status, "shipped");
    const verif = state.audits.find((a) => a.event === "loop.subtask_verification");
...
test("loop: projected-cost gating aborts BEFORE starting an unaffordable sub-task (beta.7 fix #2)",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("SB1", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
    // beta.120 (fix 1): the spend gate still fires at exactly the same point,
    // and hitting a ceiling still does not destroy the branch.
    //
...
test("loop: review is skipped + cycle aborts when remaining budget < review estimate (beta.7 fix #2)",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("SB2", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
...
test("loop: write-only sub-task passes file_written with fileExistsOnDisk probe (beta.9 regression fix)",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S_REG1", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
...
test("loop: 5-sub-task plan (write, commit, push, open PR, verify) all pass with beta.9 probes",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S_5ST", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
...
test("loop: malicious worker writes empty file — harness catches via fileExistsOnDisk (beta.9)",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S_MAL1", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
...
test("loop: malicious worker claims file written but file absent — harness catches (beta.9)",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S_MAL2", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
...
test("loop: remote_branch_verify_failed fires alongside push_verify_failed for branch_pushed failures",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S_EVT1", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
    assert.equal(outcome.status, "failed");
...
test("loop: file_written_verify_failed fires alongside file_verify_failed (backward compat + new name)",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S_EVT2", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
    assert.equal(outcome.status, "failed");
...
test("loop: push sub-task fires push_verify_failed AND remote_branch_verify_failed exactly ONCE each (no duplicates)",
  { skip: OrchestratorLoop === null }, async () => {
    ...
    const outcome = await loop.runConfirmedControl("S_NODUP1", brief, createInternalConfirmedControlAuthorityGuard(() => {}));
    assert.equal(outcome.status, "failed");
