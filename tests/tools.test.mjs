...
test("registration: registers 20 tools",
  { skip: registerHarnessTools === null }, () => {
    ...
    assert.deepEqual(
      [...tools.keys()].sort(),
      ["harness_answer", "harness_bootstrap_test_repo", "harness_cancel", "harness_health", "harness_help", "harness_link_pr", "harness_list_revisable", "harness_logs", "harness_merge_pr", "harness_onboard", "harness_progress", "harness_resume", "harness_retention_prune", "harness_revise", "harness_run", "harness_session_get", "harness_start_session", "harness_status", "harness_telemetry", "harness_upload_logs"],
    );
  });
...
test("harness_health: reports OK when config + schema are minimally valid",
  { skip: registerHarnessTools === null }, async () => {
    ...
    assert.equal(res.details.versionInfo.pluginVersion, "2.0.0-rc.11");
