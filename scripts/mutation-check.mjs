...
const FILTER = process.argv.slice(2).find((arg) => arg !== "--check-anchors") ?? "";
let ran = 0;
...
const MUTATIONS = [
  ...
  {
    name: "rc.13 human provenance: host provenance marker is mandatory",
    file: "dist/control/service.js",
    find: 'att.provenance !== "host_verified"',
    replace: 'false',
    tests: ["tests/control-service.test.mjs"],
  },
  {
    name: "rc.13 human provenance: operation kind is bound",
    file: "dist/control/service.js",
    find: "att.operation !== operation",
    replace: "false",
    tests: ["tests/control-service.test.mjs"],
  },
  {
    name: "rc.13 human provenance: requester identity is bound",
    file: "dist/control/service.js",
    find: "if (actor !== run.requesterId || att.actorIdentity !== run.requesterId)\n            throw new ControlError(\"wrong_actor\", \"The confirmation must come from the preparing requester.\");",
    replace: "if (false)\n            throw new ControlError(\"wrong_actor\", \"The confirmation must come from the preparing requester.\");",
    tests: ["tests/control-service.test.mjs"],
  },
  {
    name: "rc.13 human provenance: conversation identity is bound",
    file: "dist/control/service.js",
    find: "if (conversation !== run.conversationId || att.conversationIdentity !== run.conversationId)\n            throw new ControlError(\"wrong_conversation\", \"The confirmation must come from the preparing conversation.\");",
    replace: "if (false)\n            throw new ControlError(\"wrong_conversation\", \"The confirmation must come from the preparing conversation.\");",
    tests: ["tests/control-service.test.mjs"],
  },

  {
    name: "rc.13 human provenance: persisted merge authorization is digest-bound",
    file: "dist/control/merge.js",
    find: "mergeAuthorizationDigest(unsigned) !== String(row.binding_digest)",
    replace: "false",
    tests: ["tests/control-production-merge-recovery.test.mjs"],
  },
  {
    name: "rc.13 human provenance: persisted readiness payload is digest-bound",
    file: "dist/control/merge.js",
    find: "result.contentDigest !== String(row.content_digest)",
    replace: "false",
    tests: ["tests/control-production-merge-recovery.test.mjs"],
  },
...
function runTests(files) {
  const r = spawnSync(process.execPath, ["--test", ...new Set([...files, "tests/build-integrity.test.mjs"])], {
    cwd: root,
    encoding: "utf8",
...
const ACTIVE_MUTATIONS = MUTATIONS.filter((mutation) => {
  const path = join(root, mutation.file);
  return existsSync(path) && Boolean(locate(readFileSync(path, "utf8"), mutation.find));
});
if (ACTIVE_MUTATIONS.length < 300) {
  console.error(`Mutation inventory collapsed to ${ACTIVE_MUTATIONS.length}; expected at least 300 current release-critical probes.`);
  process.exit(1);
}
const SELECTED_MUTATIONS = FILTER
  ? ACTIVE_MUTATIONS.filter((mutation) => mutation.name.includes(FILTER))
  : ACTIVE_MUTATIONS;
if (FILTER && SELECTED_MUTATIONS.length === 0) {
  console.error(`No mutations matched required filter: ${FILTER}`);
  process.exit(1);
}

const PRISTINE = new Map();
for (const m of SELECTED_MUTATIONS) {
...
if (CHECK_ANCHORS) {
  const stale = [];
  for (const mutation of SELECTED_MUTATIONS) {
    const original = PRISTINE.get(join(root, mutation.file));
    if (!original || !locate(original, mutation.find)) stale.push({ name: mutation.name, file: mutation.file });
  }
  if (stale.length) {
    console.error(JSON.stringify(stale, null, 2));
    process.exit(1);
  }
  console.log(`All ${SELECTED_MUTATIONS.length} current mutation anchors are valid (${MUTATIONS.length - ACTIVE_MUTATIONS.length} retired-surface probes excluded).`);
  process.exit(0);
}
