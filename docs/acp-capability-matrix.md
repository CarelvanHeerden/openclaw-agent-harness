...
| `tokensIn` / `tokensOut` | **yes — on the `session/prompt` RESULT** (see correction below) | same | no usage at all |
...
- **Correction (v2.0.0-beta.1).** The row above originally read "unavailable —
  protocol has no in/out split", and that was wrong. It was concluded from the
  `usage_update` notification, which genuinely carries only `used`, `size` and
  `cost`. The token split is on the **`session/prompt` result** instead, and it
  was in the captured probe sessions the whole time:

  ```json
  {"jsonrpc":"2.0","id":4,"result":{"stopReason":"end_turn",
    "usage":{"inputTokens":10,"outputTokens":132,
             "totalTokens":2137,"cachedWriteTokens":1995}}}
  ```

  (`probe/runs/opencode-2026-08-03T11-05-30-800Z.jsonl`.) The adapter now reads
  it, so `checkPriceDrift()` CAN run on ACP-backed spend. Cache tokens are kept
  separate rather than folded into `inputTokens`, because every provider that
  has them prices them differently.

  The lesson is worth keeping alongside the correction: the measurement was
  taken from one frame type and generalised to "the protocol". Two of the three
  conclusions in this document that followed from it were wrong.
...
4. **The budget ledger survives** on OpenCode: real cost, delta-ed per turn,
   plus a real token split off the `session/prompt` result. A local provider
   reports tokens but no cost, which `usageSource: "tokens-only"` distinguishes
   from an agent that reported nothing — the ledger must never read either as
   zero spend.
