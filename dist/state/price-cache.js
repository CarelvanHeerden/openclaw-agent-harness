...
function looksLikeCatalogue(x) {
    if (x === null || typeof x !== "object")
        return false;
    const c = x;
    if (typeof c.fetchedAt !== "number" || !Number.isFinite(c.fetchedAt))
        return false;
    if (c.entries === null || typeof c.entries !== "object")
        return false;
    return Object.keys(c.entries).length > 0;
}
export function catalogueStore(db) {
    return {
        read: () => {
            try {
                const row = db
        ...
        write: (cat) => {
            try {
                db.prepare(`INSERT INTO model_prices (id, fetched_at, payload) VALUES (1, ?, ?)
             ON CONFLICT(id) DO UPDATE SET fetched_at = excluded.fetched_at,
                                           payload    = excluded.payload`).run(cat.fetchedAt, JSON.stringify(cat));
            }
            catch {
