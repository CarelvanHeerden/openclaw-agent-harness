 ...
 */
export function looksLikeMultiplePaths(raw) {
    if (!raw.includes(", "))
        return false;
...
export function resolvePathForPolicy(raw, opts = {}) {
    ...
    if (looksLikeMultiplePaths(trimmed)) {
        ...
        return {
            raw,
            candidates: [],
            refuse: `path string appears to name ${n} files rather than one ` +
    ...
    const add = (p) => {
        ...
        if (root && norm.startsWith(root + "/"))
            canonical.add(norm.slice(root.length + 1));
    };
    add(trimmed);
    ...
    if (candidates.length === 0)
        return { raw, candidates: [], refuse: "path normalised to nothing" };
 ...
 */
export function pathsFromPatchText(patchText) {
...
export function parsePatchTargets(patchText) {
    ...
    for (let i = first + 1; i < last; i++) {
        ...
        const path = m[2].trim();
        if (!path)
            return { complete: false, paths: [], reason: `empty patch target at line ${i + 1}` };
    ...
    if (out.length === 0)
        return { complete: false, paths: [], reason: "patch contains no file directive" };
...
function looksHighEntropy(value) {
    if (value.endsWith("="))
        return true; // base64 padding
    if (/^[0-9a-f]{32,}$/i.test(value))
        return true; // hex digest / hex token
 ...
 */
export function scanPatchForSecrets(patchText) {
 ...
 */
export function templateExceptionApplies(resolution, exceptions) {
