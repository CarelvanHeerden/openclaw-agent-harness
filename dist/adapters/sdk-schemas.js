 ...
 */
export function parseAndValidate(jsonText, schema, rawOutput, callerLabel) {
    let obj;
    try {
        obj = JSON.parse(jsonText);
    }
    catch (err) {
        throw new Error(`[${callerLabel}] model returned invalid JSON: ${String(err)}. Raw output (first 500 chars): ${rawOutput.slice(0, 500)}`);
    }
    ...
    if (!result.success) {
        const issues = result.error.issues
            ...
            .join("; ");
        throw new Error(`[${callerLabel}] model output failed schema validation (${issues}). Raw output (first 500 chars): ${rawOutput.slice(0, 500)}`);
    }
    return result.data;
}
