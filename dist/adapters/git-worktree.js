 ...
 */
const COMMIT_MSG_NOISE_RE = /(^|\/)\.?(git-?)?commit-?(msg|message)[^/]*$|(^|\/)COMMIT_EDITMSG$|(^|\/)\.gitmessage$/i;
 ...
 */
const inFlightWorktrees = new Set();
/**
...
export function inFlightBranchHolders() {
    return [...inFlightBranches].map(([key, sessionId]) => ({ key, sessionId }));
}
export function inFlightWorktreePaths() {
...
`;
     ...
     */
    async authenticatedRunner(ghToken) {
        const ask = ghToken ? await this.makeAskpass(ghToken) : undefined;
        const run = (args, cwd) => new Promise((resolveRun) => {
            ...
            proc.on("error", (e) => resolveRun({ code: 127, stdout: "", stderr: redactSecrets(String(e), ghToken) }));
    ...
    async allocate(ctx) {
        ...
        if (holder && holder !== ctx.sessionId) {
            throw new Error(`branch ${ctx.sessionBranch} on ${ctx.repoFullName} is already being built by another live session ` +
        ...
        try {
            ...
            if (inFlightBranches.get(branchKey) === ctx.sessionId)
                inFlightBranches.delete(branchKey);
        }
    }
    async allocateInner(ctx, bare, wt) {
    ...
    async localBranchExists(bare, branch) {
        try {
     ...
     */
    async rescueBranchIfAhead(bare, branch, startPoint) {
        try {
            if (!(await this.localBranchExists(bare, branch)))
                return;
            ...
            this.opts.logger.warn("[git] branch reset would have orphaned commits; parked them under a rescue ref before resetting", { branch, startPoint, orphanCount: doomed.length, tip, rescueRef, orphans: doomed.slice(0, 20) });
        }
        catch (err) {
     ...
     */
    async unreachableCommits(worktreePath, from, shas) {
        const out = [];
     ...
     */
    async listFilesInCommits(worktreePath, shas) {
        const out = new Map();
        for (const sha of shas) {
            ...
            for (const line of text.split("\n")) {
                const f = line.trim();
                if (f && !isCommitMsgNoise(f) && !out.has(f))
                    out.set(f, sha);
            }
        }
        return out;
     ...
     */
    async pathRenamedAwaySince(worktreePath, base, path) {
        const miss = { renamed: false, to: "", score: "", detail: "" };
        if (!base)
            return { ...miss, detail: "no base sha" };
        const out = await this.run([
    ...
    async remoteBranchSha(worktreePath, remote, branch, ghToken) {
        ...
        const ask = ghToken ? await this.makeAskpass(ghToken) : undefined;
        try {
            const out = await this.run(["-C", worktreePath, "ls-remote", remote, ref], undefined, ask?.path, ghToken).catch(() => "");
     ...
     */
    async appendExcludes(worktreePath, patterns) {
    ...
    async applyHarnessExcludes(worktreePath) {
        try {
     ...
     */
    async excludeRunawayUntracked(worktreePath) {
        try {
            ...
            this.opts.logger.warn(`[git] beta.110: excluded ${runaway.length} runaway untracked director(ies) from this commit ` +
                `-- ${runaway.map((r) => `${r.dir}/ (${r.count} files)`).join(", ")}. This is almost always a tool ` +
                `cache or build output written into the worktree. The sub-task's real changes still commit.`, { worktreePath, runaway, threshold, event: "harness.runaway_untracked_excluded" });
            return runaway;
        }
        catch {
            return []; // never let housekeeping break a commit
    ...
    async revertNeverCommitPaths(worktreePath, authorizedPaths = []) {
        ...
        try {
            ...
            this.opts.logger.warn(`[git] beta.114: reverted ${matched.length} change(s) to never-commit path(s); they are not this change's work`, {
                worktreePath,
                globs,
     ...
     */
    async mergeBase(worktreePath, ref) {
    ...
    async commitCount(worktreePath, base) {
        try {
     ...
     */
    async listTrackedFiles(worktreePath) {
