...
const CANONICAL_SCRIPTS = ["typecheck", "lint", "test", "build"];
/**
 ...
 */
export function detectCheckScripts(worktreePath) {
    try {
        ...
        return CANONICAL_SCRIPTS.filter((s) => typeof scripts[s] === "string" && scripts[s].trim().length > 0);
    }
    catch {
        return [];
    }
...
export function hasExistingWorkflow(worktreePath) {
    try {
 ...
 */
export function renderCiWorkflowYaml(scripts) {
    const steps = scripts
