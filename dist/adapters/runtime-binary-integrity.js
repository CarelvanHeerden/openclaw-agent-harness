...
function expectedDigest(entry, relativeBinary) {
    const suffix = `  ${relativeBinary}`;
    const line = entry.entries.find((candidate) => candidate.endsWith(suffix));
    if (!line || !/^[a-f0-9]{64}  /.test(line))
        throw new Error(`runtime integrity manifest is missing ${relativeBinary}`);
    return line.slice(0, 64);
}
function verifiedBytes(path, expected) {
    const noFollow = typeof constants.O_NOFOLLOW === "number" ? constants.O_NOFOLLOW : 0;
    const fd = openSync(path, constants.O_RDONLY | noFollow);
    try {
        const metadata = fstatSync(fd);
        if (!metadata.isFile())
            throw new Error(`runtime executable is not a regular file: ${path}`);
        const bytes = readFileSync(fd);
        const actual = createHash("sha256").update(bytes).digest("hex");
        if (actual !== expected)
            throw new Error(`runtime executable integrity check failed: ${path}`);
        return bytes;
    }
    finally {
        closeSync(fd);
...
function snapshot(bytes, parent, prefix, filename) {
...
function loadManifest(pluginRoot, filename) {
    const path = resolve(pluginRoot, "scripts", filename);
    const metadata = lstatSync(path);
    if (!metadata.isFile() || metadata.isSymbolicLink())
        throw new Error(`runtime integrity manifest is not a regular file: ${path}`);
    const manifest = JSON.parse(readFileSync(path, "utf8"));
    if (manifest.version !== 1 || !manifest.packages)
        throw new Error(`invalid runtime integrity manifest: ${path}`);
    return manifest;
}
export function verifyClaudeRuntime(pluginRoot, snapshotParent) {
    ...
    if (!entry || entry.version !== packageJson.version)
        throw new Error(`Claude runtime package is not covered by the integrity manifest: ${packageName}`);
...
export function verifyOpenCodeRuntime(pluginRoot, snapshotParent) {
    const requireFromPlugin = createRequire(resolve(pluginRoot, "package.json"));
    ...
    for (const packageName of candidates) {
        ...
        try {
            ...
            if (entry.version !== packageJson.version)
                throw new Error(`OpenCode runtime version is not authenticated: ${packageName}`);
...
//# sourceMappingURL=runtime-binary-integrity.js.map