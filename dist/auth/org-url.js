 ...
 * resolves.
 */
export function acceptedHosts(providers) {
    const hosts = new Map();
    for (const [name, cfg] of Object.entries(providers ?? {})) {
        ...
        if (hostname.startsWith("api."))
            hosts.set(hostname.slice("api.".length), provider);
    }
    return hosts;
}
/**
 ...
 */
export function parseOrgUrl(input, providers) {
    const raw = (input ?? "").trim();
    if (!raw)
        return { ok: false, error: "no org URL was given" };
    const hosts = acceptedHosts(providers);
    if (hosts.size === 0)
        return { ok: false, error: "no git providers are configured" };
    let host;
    let path;
    ...
    const segments = path
        ...
        .map((s) => s.trim())
        .filter((s) => s.length > 0);
    if (segments.length > 0 && ORG_PREFIXES.has(segments[0].toLowerCase()))
        segments.shift();
    const first = segments[0];
    if (!first)
        return { ok: false, error: `"${raw}" names a host but no org` };
    let org;
    try {
...
//# sourceMappingURL=org-url.js.map