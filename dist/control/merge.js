...
export class InternalMergeService {
    ...
    async recoverPending() {
        ...
        this.recoveryInFlight = (async () => {
            const rows = this.db.prepare(`SELECT authorization_id FROM control_engine_merge_intents WHERE status IN ('authorized','merging') ORDER BY created_at LIMIT 25`).all();
            await Promise.allSettled(rows.map((row) => this.merge(row.authorization_id)));
        })().finally(() => { this.recoveryInFlight = null; });
        return this.recoveryInFlight;
    }
    async merge(id) {
        const intent = this.db.prepare(`SELECT * FROM control_engine_merge_intents WHERE authorization_id=?`).get(id);
        if (!intent)
        ...
        if (intent.status === "verification_failed")
            return Object.freeze({ status: "merge_failed", code: "verification_failed" });
        const lease = this.acquireIntentLease(intent.id);
        if (!lease)
            return this.waitForIntent(intent.id, id);
        try {
            return await this.mergeLeased(id, intent, lease);
        }
        finally {
            this.releaseIntentLease(intent.id, lease);
        }
    }
    async waitForIntent(intentId, authorizationId) {
    ...
    async mergeLeased(id, intent, lease) {
        const now = this.now();
        const raw = this.db.prepare(`SELECT * FROM control_merge_authorizations WHERE id=?`).get(id);
        ...
        if (!proposal || proposal.published_sha !== auth.expectedHeadSha || proposal.readiness_digest !== auth.readinessDigest || intent.change_id !== auth.runId || intent.expected_head_sha !== auth.expectedHeadSha) {
        ...
        if (!inspection.open || inspection.repository !== auth.repository || inspection.baseRef !== auth.baseRef || inspection.prNumber !== auth.prNumber) {
            this.refuseRun(run.id, intent.id, "pr_identity_mismatch", lease);
            return Object.freeze({ status: "refused", code: "pr_identity_mismatch" });
        }
        if (inspection.headSha !== auth.expectedHeadSha) {
            this.refuseRun(run.id, intent.id, "stale_pr_head", lease);
            return Object.freeze({ status: "refused", code: "stale_pr_head" });
        }
        const live = evaluatePrReadiness(inspection.readiness, now);
        if (!live.ready || live.verifiedSha !== auth.expectedHeadSha || !this.verifyPersistedReadiness(run.id, auth.readinessDigest)) {
            this.refuseRun(run.id, intent.id, "readiness_changed", lease);
            return Object.freeze({ status: "refused", code: "readiness_changed" });
        }
        this.db.exec("BEGIN IMMEDIATE");
        ...
        try {
            ...
            if (!this.completeRun(run.id, intent.id, merged.mergeSha, lease))
                return Object.freeze({ status: "merge_in_progress" });
    ...
    verifyPersistedAuthorizationRow(id) { const row = this.db.prepare(`SELECT * FROM control_merge_authorizations WHERE id=?`).get(id); return Boolean(row && this.verifyPersistedAuthorization(row)); }
    verifyPersistedAuthorization(row) {
        try {
            const unsigned = { version: 2, id: String(row.id), runId: String(row.run_id), actorIdentity: String(row.actor_identity), conversationIdentity: String(row.conversation_identity), repository: String(row.repository_identity), baseRef: String(row.base_ref), prNumber: Number(row.pr_number), expectedHeadSha: String(row.expected_head_sha), publishedSha: String(row.expected_head_sha), readinessDigest: String(row.readiness_digest), nonce: String(row.nonce), issuedAt: Number(row.issued_at), expiresAt: Number(row.expires_at) };
            if (mergeAuthorizationDigest(unsigned) !== String(row.binding_digest))
                return null;
    ...
    verifyPersistedReadiness(runId, contentDigest) {
        const row = this.db.prepare(`SELECT * FROM control_readiness_attestations WHERE run_id=? AND content_digest=?`).get(runId, contentDigest);
        if (!row)
            return null;
        try {
            const input = JSON.parse(String(row.input_json));
            const result = evaluatePrReadiness(input, Number(row.created_at));
            const failures = JSON.parse(String(row.failures_json));
            const expectedFailures = result.ready ? [] : result.failures;
            if (result.contentDigest !== String(row.content_digest) || result.policyVersion !== String(row.policy_version) || (result.ready ? 1 : 0) !== Number(row.ready) || (result.ready ? result.verifiedSha : null) !== (row.verified_sha ?? null) || stable(failures) !== stable(expectedFailures))
                return null;
            return { input, result };
        }
        catch {
    ...
    refuseRun(runId, intentId, code, lease) { return this.terminalize(runId, intentId, "verification_failed", code, lease); }
    failRun(runId, intentId, code, lease) { return this.terminalize(runId, intentId, code === "provider_failure" ? "merge_failed" : "verification_failed", "merge_failed", lease); }
    terminalize(runId, intentId, intentStatus, runCode, lease) {
        const at = this.now();
        this.db.exec("BEGIN IMMEDIATE");
        try {
            ...
            const params = [intentStatus, at, intentId];
            let sql = `UPDATE control_engine_merge_intents SET status=?,updated_at=? WHERE id=? AND status IN ('authorized','merging')`;
            if (lease) {
                sql += ` AND recovery_owner=? AND recovery_fence=? AND recovery_lease_expires_at>?`;
                params.push(lease.owner, lease.fence, at);
            }
            ...
            if (Number(runChanged.changes) !== 1)
                throw new Error("terminal_generation_lost");
    ...
    completeRun(runId, intentId, mergeSha, lease) {
        const at = this.now();
        this.db.exec("BEGIN IMMEDIATE");
        try {
            const current = this.db.prepare(`SELECT state,version FROM control_runs WHERE id=?`).get(runId);
