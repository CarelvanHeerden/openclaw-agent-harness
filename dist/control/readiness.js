...
function sameSet(required, successful) {
...
export function evaluatePrReadiness(input, checkedAt = Date.now()) {
    ...
    if (!Number.isSafeInteger(probes.completed) ||
    ...
    if (!input.publication || input.publication.sha !== input.candidateSha)
        failures.push("stale_publication");
    if (!input.pullRequest.open ||
    ...
    if (!Number.isFinite(input.spendUsd) || !Number.isFinite(input.budgetUsd) || input.spendUsd > input.budgetUsd)
        failures.push("spend_exceeded");
    if (failures.length > 0)
        return Object.freeze({ ready: false, state: "failed", failures: Object.freeze([...new Set(failures)]) });
