...
function canonicalComponent(value) {
    if (!ASCII_REPOSITORY_COMPONENT.test(value) || value === "." || value === "..")
        return null;
...
function canonicalAllowEntry(value) {
    if (typeof value !== "string" || value !== value.trim())
        return null;
    const parts = value.split("/");
    if (parts.length !== 2)
...
export function isRepositoryAllowed(repoFullName, allowed) {
    const repository = canonicalRepositoryIdentity(repoFullName);
    if (!repository || !Array.isArray(allowed))
        return false;
    return allowed.some((configured) => {
        const allowedRepo = canonicalAllowEntry(configured);
        if (!allowedRepo || allowedRepo.owner !== repository.owner)
            return false;
        if (allowedRepo.repository === "*")
            return true;
        return allowedRepo.repository === repository.repository;
    });
}
//# sourceMappingURL=repository-allowlist.js.map