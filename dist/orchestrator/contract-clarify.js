...
export function missingFromCommit(m) {
 ...
 */
export function autoResolveContract(m) {
    ...
    return {
        resolved: true,
        reason: `every expected path this sub-task did not touch was already changed earlier on this branch ` +
            `(${covered.join(", ")}), so the work exists and the contract is satisfied by the branch as a whole`,
        coveredEarlier: covered,
    };
}
const bullet = (opt, text) => `  • ${opt.padEnd(15)} ${text}`;
/**
 ...
 */
export function buildContractClarification(m) {
    const missing = missingFromCommit(m);
    const auto = autoResolveContract(m);
    const lines = [];
    lines.push(`Sub-task ${m.seq} ("${m.title}") committed ${m.commitSha.slice(0, 7)}, but it did not change ` +
    ...
    if (auto.coveredEarlier.length > 0) {
        lines.push("");
        lines.push(`Suggestion: "accept" looks right. ${auto.coveredEarlier.join(" and ")} ` +
    ...
    lines.push("");
    lines.push(`Detail -- contract expected: ${m.expected.join(", ")}; this commit touched: ` +
        `${m.actual.join(", ") || "(no files reported)"}.`);
    return lines.join("\n");
}
//# sourceMappingURL=contract-clarify.js.map