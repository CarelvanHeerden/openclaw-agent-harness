 ...
 */
export function extractStatedReason(finalMessage, expectedPaths = [], actualPaths = [], maxChars = 400) {
    ...
    for (const s of candidates) {
        ...
        if (needles.some((n) => n && lower.includes(n)))
            score += 2;
        if (DEVIATION_CUES.some((c) => lower.includes(c)))
            score += 1;
...
//# sourceMappingURL=worker-reason.js.map