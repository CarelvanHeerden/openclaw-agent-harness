...
export function resolveBudgetPolicy(input) {
    const approved = typeof input.authorizedMaximumUsd === "number" && Number.isFinite(input.authorizedMaximumUsd) && input.authorizedMaximumUsd > 0
 ...
 */
export function projectCycleCostUsd(spentUsd, cyclesRan) {
 ...
 */
export function assessRepairFunding(input) {
    const { policy, repairSpentUsd, repairCyclesGranted, projectedRepairCostUsd } = input;
    if (!(policy.repairReserveUsd > 0))
        return { funded: false, basis: "no_reserve", shortfallUsd: 0 };
    if (repairCyclesGranted < 1)
        return { funded: true, basis: "first_repair" };
    const wouldSpend = Math.max(0, repairSpentUsd) + Math.max(0, projectedRepairCostUsd);
    if (wouldSpend <= policy.repairReserveUsd)
        return { funded: true, basis: "reserve_covers_projection" };
...
export function describeBudgetPolicy(policy) {
    if (!(policy.authorizedMaximumUsd > 0))
        return "no session budget is set, so nothing is reserved for repair";
    return (`$${policy.authorizedMaximumUsd.toFixed(2)} approved: ` +
