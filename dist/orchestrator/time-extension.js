...
export function isTimeExtensionPause(clarificationSubtask) {
 ...
 */
export function renderTimeExtensionMarker(waitUntilMs) {
    return JSON.stringify({ kind: TIME_EXTENSION_KIND, waitUntilMs });
}
export function readTimeExtensionWaitUntil(clarificationSubtask) {
    if (!clarificationSubtask)
        return 0;
    try {
        const parsed = JSON.parse(clarificationSubtask);
        if (parsed?.kind !== TIME_EXTENSION_KIND)
            return 0;
...
export function parseTimeExtensionReply(answer, opts) {
    const raw = (answer ?? "").trim();
    const max = Math.max(0, opts.maxSeconds ?? MAX_EXTENSION_SECONDS);
    const clamp = (s) => Math.max(0, Math.min(Math.round(s), max));
    if (!raw)
        return { approved: false, seconds: 0, interpretation: "unrecognised" };
    if (HARD_STOP.test(raw))
        return { approved: false, seconds: 0, interpretation: "declined" };
    const d = DURATION.exec(raw);
    if (!d && SOFT_NEGATIVE.test(raw))
        return { approved: false, seconds: 0, interpretation: "declined" };
    if (d) {
        ...
        if (seconds > 0)
            return { approved: true, seconds, interpretation: "explicit_duration" };
    }
    if (AFFIRMATIVE.test(raw)) {
        const seconds = clamp(opts.defaultSeconds);
        if (seconds > 0)
            return { approved: true, seconds, interpretation: "approved_default" };
    }
...
//# sourceMappingURL=time-extension.js.map