...
export function pathMatchRule(committed, contract) {
    ...
    if (tSegs.length >= 2 && !looksLikeFileName(tSegs[tSegs.length - 1])) {
        if (cg.startsWith(`${tg}/`) || cg.includes(`/${tg}/`))
            return "directory-implied";
    }
    return null;
}
...
export function isTestFilePath(p) {
    ...
    if (/\.(test|spec)\.[a-z0-9]+$/.test(bn))
        return true; // foo.test.ts / foo.spec.tsx
    if (/(^|[._-])test\.[a-z0-9]+$/.test(bn))
        return true; // foo_test.go / foo-test.js
    if (/^test_.+\.[a-z0-9]+$/.test(bn))
        return true; // test_foo.py (pytest)
...
export function resolveContractPath(realFiles, contract, opts = {}) {
    ...
    if (opts.allowBasenameFallback) {
        ...
        if (tHasDir) {
            ...
            if (hits.length === 1)
                return { file: hits[0], rule: "basename-unique" };
        }
    }
    ...
    if (opts.allowTestFileFallback && isTestFilePath(contract)) {
        ...
        if (testHits.length === 1)
            return { file: testHits[0], rule: "test-file-unique" };
    }
    return null;
}
