...
export function buildWorkerSystemPrompt(brief, subTask) {
    ...
    `- DO NOT run repo-wide generators, full-repo builds, or whole-project`, `  typechecks inside your turn. Specifically: do NOT run bundle/artifact`, `  regenerators (e.g. \`npm run okf\`, codegen, "regenerate the bundle"),`, `  do NOT run a repo-wide \`tsc --noEmit\` / full \`npm run build\`, and do NOT`, `  run a whole-repo lint. The harness runs the repo's declared check scripts`, `  (typecheck, lint, okf:check, and any regenerator) AFTER your turn, once,`, `  in its convention-check phase. Running them yourself duplicates minutes of`, `  work and often produces a zero diff. If a repo convention says "regenerate`, `  the bundle", the harness does that for you post-turn -- you do NOT.`, `- For your OWN correctness feedback you MAY read the specific files you`, `  changed and reason about them, or run a check scoped to ONLY the file(s)`, `  you touched if the tooling supports it cheaply. Never expand a scoped`, `  check into a whole-repo run. When in doubt, make the edit, commit, stop.`);
