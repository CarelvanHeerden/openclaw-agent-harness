...
export function buildProgressSnapshot(db, sessionId, limit = 12, stallSeconds = 1800, sdkActivityStallSeconds = 90) {
    ...
    if (row.lead_plan_json) {
        try {
            const parsedPlan = JSON.parse(row.lead_plan_json);
            if (Array.isArray(parsedPlan?.subTasks))
                plannedTotal = parsedPlan.subTasks.length;
        }
        catch {
    ...
    if (latestCycle > 1) {
        try {
            ...
            for (const event of scoped) {
                ...
                if (payload.cycle !== latestCycle)
                    continue;
                if (typeof payload.run === "number" && payload.run >= 0)
                    plannedTotal = payload.run;
                break;
            }
        }
        catch {
    ...
    const current = all.find((s) => ACTIVE_TASK_STATES.has(s.status)) ??
    ...
    const headlineWithWarnings = storageState !== "ok" && storageState !== "unknown" && storageState !== "released"
...
export function renderWorklog(subTasks, total) {
    ...
    for (const st of subTasks) {
        const glyph = st.status === "done" || st.status === "completed"
            ? st.commitSha
                ? "✓"
                : "·"
        ...
        else if ((st.status === "done" || st.status === "completed") && !st.commitSha)
            bits.push("no change");
...
//# sourceMappingURL=legacy-progress.js.map