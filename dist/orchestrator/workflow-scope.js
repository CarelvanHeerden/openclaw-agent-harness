...
export function planTouchesWorkflows(subTasks) {
    const out = [];
    for (const st of subTasks) {
        for (const p of st.filesLikelyTouched ?? []) {
            ...
            if (isWorkflowPath(clean) && !out.includes(clean))
                out.push(clean);
        }
    }
    return out;
 ...
 */
export function canPushWorkflows(scopes) {
...
export function describeMissingWorkflowScope(files) {
    return [
...
//# sourceMappingURL=workflow-scope.js.map