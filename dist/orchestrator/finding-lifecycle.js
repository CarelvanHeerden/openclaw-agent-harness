...
export const CYCLE_DRIVING_STATES = new Set([
    "open",
    "late_discovery",
]);
// ---------------------------------------------------------------------------
 ...
 */
export function normaliseFindingText(s) {
    return (s ?? "")
        .toLowerCase()
        .replace(/[`'"]/g, "")
        .replace(/\b\d+\b/g, " ")
...
export function normaliseFindingPath(p) {
    return (p ?? "")
        .trim()
        .replace(/^\.\//, "")
        .replace(/^\/+/, "")
    ...
    for (const raw of normaliseFindingText(title).split(" ")) {
        ...
        if (raw.includes("/") || raw.includes(".")) {
            for (const seg of raw.split(/[/.]+/)) {
                if (seg.length >= 4 && !TITLE_STOPWORDS.has(seg))
                    out.add(seg);
            }
        }
    }
 ...
 */
export function findingsAreEquivalent(a, b) {
    ...
    if (ta.size === 0 || tb.size === 0)
        return normaliseFindingText(a.title) === normaliseFindingText(b.title);
    let shared = 0;
    for (const t of ta)
        if (tb.has(t))
            shared += 1;
 ...
 */
export function lateDiscoveryReason(f, exposedByPreviousFix) {
    if (exposedByPreviousFix)
        return "exposed by a fix made in a previous cycle";
    if (f.severity === "high" || f.severity === "critical")
        return `severity ${f.severity} is admitted regardless of when it was found`;
    if (SECURITY_DIMENSIONS.has(f.dimension))
        return "security-significant, admitted regardless of when it was found";
...
export function reconcileFindings(input) {
    ...
    const records = [];
    const transitions = [];
    const lateDiscoveries = [];
    for (const f of current) {
        const fp = f.fingerprint ?? findingFingerprint(f);
        const existing = priorByFp.get(fp) ??
        ...
        if (existing && existing.state === "resolved") {
            ...
            const state = regressed ? "open" : "stale";
            seen.add(existing.fingerprint);
            records.push({
                ...existing,
                state,
        ...
        const state = admitted ? "late_discovery" : "stale";
        seen.add(fp);
        records.push({
...
function newRecord(f, fingerprint, cycle, state) {
    return {
        fingerprint,
        state,
        severity: f.severity,
...
export function recordAsFinding(r) {
    return {
