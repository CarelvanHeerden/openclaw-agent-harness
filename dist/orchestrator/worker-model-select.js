 ...
 */
const MECHANICAL_INTENT_RE = /(\bsidebar\b|prisma (models?|schema|back-?relations?)|(add|create|generate).{0,40}\bmigrations?\b|prisma migrate|\bscaffold\b|\bboilerplate\b|barrel (export|file)|re-?export|\bindex (export|barrel)\b|generate(d)? (types|client)|import .{0,20}\bicon\b|(minimal(ly)? )?generali[sz]e\b|extract (a )?props?\b|add .{0,30}\bprops?\b)/i;
/**
 ...
 */
const NON_MECHANICAL_RE = /\b(api|route|endpoint|handler|authz|auth|rbac|permission|upload|download|proxy|blob|multipart|business ?logic|algorithm|refactor|migrate .* data|backfill|test|spec|debug|fix (a )?bug|security|validation logic)\b/i;
/** Is this sub-task mechanical? Honour lead hint first, then heuristic. */
export function isMechanicalSubTask(st) {
    if (st.complexity === "mechanical")
        return true;
    if (st.complexity === "standard" || st.complexity === "complex")
        return false;
    // Heuristic fallback (only when the lead gave no hint).
    const text = `${st.title ?? ""} ${st.intent ?? ""}`;
    if (NON_MECHANICAL_RE.test(text))
        return false;
    if (!MECHANICAL_INTENT_RE.test(text))
        return false;
 ...
 */
export function selectWorkerModel(st, models) {
    const mech = models.worker_mechanical?.trim();
    if (mech && isMechanicalSubTask(st))
        return mech;
    return models.worker;
}
//# sourceMappingURL=worker-model-select.js.map