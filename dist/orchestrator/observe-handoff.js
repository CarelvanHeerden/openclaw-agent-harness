 ...
 */
export function selectObserveReports(subTask, recorded) {
    if (recorded.size === 0)
        return [];
    const wanted = subTask.dependsOn && subTask.dependsOn.length > 0
 ...
 */
export function renderObserveReportsBlock(reports) {
    if (reports.length === 0)
        return "";
    const lines = [
        ``,
