...
const RECOVERIES = [
    ...
    },
    {
        ...
        remedy: "Do not push. Commit your work to the current branch and stop there -- the harness pushes the branch and opens the pull request for you.",
    },
];
...
const BLOCKERS = [
    {
        ...
        match: /\b(?:credential|api[\s-]?key|access[\s-]?token|auth[\s-]?token|secret|password|service[\s-]?account)\b[^.!?]*\b(?:missing|absent|not\s+(?:set|available|provided|configured|present)|required|need(?:ed)?|unavailable)\b|\b(?:missing|no|without)\b[^.!?]*\b(?:credential|api[\s-]?key|access[\s-]?token|secret)\b/i,
    },
    ...
    { kind: "destructive_confirmation", match: /\b(?:destructive|irreversible|data[\s-]loss)\b[^.!?]*\bconfirm/i },
    {
        ...
        match: /\b(?:acceptance\s+criteria|requirements?)\b[^.!?]*\b(?:conflict|contradict|incompatible|mutually\s+exclusive)\b|\b(?:conflicting|contradictory|mutually\s+exclusive)\b[^.!?]*\b(?:acceptance\s+criteria|requirements?)\b/i,
    },
    {
...
export function isProgressFragment(fragment) {
...
export function recoverableDenialFrom(denied) {
    for (const d of denied ?? []) {
        ...
        if (known)
            return { category: known.category, reason, title: title || undefined, remedy: known.remedy };
 ...
 */
function denialQuotedInMessage(text) {
...
export function classifyWorkerOutcome(input) {
    ...
    const refusesRequiredPath = (input.taskContext?.filesLikelyTouched ?? []).some((path) => path.trim().length > 0 &&
    ...
    if (substantive) {
        ...
        if (blocker)
            return { kind: "genuine_blocker", blockerKind: blocker.kind, explanation };
    }
...
export function buildPolicyDenialClarification(params) {
    ...
    if (partial && (partial.committed.length > 0 || partial.commitSha)) {
        lines.push("", `Work already done is KEPT: commit ${partial.commitSha ?? "(recorded)"}` +
...
export function observeEvidenceVerdict(result) {
    ...
    if (allowed === undefined && unguarded === undefined)
        return { empty: false };
    if ((allowed ?? 0) > 0)
        return { empty: false };
    if ((unguarded ?? 0) > 0)
    ...
    if (denials.length > 0) {
        ...
        return {
            empty: true,
            code: "denied_only",
            reason: `every tool call this turn made was denied (${denials.length}) and none was allowed, ` +
...
export function buildObserveEvidenceHint(params) {
    ...
    if (params.verdict.code === "denied_only") {
        parts.push(`Every tool call in your previous turn was DENIED and you read no files, so that turn produced nothing. ` +
        ...
        parts.push("Your previous turn read no files, wrote nothing and committed nothing. Whatever it reported, it " +
            "cannot have found anything, and the harness checks the tool calls rather than the prose.");
    }
    parts.push(`THE DELIVERABLE IS THE REPORT, and it is handed verbatim to the sub-tasks that depend on this one. ` +
        ...
        `sub-task, so reading is the whole job.`, `Do not end your turn describing what you are about to look at. This is attempt ${params.attempt} of ${params.maxAttempts}.`);
    return parts.join("\n\n");
}
/**
 ...
 */
export function describeContractForRetry(contract) {
    const parts = [];
...
export function buildProtocolRetryHint(params) {
    ...
    if (r) {
        parts.push(`A command in your previous turn was DENIED by the harness safety guard, which said: "${r.reason}". ` +
            `This is recoverable and does not change the task. ${r.remedy}`);
    }
    else if (params.outcome.kind === "progress_only") {
        parts.push("Your previous turn ended by describing what you were about to do next, and then stopped. " +
            "Nothing was committed, so none of it happened.");
    }
    else {
        parts.push("Your previous turn produced ZERO filesystem changes and ZERO commits, whatever its final message claimed. Git is authoritative.");
    }
    const wrote = params.uncommittedFiles ?? [];
    if (wrote.length > 0) {
        parts.push(`The harness inspected Git: these files are written but uncommitted: ${wrote.join(", ")}. Finish any remaining edits, then \`git add\` and \`git commit\` them.`);
    }
    parts.push(`OBSERVABLE CONTRACT -- the harness will verify exactly this, by inspecting Git: ${params.contractSummary}`);
    parts.push("Do NOT end your turn with a description of what you intend to do. Continue working until one of three things is true: " +
...
//# sourceMappingURL=worker-outcome.js.map