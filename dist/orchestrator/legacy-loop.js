 ...
 */
import { elideFinalScopeSubTask, validatePlan } from "./lead.js";
import { buildPolicyDenialClarification, classifyWorkerOutcome, policyDenialFrom, correctFalseUserRejection, buildProtocolRetryHint, describeContractForRetry, observeReportIsNarration, observeEvidenceVerdict, buildObserveEvidenceHint, buildClarificationResumeHint, } from "./worker-outcome.js";
 ...
 * existed.
 */
export function workerLeftUncommittedWork(result) {
...
import { findPlanPolicyConflicts, describePlanPolicyConflicts } from "./plan-policy-conflict.js";
...
import { authorizedGeneratedOutputs, generatorScriptDeclared, pendingGenerations, rescuableContractPaths, resolveGenerators, } from "./generated-artifacts.js";
...
export const TERMINAL_STATUSES = ["done", "failed", "aborted"];
/**
...
export class OrchestratorLoop {
    ...
    static advance(input) {
        ...
        const terminalVerdictInHand = input.currentStatus === "reviewing" && input.verdict === "pass";
        ...
        if (status === "awaiting_clarification" || status === "done" || status === "failed" || status === "aborted") {
            try {
                if (status === "awaiting_clarification")
                    ...
                    closeActiveDeadline(this.deps.state.db, sessionId, now);
            }
            catch (err) {
            ...
            try {
                ...
                if (snapshot.segmentStartedAt === null) {
                    resumeActiveDeadline(this.deps.state.db, sessionId, this.deps.config.loop.session_hard_timeout_seconds, now);
                }
            }
            catch {
    ...
    makeWorkerActivityCallback(sessionId, seq, cycle) {
        let lastPersistedAt = 0;
        return (info) => {
            if (info.at - lastPersistedAt < 15_000)
                return;
            lastPersistedAt = info.at;
            try {
                this.deps.state.db.prepare(`UPDATE sessions SET last_progress_at = ?, updated_at = ? WHERE id = ?`).run(info.at, info.at, sessionId);
                this.deps.state.audit("loop.worker_activity", { sessionId, seq, cycle, kind: info.kind }, sessionId);
                this.deps.interactionLog?.log(sessionId, {
                ...
                /* activity telemetry must never disturb the provider turn */
            }
        };
    }
    /**
     ...
     */
    recordSubTaskAttempt(params) {
        try {
            const db = this.deps.state.db;
            const prior = db
                .prepare(`SELECT COUNT(*) AS n FROM sub_task_attempts WHERE session_id = ? AND cycle = ? AND seq = ?`)
                .get(params.sessionId, params.cycle, params.seq);
            const attempt = (prior?.n ?? 0) + 1;
            const shas = params.commitShas ?? (params.commitSha ? [params.commitSha] : []);
            db.prepare(`INSERT INTO sub_task_attempts
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(`${params.sessionId}:${params.cycle}:${params.seq}:${attempt}`, params.sessionId, params.subTaskId ?? null, params.cycle, params.seq, attempt, params.status, params.costUsd ?? 0, params.baseSha ?? null, params.commitSha ?? null, JSON.stringify([...shas]), JSON.stringify([...(params.filesTouched ?? [])]), (params.summary ?? "").slice(0, 2000) || null, params.startedAtMs ?? null, Date.now(), params.workerStatus ?? params.status, params.verificationStatus ?? null, params.verification === undefined ? null : JSON.stringify(params.verification), params.taskOutcome ?? null);
        }
        catch (err) {
    ...
    recordObserveAttemptReport(params) {
        try {
            const parsed = params.validation && "result" in params.validation
                ? params.validation.result
                : undefined;
            this.deps.state.db.prepare(`INSERT OR REPLACE INTO observe_attempt_reports
           (id,session_id,sub_task_id,provider_call_id,cycle,seq,attempt,outcome,
            raw_report_text,parsed_result_json,validation_json,created_at)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`).run(`${params.sessionId}:${params.cycle}:${params.seq}:${params.attempt}`, params.sessionId, params.subTaskId, params.providerCallId ?? null, params.cycle, params.seq, params.attempt, params.validation?.kind ?? "unstructured", params.finalMessage ? redactTokenShapes(params.finalMessage).slice(0, 262144) : null, parsed === undefined ? null : JSON.stringify(parsed), JSON.stringify(params.validation ?? { ok: false, kind: "unstructured", reason: "no observe contract" }), Date.now());
        }
        catch (err) {
            this.deps.logger.warn("[loop] could not persist observe attempt report", {
                sessionId: params.sessionId,
                seq: params.seq,
    ...
    beginProviderCall(params) {
        ...
        try {
            ...
            const unresolved = db.prepare(`SELECT id,status,role FROM provider_calls
          WHERE session_id = ? AND status IN ('started','unknown')
          ORDER BY started_at LIMIT 1`).get(params.sessionId);
            if (unresolved) {
                throw new Error(`provider call ${unresolved.id} (${unresolved.role}) remains ${unresolved.status}; reconcile it before dispatch`);
            }
            const row = db.prepare(`SELECT COUNT(*) AS n FROM provider_calls
          WHERE session_id = ? AND role = ? AND COALESCE(cycle,-1) = COALESCE(?,-1)
            AND COALESCE(seq,-1) = COALESCE(?,-1)`).get(params.sessionId, params.role, params.cycle ?? null, params.seq ?? null);
            const attempt = (row?.n ?? 0) + 1;
            db.prepare(`INSERT INTO provider_calls
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?, 'started', ?)`).run(id, params.sessionId, params.subTaskId ?? null, params.role, params.cycle ?? null, params.seq ?? null, attempt, params.model ?? null, params.route ?? null, params.baseSha ?? null, Date.now());
            ...
            try {
                db.prepare(`UPDATE sessions SET accounting_state = 'incomplete', status = 'accounting_incomplete',
                               worktree_preserved = 1, updated_at = ? WHERE id = ?`).run(Date.now(), params.sessionId);
            }
            catch { /* preserve original */ }
            throw new AccountingPersistenceError(`provider dispatch was not recorded: ${String(err)}`);
        }
    }
    finishProviderCall(sessionId, id, data) {
        if (id.startsWith("untracked-test-double:"))
            return;
        const db = this.deps.state.db;
        try {
            const changed = db.prepare(`UPDATE provider_calls
            SET status = ?, cost_usd = ?, provider_result_id = ?, result_json = ?,
                verification_json = ?, ended_at = ?
          WHERE id = ? AND session_id = ? AND status = 'started'`).run(data.status, data.costUsd ?? null, data.providerResultId ?? null, data.result === undefined ? null : JSON.stringify(data.result), data.verification === undefined ? null : JSON.stringify(data.verification), Date.now(), id, sessionId);
            if (changed.changes !== 1)
                throw new Error(`provider call ${id} was not in started state`);
        }
        catch (err) {
            try {
                db.prepare(`UPDATE sessions SET accounting_state = 'incomplete', status = 'accounting_incomplete',
                               worktree_preserved = 1, updated_at = ? WHERE id = ?`).run(Date.now(), sessionId);
            }
    ...
    finishProviderCallWithSpend(sessionId, id, requester, data) {
        if (id.startsWith("untracked-test-double:"))
            return;
        const db = this.deps.state.db;
        const now = Date.now();
        ...
        const completionFingerprint = createHash("sha256").update(JSON.stringify({
            costUsd: data.costUsd,
            providerResultId: data.providerResultId ?? null,
            providerCumulativeCostUsd: data.providerCumulativeCostUsd ?? null,
            providerCostBaselineUsd: data.providerCostBaselineUsd ?? null,
            providerCostCurrency: data.providerCostCurrency ?? null,
            result: data.result ?? null,
            verification: data.verification ?? null,
        })).digest("hex");
        try {
            db.exec("BEGIN IMMEDIATE");
            if (!Number.isFinite(data.costUsd) || data.costUsd < 0) {
                throw new Error(`provider cost is not a finite non-negative amount: ${String(data.costUsd)}`);
            }
            if (data.providerCumulativeCostUsd !== undefined &&
                (!Number.isFinite(data.providerCumulativeCostUsd) || data.providerCumulativeCostUsd < 0)) {
                throw new Error(`provider cumulative cost is invalid: ${String(data.providerCumulativeCostUsd)}`);
            }
            const changed = db.prepare(`UPDATE provider_calls
            SET status = 'completed', cost_usd = ?, provider_result_id = ?, result_json = ?,
                verification_json = ?, completion_fingerprint = ?, ended_at = ?
          WHERE id = ? AND session_id = ? AND status = 'started'`).run(data.costUsd, data.providerResultId ?? null, data.result === undefined ? null : JSON.stringify(data.result), data.verification === undefined ? null : JSON.stringify(data.verification), completionFingerprint, now, id, sessionId);
            if (changed.changes !== 1) {
                const existing = db.prepare(`SELECT status,completion_fingerprint FROM provider_calls WHERE id = ? AND session_id = ?`).get(id, sessionId);
                if (existing?.status === "completed" && existing.completion_fingerprint === completionFingerprint) {
                    db.exec("COMMIT");
                    return;
                }
                throw new Error(`provider call ${id} was not in started state or completion conflicted`);
            }
            db.prepare(`UPDATE sessions SET cost_usd = cost_usd + ?, accounting_state = 'ok', updated_at = ? WHERE id = ?`).run(data.costUsd, now, sessionId);
            db.prepare(`INSERT INTO budgets_daily (day, user, spent_usd) VALUES (?, ?, ?)
         ON CONFLICT (day, user) DO UPDATE SET spent_usd = spent_usd + excluded.spent_usd`).run(day, requester, data.costUsd);
            db.prepare(`INSERT INTO budgets_monthly (month, user, spent_usd) VALUES (?, ?, ?)
         ON CONFLICT (month, user) DO UPDATE SET spent_usd = spent_usd + excluded.spent_usd`).run(month, requester, data.costUsd);
            if (data.providerResultId &&
                typeof data.providerCumulativeCostUsd === "number" &&
                Number.isFinite(data.providerCumulativeCostUsd)) {
                if (typeof data.providerCostBaselineUsd !== "number" ||
                    !Number.isFinite(data.providerCostBaselineUsd) ||
                    data.providerCostCurrency !== "USD") {
                    throw new Error("provider cumulative checkpoint is missing a valid USD baseline");
                }
                const checkpoint = db.prepare(`SELECT cumulative_cost_usd,currency,checkpoint_version
             FROM provider_session_usage
            WHERE backend = 'opencode' AND provider_session_id = ?`).get(data.providerResultId);
                if (checkpoint &&
                    (checkpoint.currency !== "USD" ||
                        Math.abs(checkpoint.cumulative_cost_usd - data.providerCostBaselineUsd) > 1e-9)) {
                    throw new Error(`provider cumulative checkpoint moved: stored ${checkpoint.cumulative_cost_usd} ${checkpoint.currency}, ` +
                        `turn baseline ${data.providerCostBaselineUsd} ${data.providerCostCurrency}`);
                }
                db.prepare(`INSERT INTO provider_session_usage
             (backend,provider_session_id,currency,cumulative_cost_usd,checkpoint_version,last_provider_call_id,updated_at)
           VALUES ('opencode',?,'USD',?,?,?,?)
           ON CONFLICT (backend,provider_session_id) DO UPDATE SET
             currency = excluded.currency,
             cumulative_cost_usd = excluded.cumulative_cost_usd,
             checkpoint_version = provider_session_usage.checkpoint_version + 1,
             last_provider_call_id = excluded.last_provider_call_id,
             updated_at = excluded.updated_at`).run(data.providerResultId, data.providerCumulativeCostUsd, checkpoint?.checkpoint_version ?? 1, id, now);
            }
            this.deps.state.audit("budget.spend", { user: requester, amountUsd: data.costUsd, sessionId, providerCallId: id }, sessionId);
            db.exec("COMMIT");
        }
        catch (err) {
            try {
    ...
    async runAccountedWorker(meta, invoke) {
        ...
        this.deps.interactionLog?.logSdkRequest(meta.sessionId, {
            ...
            seq: meta.seq,
            cycle: meta.cycle,
            prompt: `[accounting marker only; actual prompt receipt is recorded on worker_end_turn] physical worker invocation ${call.attempt}`,
            callId: call.id,
        ...
        try {
            ...
            if (result.usageMeasured === false) {
                ...
                try {
                    this.deps.state.db.prepare(`UPDATE sessions SET accounting_state = 'incomplete', status = 'accounting_incomplete',
                                 worktree_preserved = 1, updated_at = ? WHERE id = ?`).run(Date.now(), meta.sessionId);
                }
                ...
                throw new AccountingPersistenceError(`provider returned without measurable usage/cost (${result.usageSource ?? "unavailable"})`);
            }
            this.finishProviderCallWithSpend(meta.sessionId, call.id, meta.requester, {
                costUsd: result.costUsd,
                providerResultId: result.sdkSessionId ?? null,
                providerCumulativeCostUsd: result.providerCumulativeCostUsd,
                providerCostBaselineUsd: result.providerCostBaselineUsd,
                providerCostCurrency: result.providerCostCurrency,
                result: {
                    workerStatus: result.status,
                    reason: result.reason ?? null,
                    ...
                    actualPromptChars: result.actualPromptChars ?? null,
            ...
            this.assertConfirmedControlAuthority(meta.sessionId, {
                ...
                projectedBudgetUsd: Number(this.deps.state.db.prepare(`SELECT cost_usd FROM sessions WHERE id=?`).get(meta.sessionId)?.cost_usd ?? 0),
                projectedCycles: meta.cycle,
                projectedRetries: Math.max(0, call.attempt - 1),
            });
            ...
            if (err instanceof AccountingPersistenceError || err instanceof ConfirmedControlAuthorityError)
                throw err;
            const measuredErrorCost = err?.costUsd;
    ...
    async runAccountedProvider(meta, invoke, describe) {
    ...
    attachProviderVerification(sessionId, cycle, seq, verification) {
        if (typeof this.deps.state.db.exec !== "function")
            return;
        try {
            const changed = this.deps.state.db.prepare(`UPDATE provider_calls SET verification_json = ?
          WHERE id = (
            ...
            if (changed.changes !== 1)
                throw new Error("no completed provider call accepted verification");
            const ok = verification && typeof verification === "object" && "ok" in verification
                ? Boolean(verification.ok)
                : undefined;
            this.deps.state.db.prepare(`UPDATE sub_task_attempts
            SET verification_status = ?, verification_json = ?, task_outcome = ?
          WHERE id = (
            SELECT id FROM sub_task_attempts
             WHERE session_id = ? AND cycle = ? AND seq = ?
             ORDER BY attempt DESC LIMIT 1
          )`).run(ok === undefined ? "recorded" : ok ? "passed" : "failed", JSON.stringify(verification), ok === false ? "failed_verification" : "completed", sessionId, cycle, seq);
        }
        catch (err) {
            try {
                this.deps.state.db.prepare(`UPDATE sessions SET accounting_state = 'incomplete', status = 'accounting_incomplete',
     ...
     */
    priorAttemptCommits(sessionId, cycle, seq) {
        try {
            const rows = this.deps.state.db
                .prepare(`SELECT commit_shas FROM sub_task_attempts WHERE session_id = ? AND cycle = ? AND seq = ? ORDER BY attempt`)
                .all(sessionId, cycle, seq);
            const out = new Set();
            for (const r of rows) {
                try {
                    for (const s of JSON.parse(r.commit_shas ?? "[]")) {
    ...
    async durableCheckpoint(params) {
        ...
        try {
            ...
            /*
             * rc.10 (F1): authenticate BEFORE bundling.
             *
             * A bundle of a partial clone reaches the network. Resolving credentials
             * is itself allowed to fail -- a vault that will not open must produce an
             * honest "checkpoint not durable" rather than take the run down -- so the
             * resolution is audited and the checkpoint proceeds unauthenticated,
             * which succeeds for a local or public repo and fails legibly otherwise.
             */
            const repo = (params.repo ?? "").trim();
            const requester = (params.requester ?? "").trim();
            if (this.deps.checkpointGitRunner && repo && requester) {
                try {
                    auth = await this.deps.checkpointGitRunner({ repo, requester });
                }
                catch (err) {
                    this.deps.state.audit("loop.checkpoint_auth_unavailable", { sessionId, cycle, trigger, repo, error: redactTokenShapes(String(err)) }, sessionId);
                }
            }
            ...
            this.deps.state.audit("loop.checkpoint_failed", { sessionId, cycle, trigger, error: redactTokenShapes(String(err)) }, sessionId);
        }
        finally {
            // rc.10: the askpass helper's lifetime ends with the checkpoint, whatever
            // happened to it.
            if (auth)
    ...
    async runInner(sessionId, brief) {
        const row = this.deps.state.db
            .prepare(`SELECT id, requester, cost_usd, budget_usd, cycles_ran, status, branch, repo FROM sessions WHERE id = ?`)
            .get(sessionId);
        if (!row)
        ...
        const activeDeadline = resumeActiveDeadline(this.deps.state.db, sessionId, sessionTimeoutSeconds, startedAt);
        ...
        // rc.10 (F3): planned writes the denylist will refuse, found before spend.
        let planPolicyConflicts = [];
        // 1. Planning
        ...
        if (brief.resumeExistingPlan) {
            try {
                ...
                return await this.finaliseFailedPreserveWorktree(sessionId, `plan_resume_failed: ${String(err)}`, row.cycles_ran, row.cost_usd);
            }
        }
        ...
        try {
            ...
            plan = acceptedContinuation?.plan ?? await this.runAccountedProvider({
                ...
                role: "lead",
                cycle: 0,
                model: this.routeLog("lead", this.deps.config.models.lead).model,
            ...
            }), this.deps.config.loop.lead_timeout_seconds + scoutBudget, "lead_timeout_seconds"), (leadPlan) => ({
            ...
            this.deps.state.db
                .prepare(`UPDATE sessions SET lead_plan_json = ?, repo = ?, branch = ?, worktree_path = ?,
                               storage_state = 'ok', storage_reason = NULL, storage_checked_at = ?
            WHERE id = ?`)
                .run(JSON.stringify(plan), plan.repo, plan.branch, plan.worktreePath, Date.now(), sessionId);
                ...
                try {
                    ...
                    if (!existing?.revision_start_sha) {
                        ...
                        if (head) {
                            ...
                            this.deps.state.audit("loop.revise_baseline_captured", {
                                sessionId,
                            ...
                            this.deps.logger.warn("[loop] could not read the revision-start HEAD; scope enforcement will fall back to the whole feature diff", { sessionId });
                        }
                    }
                }
                catch (err) {
            ...
            if (this.deps.config.loop.resume_ledger_guard_enabled !== false &&
                ...
                if (check.failed) {
                    ...
                    return await this.finaliseFailed(sessionId, `ledger_commits_unreachable_at_resume: ${check.detail}`, 1, 0);
                }
            }
            ...
            /*
             * rc.10 (F3, audits 5598/5619): compare the plan against the safety
             * policy NOW, while it is still free to do so.
             *
             * Sub-task 3 required `.env.example`, the denylist contains `.env.*`, and
             * nothing put those two facts together until a worker had been dispatched
             * and billed -- twice, the second time after an operator correction that
             * changed the test path and left the policy untouched. Both were knowable
             * here, before the plan was accepted.
             *
             * Recorded, not enforced. `filesLikelyTouched` is the lead's guess, so
             * this is evidence that a sub-task is heading for a wall and not proof
             * that it must hit one. The dispatch gate below acts on it per sub-task;
             * this row is what makes the conflict visible from the first minute of
             * the run rather than from the first denial.
             */
            if (this.deps.config.loop.plan_policy_conflict_check_enabled !== false) {
                const conflicts = findPlanPolicyConflicts(plan.subTasks, this.deps.config.safety?.path_denylist ?? [], this.deps.config.safety?.path_denylist_exceptions ?? []);
                if (conflicts.length > 0) {
                    planPolicyConflicts = conflicts;
                    this.deps.state.audit("loop.plan_policy_conflict", { sessionId, count: conflicts.length, conflicts: conflicts.map((c) => ({ seq: c.seq, path: c.path, rule: c.rule })) }, sessionId);
                    this.deps.interactionLog?.log(sessionId, {
                        event: "plan_policy_conflict", phase: "plan", count: conflicts.length,
                        paths: conflicts.map((c) => c.path),
                    });
                    this.deps.logger.warn("[loop] rc.10: the plan requires a write the safety denylist will refuse", {
                        sessionId, conflicts: conflicts.map((c) => `${c.path} (rule ${c.rule}, seq ${c.seq})`),
                    });
                }
            }
            const observeContractErrors = loadBearingObserveContractErrors(plan);
            if (observeContractErrors.length > 0) {
                this.deps.state.audit("loop.observe_contract_plan_rejected", { sessionId, errors: observeContractErrors }, sessionId);
                throw new Error(observeContractErrors.join("; "));
            }
        }
        catch (err) {
            if (err instanceof AccountingPersistenceError) {
                this.deps.state.audit("loop.plan_accounting_incomplete", { sessionId, error: err.message }, sessionId);
                return {
                    status: "failed",
                    sessionId,
        ...
        if (this.deps.tokenScopes && this.deps.config.loop.workflow_scope_precheck !== false) {
            ...
            if (workflowFiles.length > 0) {
                ...
                if (verdict === false) {
                    ...
                    return await this.finaliseFailed(sessionId, `workflow_scope_missing: ${describeMissingWorkflowScope(workflowFiles)}`, 0, row.cost_usd);
                }
            }
        }
        if (planPolicyConflicts.length > 0 &&
            this.deps.config.loop.clarification_escalation_enabled !== false) {
            const firstSeq = Math.min(...planPolicyConflicts.map((conflict) => conflict.seq));
            const task = plan.subTasks.find((candidate) => candidate.seq === firstSeq);
            const conflicts = planPolicyConflicts.filter((conflict) => conflict.seq === firstSeq);
            this.deps.state.audit("loop.plan_policy_conflict_pre_dispatch", {
                sessionId,
                seq: firstSeq,
                conflicts: conflicts.map((conflict) => ({ path: conflict.path, rule: conflict.rule })),
                dispatchedWorkers: 0,
            }, sessionId);
            return await this.finaliseAwaitingClarification(sessionId, describePlanPolicyConflicts(conflicts), firstSeq, 0, row.cost_usd + leadPlanningCostUsd, {
                title: task?.title ?? `sub-task ${firstSeq}`,
                intent: task?.intent ?? "",
                task: task ? structuredClone(task) : undefined,
                policyConflicts: conflicts.map((conflict) => ({ path: conflict.path, rule: conflict.rule })),
            });
        }
        let cycle = 0;
        ...
        let ciPolledSha = "";
        const requiredBehaviorChecks = plan.subTasks.flatMap((task) => (task.requiredBehaviorChecks ?? []).filter((check) => check.required !== false));
        let behaviorVerificationPassed = requiredBehaviorChecks.length === 0;
        let behaviorVerificationFailure = null;
        ...
        shipAttempts: for (;;) {
             ...
             */
                ...
                const runOneInner = async (st, workerWorktree) => {
                    ...
                    const reviseHint = isResumedSeq
                        ...
                        : hintWithSuspects;
                    if (isResumedSeq) {
                        this.deps.state.audit("loop.subtask_resumed_with_answer", {
                    ...
                    if (confirmedControl && totalCost > row.budget_usd) {
                        failed.err = "budget_exceeded";
                        failed.seq = st.seq;
                        return;
                    }
             ...
             VALUES (?, ?, ?, ?, ?, ?, 'running', 0, ?, ?, ?)`).run(subTaskId, sessionId, cycle, st.seq, st.title, ledgerModel, now, now, now);
                    ...
                    // rc.10: the commits earlier attempts of this same sub-task produced.
                    // Read once per turn, before the worker runs, so this attempt's own
                    // commits are not in it -- the point is what came BEFORE.
                    const priorCommitsForVerify = this.priorAttemptCommits(sessionId, cycle, st.seq);
                    ...
                    /*
                     * rc.10 (F3): do not buy a turn that safety policy will refuse.
                     *
                     * Sub-task 3 of session aad3fc57 required `.env.example` against a
                     * denylist containing `.env.*`. It was dispatched anyway, denied
                     * (audit 5598), escalated as a path mismatch, corrected by an operator
                     * in a way that did not touch policy, dispatched AGAIN, and denied
                     * identically (audit 5619). The second dispatch cost $0.4262756 to
                     * re-learn a fact the harness held before the first one.
                     *
                     * Scoped to the one sub-task: the rest of the plan is unaffected and
                     * still runs. `isResumedSeq` is the release -- once an operator has
                     * answered for this seq, their decision governs and we do not re-ask,
                     * which is what stops this becoming a loop when the answer is "do it
                     * another way" rather than a config change. If the answer WAS a config
                     * change the conflict is simply gone by now, because the check reads
                     * live config.
                     */
                    const myPolicyConflicts = planPolicyConflicts.filter((c) => c.seq === st.seq);
                    if (myPolicyConflicts.length > 0 &&
                        !isResumedSeq &&
                        !clarify.question &&
                        this.deps.config.loop.clarification_escalation_enabled !== false) {
                        const summary = `blocked by safety policy before dispatch: ` +
                            myPolicyConflicts.map((c) => `\`${c.path}\` (rule \`${c.rule}\`)`).join(", ") +
                            `; no worker was run, so nothing was spent on it`;
                        this.deps.state.db
                            .prepare(`UPDATE sub_tasks SET status = 'failed', summary = ?, updated_at = ? WHERE id = ?`)
                            .run(summary, Date.now(), subTaskId);
                        this.deps.state.audit("loop.plan_policy_conflict_gate", {
                            sessionId, seq: st.seq, subTaskId, cycle,
                            conflicts: myPolicyConflicts.map((c) => ({ path: c.path, rule: c.rule })),
                            dispatched: false,
                        }, sessionId);
                        this.deps.interactionLog?.log(sessionId, {
                            event: "plan_policy_conflict_gate", phase: "worker", seq: st.seq, cycle,
                            paths: myPolicyConflicts.map((c) => c.path),
                        });
                        this.deps.logger.warn("[loop] rc.10: refusing to dispatch a sub-task whose required write the denylist blocks", {
                            sessionId, seq: st.seq, conflicts: myPolicyConflicts.map((c) => `${c.path} (${c.rule})`),
                        });
           ...
           SET status = 'verifying', cost_usd = ?, files_touched = ?, commit_sha = ?, sdk_session_id = ?, summary = ?, completed_at = NULL, updated_at = ?
           WHERE id = ?`).run(result.costUsd, JSON.stringify(result.filesChanged), result.commitSha ?? null, result.sdkSessionId ?? null, result.reason ?? null, Date.now(), subTaskId);
                    ...
                    // this is where they become searchable history.
                    {
                        ...
                        if (recon) {
                            ...
                            if (recon.workerCommitSha) {
                                this.deps.state.audit("loop.worker_existing_commit_detected", { ...common, workerCommitSha: recon.workerCommitSha, headBefore: recon.headBefore }, sessionId);
                            }
                            if (recon.dirtyBefore.length > 0) {
                                this.deps.state.audit("loop.worker_dirty_worktree_detected", { ...common, dirtyFiles: recon.dirtyBefore.slice(0, 100), dirtyCount: recon.dirtyBefore.length }, sessionId);
                            }
                            if (recon.harnessCommitSha) {
                                this.deps.state.audit("loop.worker_safe_commit_created", { ...common, harnessCommitSha: recon.harnessCommitSha, files: recon.dirtyBefore.length }, sessionId);
                            }
                            if (recon.state === "uncommitted_changes" || recon.state === "git_error") {
                                this.deps.state.audit("loop.worker_commit_failure_reconciled", {
                                    ...common,
                    ...
                    // tool calls and no concluding text.
                    {
                        ...
                        this.deps.state.audit("loop.worker_end_turn", {
                            ...
                            allowedToolCalls: result.allowedToolCalls ?? null,
                            actualPromptChars: result.actualPromptChars ?? null,
                            actualPromptSha256: result.actualPromptSha256 ?? null,
                        }, sessionId);
                        ...
                        // rc.10: the same turn, in a row that the next attempt cannot
                        // overwrite. This is the one point every worker turn passes through
                        // regardless of how it ended, which is what makes it the honest place
                        // to append. It sits AFTER the progress post deliberately: beta.85
                        // requires deliverProgress to follow the worker_end_turn audit
                        // closely, and history is the one thing here with no deadline.
                        this.recordSubTaskAttempt({
                            ...
                            commitShas: result.commitShas ?? (result.commitSha ? [result.commitSha] : []),
                            filesTouched: result.filesChanged,
                            summary: fm.slice(0, 2000),
                            startedAtMs: subTaskStartedAtMs,
                        });
                    }
                    ...
                    const effectiveTaskMode = cycle > 1 && st.taskMode === "mutate" && !result.commitSha && !workerDirty
                        ? "observe"
                        : st.taskMode;
                    if (workerDirty && cycle > 1 && st.taskMode === "mutate" && !result.commitSha) {
                        this.deps.state.audit("loop.cycle_no_change_rejected_dirty", {
                            sessionId,
                            seq: st.seq,
                            cycle,
                    ...
                    /*
                     * rc.10 (audit 5591): give re-derivation the repository's own file list.
                     *
                     * Without it the only ground truth re-derivation has is the handful of
                     * paths THIS RUN touched, so a contract naming a file that has existed
                     * in the repo for a year looks exactly like a contract naming a file
                     * that never existed. That is how a real test path was rewritten to a
                     * phantom one: the sub-task meant to touch it had been denied, so the
                     * declared path was absent from the touched set and read as stale.
                     *
                     * Best-effort by design. When the listing fails the guard simply does
                     * not fire and the rc.10 kind rule still stands on its own.
                     */
                    const repoInventory = rederiveEnabled && this.deps.listRepoFiles
                        ? await this.deps.listRepoFiles(workerWorktree).catch(() => [])
                        : [];
                    ...
                    const contract = rawContract.map((v) => {
                        ...
                        if (!("path" in v) || !v.path || v.kind === "file_in_pr")
                            return v;
                        const rd = rederiveContractPath(v.path, [...discoveredRealPaths], { repoFiles: repoInventory });
                        if (rd.suggestion) {
                    ...
                    if (contract.length > 0 && this.deps.buildVerifyProbes) {
                        ...
                        try {
                            verification = await verifySubTaskOutput(contract, {
                                ...
                                // rc.10: what earlier attempts of THIS sub-task committed,
                                // so a continuation is not asked to re-edit a file it has
                                // already written correctly. See verify.ts.
                                priorAttemptCommits: priorCommitsForVerify,
                        ...
                        if (!verification.ok) {
                            ...
                            for (;;) {
                                ...
                                if (envWaitOnly) {
                                    ...
                                    const protocolRetryStartedAt = Date.now();
                                    try {
                                        ...
                                        const retry = await withTimeout(this.runAccountedWorker({
                                            sessionId,
                                            subTaskId,
                                            cycle,
                                            seq: st.seq,
                                            ...
                                            onActivity: this.makeWorkerActivityCallback(sessionId, st.seq, cycle),
                                        ...
                                        this.recordSubTaskAttempt({
                                            ...
                                            startedAtMs: protocolRetryStartedAt,
                                        ...
                                        this.deps.state.db.prepare(`UPDATE sub_tasks
                        SET cost_usd = cost_usd + ?, sdk_session_id = ?, summary = ?, updated_at = ?
                      WHERE id = ?`).run(retry.costUsd, retry.sdkSessionId ?? null, `${retry.reason ?? retry.status}; ${retryVerification.summary}`.slice(0, 2000), Date.now(), subTaskId);
                                        ...
                                        if (retryVerification.ok) {
                                            ...
                                            await this.durableCheckpoint({
                                                ...
                                                repo: row.repo, requester: row.requester,
                                            });
                            ...
                            /*
                             * rc.10 (F3, audits 5598/5601/5603): a deterministic denial is a
                             * fact about the turn, not a property of whether anything ELSE got
                             * committed.
                             *
                             * rc.9 predicated every policy-denial path on `!result.commitSha`,
                             * so the case that actually happened fell straight through it. The
                             * worker committed two implementation files AND was refused
                             * `.env.example`; because a commit existed, `policyDenied` was
                             * false, and the escalation took the contract-path-mismatch branch
                             * -- which asked the operator whether the missing template was a
                             * path mistake and offered "accept", "skip", or another path. The
                             * decisive fact, that a safety rule had refused the write, was not
                             * in the question at all.
                             *
                             * When the SAME denial recurred with no new commit (audit 5627) the
                             * classification was correct, which is the clearest possible
                             * evidence that the commit -- not the denial -- was deciding which
                             * question got asked.
                             *
                             * Read straight off the structured denials, independent of commit
                             * state and of NO_CHANGE_ONLY.
                             */
                            const deterministicDenial = policyDenialFrom(result.deniedToolCalls);
                            const policyDeniedWithPartialWork = !policyDenied && !!deterministicDenial && failedResults.length > 0;
                             ...
                             * some of the way there before being refused.
                             *
                             * Placed above the contract-path-mismatch escalation on purpose.
                             * That branch fires on `!!result.commitSha` and would otherwise
                             * claim this turn, which is precisely the substitution audit 5601
                             * made: a policy block reported as a possible path typo. Both facts
                             * belong in the question -- the work is real and must be preserved,
                             * AND the reason the rest is missing is a rule, not a mistake.
                             */
                            if (policyDeniedWithPartialWork && this.deps.config.loop.clarification_escalation_enabled !== false) {
                                const unmet = [...new Set(failedResults.map((x) => x.path).filter((p) => !!p))];
                                const committedFiles = (result.filesChanged ?? []).filter((f) => typeof f === "string" && !!f.trim());
                                this.deps.state.audit("loop.worker_policy_denied", {
                                    sessionId, seq: st.seq, subTaskId, cycle,
                                    code: deterministicDenial.code,
                                    rule: deterministicDenial.rule ?? null,
                                    paths: deterministicDenial.paths,
                                    tool: deterministicDenial.tool ?? null,
                                    attempts: deterministicDenial.attempts,
                                    retryCount: protocolRetries,
                                    failedKinds: failedResults.map((x) => x.kind),
                                    // rc.10: what distinguishes this row from the rc.9 one.
                                    withPartialCommit: true,
                                    commitSha: result.commitSha ?? null,
                                ...
                                this.deps.interactionLog?.log(sessionId, {
                                    event: "worker_policy_denied", phase: "worker", seq: st.seq, cycle,
                                    reasonFirstLine: `${deterministicDenial.code}${deterministicDenial.rule ? `: ${deterministicDenial.rule}` : ""}` +
                                        ` (partial commit preserved)`,
                                });
                                this.deps.logger.warn("[loop] rc.10: safety policy blocked part of this sub-task; the committed work stands and the block is the reason for the rest", { sessionId, seq: st.seq, rule: deterministicDenial.rule ?? null, paths: deterministicDenial.paths, unmet });
                                clarify.question = buildPolicyDenialClarification({
                                    seq: st.seq,
                                    title: st.title,
                                    policy: deterministicDenial,
                                    workerNote: terminalOutcome.explanation?.slice(0, 300),
                        ...
                        /*
                         * rc.10 (F4, audits 5572-5578): an observe turn must have LOOKED at
                         * something, not merely said something.
                         *
                         * The rc.2 gate above asks whether the report is narration, which is
                         * a judgement about English and duly missed a bare "I'll inspect ..."
                         * followed by a scope disclaimer. The counter beside it in the same
                         * audit row -- `unguardedReads: 0` -- was not a judgement about
                         * anything, and the harness already had it. The prerequisite was
                         * completed on four denied nested-agent calls and zero reads, and its
                         * promise was handed to two dependent workers as findings.
                         */
                        const observeHasNoEvidence = () => !st.observeContract &&
                            this.deps.config.loop.observe_evidence_check_enabled !== false &&
                            observeEvidenceVerdict(result).empty;
                        const validateStructuredObserve = async () => {
                            if (!st.observeContract)
                                return null;
                        ...
                        let priorObserveFailureFingerprint;
                        this.recordObserveAttemptReport({
                            sessionId,
                            subTaskId,
                            cycle,
                            seq: st.seq,
                            ...
                            finalMessage: result.finalMessage,
                            validation: observeValidation,
                        });
                        if (observeValidation?.kind === "blocked") {
                            const blockers = observeValidation.result.blockers?.filter((value) => typeof value === "string" && value.trim().length > 0) ?? [];
                            const findingIds = observeValidation.result.findings.map((finding) => finding.id);
                            const blockerLines = blockers.slice(0, 12).map((blocker) => `- ${blocker.slice(0, 800)}`).join("\n");
                            const question = `Sub-task ${st.seq} ("${st.title}") returned a validated BLOCKED observation. ` +
                                `No dependent mutation task was dispatched, and the full evidence report was preserved. ` +
                                `Review these worker-reported unresolved decisions; answer only the genuine authority or business-policy gaps, ` +
                                `and identify any item that is implementation work already authorised by the brief:\n${blockerLines}`;
                            this.attachProviderVerification(sessionId, cycle, st.seq, {
                                ok: false,
                                kind: "observe_blocked",
                                detail: observeValidation.reason,
                                findings: findingIds,
                            });
                            this.deps.state.db.prepare(`UPDATE sub_tasks SET status = 'blocked', summary = ?, updated_at = ? WHERE id = ?`).run(observeValidation.reason.slice(0, 2000), Date.now(), subTaskId);
                            this.deps.state.audit("loop.observe_contract_blocked", {
                                sessionId,
                                seq: st.seq,
                                cycle,
                                ...
                                missingRequired: observeValidation.missingRequired,
                            }, sessionId);
                            ...
                            clarify.subtask = {
                                ...
                                intent: st.intent,
                                task: structuredClone(st),
                                observeBlockedReport: {
                                    blockers,
                                    findingIds,
                                    providerCallId: result.providerCallId ?? null,
                                },
                            };
                            failed.err = `subtask_${st.seq}_observe_blocked`;
                            failed.seq = st.seq;
                            return;
                        }
                        while (((!st.observeContract && observeReportIsNarration(result.finalMessage)) || observeHasNoEvidence() || !structuredObserveOk) &&
                            ...
                            const observeFailureFingerprint = createHash("sha256")
                                ...
                                kind: observeValidation?.kind ?? "legacy",
                                reason: observeValidation && !observeValidation.ok
                                    ? observeValidation.reason
                                    : observeEvidenceVerdict(result).reason ?? "narration",
                                finalMessage: result.finalMessage ?? "",
                            }))
                                .digest("hex");
                            if (priorObserveFailureFingerprint === observeFailureFingerprint) {
                                this.deps.state.audit("loop.observe_retry_repeated", { sessionId, seq: st.seq, cycle, fingerprint: observeFailureFingerprint, retryCount: observeRetries }, sessionId);
                                break;
                            }
                            priorObserveFailureFingerprint = observeFailureFingerprint;
                            const currentStructuredReason = observeValidation && !observeValidation.ok ? observeValidation.reason : undefined;
                            this.attachProviderVerification(sessionId, cycle, st.seq, {
                                ok: false,
                                kind: "observe_report",
                                detail: currentStructuredReason ??
                            ...
                            this.deps.interactionLog?.log(sessionId, {
                                event: "worker_protocol_retry", phase: "worker", seq: st.seq, cycle,
                                reasonFirstLine: observeEvidenceVerdict(result).reason ?? "observe report was progress narration",
                            });
                            this.deps.state.audit("loop.worker_protocol_retry", {
                                sessionId, seq: st.seq, subTaskId, cycle,
                                outcome: observeValidation?.kind ?? "progress_only",
                                taskMode: "observe",
                                category: null,
                            ...
                            // rc.10: when the turn was denied its way of working, say so and
                            ...
                            const observeHint = structuredReason
                                ? `OBSERVE CONTRACT ${observeValidation?.kind ?? "invalid"} (attempt ${observeRetries + 1}/${observeMaxAttempts}): ${structuredReason}.\n` +
                                    renderObserveContractInstructions(st.observeContract)
                                : evidenceVerdict.empty
                                    ...
                                    : buildProtocolRetryHint({
                                        ...
                                        contractSummary: `a written report answering this probe's question -- ${st.intent || st.title}. ` +
                            ...
                            const observeRetryStartedAt = Date.now();
                            try {
                                const retry = await withTimeout(this.runAccountedWorker({
                                    sessionId,
                                    subTaskId,
                                ...
                                result = retry;
                                this.deps.state.db.prepare(`UPDATE sub_tasks
                    SET status = 'verifying', cost_usd = cost_usd + ?, sdk_session_id = ?,
                        summary = ?, updated_at = ? WHERE id = ?`).run(retry.costUsd, retry.sdkSessionId ?? null, (retry.reason ?? retry.status).slice(0, 2000), Date.now(), subTaskId);
                                this.recordSubTaskAttempt({
                                    sessionId,
                                    subTaskId,
                                    seq: st.seq,
                                    ...
                                    startedAtMs: observeRetryStartedAt,
                                    workerStatus: retry.status,
                                ...
                                this.recordObserveAttemptReport({
                                    ...
                                    seq: st.seq,
                                    attempt: observeRetries + 1,
                                    providerCallId: retry.providerCallId,
                                ...
                                if (observeValidation?.kind === "blocked") {
                                    ...
                                    const findingIds = observeValidation.result.findings.map((finding) => finding.id);
                                    clarify.question =
                                        `Sub-task ${st.seq} ("${st.title}") returned a validated BLOCKED observation. ` +
                                            `No dependent mutation task was dispatched, and the full evidence report was preserved. ` +
                                            `Review these worker-reported unresolved decisions; answer only genuine authority or business-policy gaps:\n` +
                                            blockers.slice(0, 12).map((blocker) => `- ${blocker.slice(0, 800)}`).join("\n");
                                    clarify.seq = st.seq;
                                    clarify.subtask = {
                                        title: st.title,
                                        intent: st.intent,
                                        ...
                                        observeBlockedReport: {
                                            blockers,
                                            findingIds,
                                            providerCallId: retry.providerCallId ?? null,
                                        },
                                    };
                                    this.deps.state.db.prepare(`UPDATE sub_tasks SET status = 'blocked', summary = ?, updated_at = ? WHERE id = ?`).run(observeValidation.reason.slice(0, 2000), Date.now(), subTaskId);
                                    this.deps.state.audit("loop.observe_contract_blocked", {
                        ...
                        if ((!st.observeContract && observeReportIsNarration(result.finalMessage)) || observeHasNoEvidence() || !structuredObserveOk) {
                            ...
                            //
                            // rc.10: say which check refused it. "Ended describing what it
                            ...
                            const failSummary = `observe sub-task produced no findings after ${observeRetries} retr${observeRetries === 1 ? "y" : "ies"}: ` +
                                failReason;
                            this.attachProviderVerification(sessionId, cycle, st.seq, {
                                ok: false,
                                kind: "observe_report",
                            ...
                            this.deps.state.audit("loop.worker_retry_exhausted", {
                                ...
                                hasCommit: Boolean(result.commitSha),
                                failedKinds: [],
                                failedChecks: [{ kind: "observe_report", detail: failReason }],
                                contract: "a written report answering the probe's question",
                                finalMessage: (result.finalMessage ?? "").slice(0, 1000),
                                finalOutcome: "failed_verification",
                                // rc.10: the non-linguistic evidence, so a reader can tell a
                                // narrating worker from a blocked one without re-reading prose.
                                evidenceCode: exhaustedVerdict.code ?? null,
                                allowedToolCalls: result.allowedToolCalls ?? null,
                                unguardedReads: result.unguardedReads ?? null,
                                deniedToolCalls: (result.deniedToolCalls ?? []).length,
                            }, sessionId);
                            this.deps.interactionLog?.log(sessionId, {
                                event: "worker_retry_exhausted", phase: "worker", seq: st.seq, cycle,
                                reasonFirstLine: failReason.slice(0, 300),
                            });
                            this.deps.state.db.prepare(`UPDATE sub_tasks SET status = 'failed_verification', summary = ?, updated_at = ? WHERE id = ?`).run(failSummary, Date.now(), subTaskId);
                        ...
                        if (st.observeContract && observeValidation?.ok) {
                            try {
                                ...
                                validatePlan(bound.plan, this.deps.config);
                                const changedTasks = bound.plan.subTasks.filter((task) => bound.changedConsumers.includes(task.seq));
                                const conflicts = findPlanPolicyConflicts(changedTasks, this.deps.config.safety?.path_denylist ?? [], this.deps.config.safety?.path_denylist_exceptions ?? []);
                                if (conflicts.length > 0) {
                                    throw new Error(`observe bindings would create a policy conflict: ${conflicts.map((c) => `${c.path} (${c.rule})`).join(", ")}`);
                                }
                                ...
                                if (workflowFiles.length > 0 && this.deps.tokenScopes) {
                                    ...
                                    if (canPush === false) {
                                        throw new Error(`observe bindings introduced workflow files but the routed token lacks workflow scope: ${workflowFiles.join(", ")}`);
                                    }
                                }
                                const resultRevision = sourceRevision + (bound.changedConsumers.length > 0 ? 1 : 0);
                                const reportId = `${sessionId}:${cycle}:${st.seq}:${sourceRevision}:${observeValidation.bindingsHash.slice(0, 16)}`;
                 ...
                 VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`).run(reportId, sessionId, subTaskId, cycle, st.seq, observeTaskHash(st), sourceRevision, resultRevision, JSON.stringify({ finalMessage: result.finalMessage ?? "", result: observeValidation.result }), observeValidation.bindingsHash, JSON.stringify({ ok: true, changedConsumers: bound.changedConsumers }), Date.now());
                                ...
                                if (bound.changedConsumers.length > 0) {
                                    ...
                                    for (const seq of bound.changedConsumers) {
                                        ...
                                        if (currentTask && revisedTask)
                                            Object.assign(currentTask, structuredClone(revisedTask));
                                    }
                                }
                                this.deps.state.audit("loop.observe_contract_validated", {
                                    sessionId,
                                    seq: st.seq,
                                    cycle,
                                ...
                                this.deps.state.db.prepare(`UPDATE sub_tasks SET status = 'failed_verification', summary = ?, updated_at = ? WHERE id = ?`).run(failSummary, Date.now(), subTaskId);
                                this.deps.state.audit("loop.observe_contract_apply_failed", { sessionId, seq: st.seq, cycle, error: String(err) }, sessionId);
                                failed.err = `subtask_${st.seq}_failed_verification: ${failSummary}`;
                                failed.seq = st.seq;
                                return;
                            }
                     ...
                     */
                    this.deps.state.db.prepare(`UPDATE sub_tasks SET status = 'completed', completed_at = ?, updated_at = ? WHERE id = ?`).run(Date.now(), Date.now(), subTaskId);
                    this.checkpoint(sessionId, cycle, subTaskId, result.sdkSessionId, "completed");
                    await this.durableCheckpoint({
                        sessionId, cycle, subTaskId, trigger: "sub_task_complete",
                        worktreePath: workerWorktree, branch: (row.branch ?? "").trim() || null,
                ...
                for (let i = 0;; i++) {
                    if (i >= ordered.length) {
                        if (generationChecked || failed.err)
                            break;
                ...
                if (failed.err) {
                    ...
                    if (String(failed.err).startsWith("accounting_incomplete")) {
                        this.deps.state.audit("loop.accounting_incomplete", { sessionId, seq: failed.seq, reason: String(failed.err), worktreePreserved: true }, sessionId);
                        return {
                            status: "failed",
                            sessionId,
                ...
                if (this.deps.config.loop.early_exit_no_change_cycle !== false &&
                    ...
                    const dirtyNow = tipNow && tipNow === cycleBaseSha ? await this.worktreeDirtyFiles(sessionId, plan.worktreePath) : [];
                ...
                try {
                    ...
                    report = await this.runAccountedProvider({
                        ...
                        role: "adversary",
                        cycle,
                        model: this.routeLog("adversary", this.deps.config.models.adversary).model,
                        route: this.routeLog("adversary", this.deps.config.models.adversary).backend,
                        baseSha: adversaryBaseSha,
                    }, () => withTimeout(this.deps.runAdversary({ brief, plan, sessionId, runtime, requester: row.requester, baseSha: adversaryBaseSha, priorFindings: lastReview?.findings, revision: revisionContext }), this.deps.config.loop.adversary_timeout_seconds, "adversary_timeout_seconds"), (review) => ({
                    ...
                    const accountingCause = accountingError ? err.cause : undefined;
                ...
                if (report.verdict === "pass" &&
                    ...
                    try {
                        ...
                        return await this.finaliseFailedPreserveWorktree(sessionId, `preview_push_error (${diagnosis.kind}; worktree preserved): ${String(err)}`, cycle, totalCost);
                    }
                    let previewRuntime;
                    try {
                    ...
                    if (previewRuntime?.provider !== "vercel" ||
                        ...
                        return await this.finaliseFailedPreserveWorktree(sessionId, `preview_runtime_unavailable (exact SHA ${previewHeadSha.slice(0, 12)} was not verified; worktree preserved)`, cycle, totalCost);
                    }
                    ...
                    try {
                        const runtimeReport = await this.runAccountedProvider({
                            ...
                            route: this.routeLog("adversary", this.deps.config.models.adversary).backend,
                            baseSha: adversaryBaseSha,
                        }, () => withTimeout(this.deps.runAdversary({
                            brief,
                            plan,
                            sessionId,
                        ...
                        }), this.deps.config.loop.adversary_timeout_seconds, "adversary_timeout_seconds"), (review) => ({
                            costUsd: review.costUsd,
                            usageMeasured: review.usageMeasured,
                            providerResultId: review.sdkSessionId ?? null,
            ...
            try {
                ...
                return await this.finaliseFailedPreserveWorktree(sessionId, `pr_error (${diagnosis.kind}; worktree preserved): ${describePreservedPushFailure({
            ...
            // beta.127: declared outside the ship-attempt loop, above.
            {
                ...
                if (!headSha) {
                    ...
                    if (headSha) {
                        ...
                        this.deps.state.audit("loop.ci_polled_unverified_sha", { sessionId, cycle, sha: headSha, reason: "no verified publication evidence for this candidate" }, sessionId);
                    }
                }
                if (headSha && (this.deps.ciCombinedStatus || this.deps.ciSnapshot)) {
                    ...
                    if (ci.outcome === "success") {
                        ...
                        if (missing.length > 0) {
                            ...
                            this.deps.state.audit("loop.behavior_verification_failed", {
                                sessionId,
                                cycle,
                            ...
                            this.deps.state.audit("loop.behavior_verification_passed", { sessionId, cycle, candidateSha: headSha, observed: observedNames, required: required.map((check) => check.ciCheck) }, sessionId);
                        }
                    }
            ...
            // reason: without the failing assertion there is nothing to hand a worker.
            {
                ...
                const repairFunding = assessRepairFunding({
                    ...
                    projectedRepairCostUsd: ciRepairCyclesGranted > 0
                ...
                if (canRepair) {
                    ...
                    if (ciRepairCyclesGranted === 0)
                        repairSpendBaselineUsd = totalCost;
        ...
        if (!behaviorVerificationPassed) {
            return await this.finaliseFailedPreserveWorktree(sessionId, `behavior_verification_failed: ${behaviorVerificationFailure ?? "required CI checks never produced a green verdict on the candidate SHA"}`, cycle, totalCost);
        }
        ...
        this.deps.state.audit("loop.shipped", {
    ...
    loadAcceptedContinuation(sessionId) {
        const row = this.deps.state.db
            .prepare(`SELECT lead_plan_json, repo, branch,
                clarification_seq, clarification_question, clarification_answer
    ...
    hydrateObserveReports(sessionId, plan) {
        ...
        try {
            ...
            for (const [seq, report] of legacy)
                if (!out.has(seq))
                    out.set(seq, report);
            if (out.size > 0) {
    ...
    async tryReleaseWorktree(sessionId, repoFullName, worktreePath, reason) {
        ...
        try {
            ...
            if (outcome.ok) {
                this.deps.state.db.prepare(`UPDATE sessions SET storage_state = 'released', storage_reason = ?,
                               storage_checked_at = ?, updated_at = ? WHERE id = ?`).run(`worktree intentionally released after terminal ${reason}`, Date.now(), Date.now(), sessionId);
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            try {
                ...
                if (err instanceof WorkerTimeoutError) {
                    ...
                    try {
                        const now = Date.now();
                        this.deps.state.db.prepare(`UPDATE provider_calls
                  SET status = 'unknown', result_json = ?, ended_at = ?
                WHERE session_id = ? AND role = 'worker' AND cycle = ? AND seq = ? AND status = 'started'`).run(JSON.stringify({ error: "local worker timeout; provider completion and cost unknown" }), now, sessionId, cycle, st.seq);
                        this.deps.state.db.prepare(`UPDATE sessions SET accounting_state = 'incomplete', status = 'accounting_incomplete',
                                   worktree_preserved = 1, updated_at = ? WHERE id = ?`).run(now, sessionId);
                    }
     ...
     * than a wrong merge.
     */
    async appendGenerationSubTask(params) {
        const { sessionId, plan, cycle, ordered } = params;
        if (this.deps.config.verify?.append_generation_subtask !== true)
            return null;
        ...
        if (!this.deps.worktreeCommittedFiles || !plan.worktreePath || !baseSha)
            return null;
        const changedFiles = await this.deps.worktreeCommittedFiles(plan.worktreePath, baseSha).catch(() => []);
        ...
        const subTask = {
            ...
            intent: `Declared inputs of ${scripts.map((s) => `\`${s}\``).join(", ")} changed in this branch, and no other ` +
            ...
            dependsOn: [],
            verify: contractFiles.map((path) => ({ kind: "file_committed", path })),
        };
        this.deps.state.audit("loop.generation_subtask_appended", {
        ...
        this.deps.logger.info("[loop] rc.7: appending a generation sub-task -- declared inputs moved and nothing in the plan owns the output", { sessionId, cycle, seq, scripts });
        return subTask;
    }
    ...
    async runFinalScopeCheck(sessionId, plan, cycle) {
        ...
        if (revisionScoped) {
            ...
            if (grandfathered.length > 0) {
                this.deps.state.audit("loop.revision_scope_grandfathered", {
                    sessionId,
                    cycle,
     ...
     */
    hardCapsAllow(requester, spentUsd, cyclesRan, overridden) {
        try {
            if (overridden)
                return true;
    ...
    warnSessionBudgetSoft(sessionId, user, totalCost, sessionBudget) {
        try {
            ...
            // run.
            let text = `:warning: This run passed its session budget TARGET ` +
     ...
     */
    async cancelSession(sessionId, opts = {}) {
        const row = this.deps.state.db
        ...
        this.deps.state.db
            .prepare(`UPDATE sessions SET reactions_json = ?, terminal_cause = 'user_cancel',
        ...
        this.deps.state.audit("loop.cancel_requested", { sessionId, reason, classification, requester: opts.requester ?? null, status: row.status, loopRunning }, sessionId);
        if (loopRunning) {
        ...
        try {
            ...
            if (row.status === "awaiting_clarification") {
                this.deps.state.db
                    .prepare(`UPDATE sessions SET clarification_question = NULL, clarification_seq = NULL,
            ...
            const outcome = await this.finaliseAbortSalvaging(sessionId, reason, row.cycles_ran ?? 0, row.cost_usd ?? 0);
            this.deps.state.audit("loop.cancel_terminated", { sessionId, reason, classification, requester: opts.requester ?? null, fromStatus: row.status, finalStatus: outcome.status }, sessionId);
            this.deps.interactionLog?.log(sessionId, {
                event: "cancel_terminated",
                phase: mapPhase(row.status),
    ...
    terminalCauseFor(reason, fallback) {
        if (reason === "budget_exhausted" || reason === "daily_max_exhausted")
            return "budget_exhausted";
        if (reason === "hard_timeout")
            return "hard_timeout";
        if (reason.startsWith("user_cancel"))
            return "user_cancel";
        if (reason.startsWith("accounting_incomplete"))
            return "accounting_incomplete";
        return fallback;
    }
    persistTerminalCause(sessionId, reason, fallback) {
        this.deps.state.db.prepare(`UPDATE sessions SET terminal_cause = COALESCE(terminal_cause, ?), updated_at = ? WHERE id = ?`).run(this.terminalCauseFor(reason, fallback), Date.now(), sessionId);
    }
            ...
            if (planJson) {
                try {
                    ...
                    if (!published.ok) {
                        if (published.kind === "push_failed")
                            throw published.pushError;
                        this.deps.state.audit("loop.abort_salvage_unpublished", {
     ...
     */
    async worktreeDirtyFiles(sessionId, worktreePath) {
        if (!worktreePath || !this.deps.worktreeStatusPorcelain)
            return [];
        try {
            ...
            this.deps.state.audit("loop.worktree_status_probe_indeterminate", { sessionId, worktreePath, error: String(err?.message ?? err).slice(0, 500) }, sessionId);
    ...
    async abortHasSalvageableCommits(sessionId, row) {
        ...
        try {
            ...
            if (dirty.length > 0) {
                this.deps.state.audit("loop.abort_dirty_worktree_salvageable", { sessionId, worktreePath: row.worktree_path, headSha: head, dirtyFiles: dirty.slice(0, 100), dirtyCount: dirty.length }, sessionId);
                return true;
            }
            return false;
        }
     ...
     */
    async finaliseFailed(sessionId, reason, cycles, totalCostUsd) {
        ...
        if (recoverable) {
            this.deps.state.audit("loop.failed_recoverable_work_detected", { sessionId, reason, cycles, ...recoverable }, sessionId);
     ...
     * for the HEAD probe.
     */
    async failureHasRecoverableWork(sessionId) {
        const row = this.deps.state.db
        ...
        const dirty = await this.worktreeDirtyFiles(sessionId, row.worktree_path);
        if (!commitsAhead && dirty.length === 0)
            return null;
     ...
     */
    preservedWorktreeRecoveryAction(sessionId, row) {
        if (!row?.worktree_path)
            return "No worktree path is recorded for this session.";
     ...
     */
    get publicationVerificationRequired() {
     ...
     */
    async resolveCandidateSha(worktreePath) {
        if (!this.deps.worktreeHeadSha)
            return "";
     ...
     */
    async verifyPublication(params) {
        ...
        const result = await verifyRemoteSha({
            ...
            readRemoteSha: () => probe({ plan: params.plan, branch: params.plan.branch, requester: params.requester }),
            ...
            signal: params.deadlineMs === undefined
                ? undefined
                : { get aborted() { return Date.now() > params.deadlineMs; } },
        });
        this.deps.state.audit(result.ok ? "loop.publication_verified" : "loop.publication_unverified", {
            sessionId: params.sessionId,
            cycle: params.cycle,
            stage: params.stage,
            branch: params.plan.branch,
    ...
    async publishCandidate(params) {
        ...
        if (params.existing && !reuse) {
            this.deps.state.audit("loop.publication_evidence_invalidated", {
        ...
        if (!this.publicationVerificationRequired) {
            ...
            this.deps.state.audit("loop.publication_unverifiable", { sessionId, cycle, stage, branch: plan.branch, candidateSha: candidateSha || "(unresolved)", reason: "no remoteBranchSha probe wired" }, sessionId);
     ...
     */
    recordPublicationEvidence(sessionId, evidence) {
        if (!evidence)
            return;
        try {
            this.deps.state.db
    ...
    async finaliseUnpublished(params) {
        this.deps.state.audit("loop.unpublished", {
            sessionId: params.sessionId,
            cycle: params.cycle,
            branch: params.plan.branch,
            repo: params.plan.repo,
        ...
        return await this.finaliseFailedPreserveWorktree(params.sessionId, `publication_unverified (${params.kind}): ${describeUnpublished({
            kind: params.kind,
        ...
        })}`, params.cycle, params.totalCost);
    }
    async finaliseFailedPreserveWorktree(sessionId, reason, cycles, totalCostUsd) {
        this.persistTerminalCause(sessionId, reason, "failed");
        ...
        const row = this.deps.state.db
            ...
            .get(sessionId);
        const headSha = row?.worktree_path && this.deps.worktreeHeadSha
            ? await this.deps.worktreeHeadSha(row.worktree_path).catch(() => "")
            : "";
        const dirty = await this.worktreeDirtyFiles(sessionId, row?.worktree_path);
        this.deps.state.audit("loop.failed_worktree_preserved", {
            sessionId,
            reason,
            cycles,
    ...
    async finaliseAwaitingClarification(sessionId, question, seq, cycles, totalCostUsd, subtask) {
        ...
        try {
            const row = this.deps.state.db
                .prepare(`SELECT worktree_path, branch, current_cycle, repo, requester FROM sessions WHERE id = ?`)
                .get(sessionId);
            if (row) {
                await this.durableCheckpoint({
                    sessionId,
        ...
        try {
            ...
            this.deps.state.db.prepare(`UPDATE sessions SET status = 'awaiting_clarification', clarification_question = ?,
                             clarification_seq = ?, clarification_id = ?, clarification_answer = NULL,
                             clarification_subtask = ?, last_progress_at = ?, updated_at = ? WHERE id = ?`).run(question, seq, clarificationId, subtask ? JSON.stringify(subtask) : null, pausedAt, pausedAt, sessionId);
            this.deps.state.db.exec("COMMIT");
        }
        catch (err) {
            try {
        ...
        try {
            this.deps.deliverProgress?.(sessionId, "awaiting_clarification");
        }
        catch { /* best effort */ }
        this.deps.state.audit("loop.clarification_requested", { sessionId, seq, clarificationId, question: question.slice(0, 1000), cycle: cycles }, sessionId);
...
//# sourceMappingURL=legacy-loop.js.map