...
export class OrchestratorLoop {
    ...
    async runInner(sessionId, brief) {
        ...
        while (cycle < this.deps.config.loop.max_cycles) {
            ...
            if (cycle > 1 &&
                ...
                try {
                    ...
                    if (refreshed?.subTasks && refreshed.subTasks.length > 0) {
                        ...
                        const withCtx = refreshed.subTasks.filter((sub) => sub.workerContext && (sub.taskMode === "mutate" || sub.taskMode === "mixed")).length;
                        this.deps.state.audit("loop.revise_spec_applied", { sessionId, cycle, subTasks: refreshed.subTasks.length, withWorkerContext: withCtx }, sessionId);
                        this.deps.interactionLog?.log(sessionId, {
            ...
            const runOne = async (st) => {
                ...
                const reviseHint = cycle > 1 && lastReview && !reviseSpecApplied ? buildReviseDispatchHint(lastReview) : undefined;
    ...
    async runWorkerCallWithRetry(p) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            try {
                ...
                this.deps.runWorker({ brief, subTask: st, plan, requester, dispatchHint, modelOverride: selectWorkerModel(st, this.deps.config.models), onStreamSlow }), this.deps.config.loop.worker_timeout_seconds);
            }
            catch (err) {
                if (err instanceof WorkerTimeoutError) {
                    threwTimeout = true;
    ...
    async finaliseReviewCrash(sessionId, err, cycle, totalCost, ctx) {
        ...
        const synthesizedReview = {
            verdict: "revise",
            findings: [],
            summary: "Adversary review crashed on an infrastructure error (e.g. out of disk) before producing a verdict; all sub-tasks self-verified green. Opened for MANUAL human review.",
            costUsd: 0,
            tokensIn: 0,
            tokensOut: 0,
        };
