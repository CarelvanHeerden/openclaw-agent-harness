        ...
        try {
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?, 'started', ?)`).run(id, params.sessionId, params.subTaskId ?? null, params.role, params.cycle ?? null, params.seq ?? null, attempt, params.model ?? null, params.route ?? null, params.baseSha ?? null, Date.now());
            db.prepare(`UPDATE sessions SET accounting_state = 'ok',
                             minimum_runtime_version = '2.0.0-rc.11', updated_at = ? WHERE id = ?`).run(Date.now(), params.sessionId);
            db.exec("COMMIT");
            return { id, attempt };
        }
        catch (err) {
        ...
        shipAttempts: for (;;) {
           ...
           WHERE id = ?`).run(result.status, result.costUsd, JSON.stringify(result.filesChanged), result.commitSha ?? null, result.sdkSessionId ?? null, result.reason ?? null, Date.now(), Date.now(), subTaskId);
                            ...
                            try {
                                ...
                                if (bound.changedConsumers.length > 0) {
                                    this.deps.state.db.prepare(`UPDATE sessions SET lead_plan_json = ?, plan_revision = ?,
                                       minimum_runtime_version = '2.0.0-rc.11', updated_at = ? WHERE id = ?`).run(JSON.stringify(bound.plan), resultRevision, Date.now(), sessionId);
                                }
                ...
                try {
                    ...
                    report = await withTimeout(this.deps.runAdversary({ brief, plan, sessionId, runtime, requester: row.requester, baseSha: adversaryBaseSha, priorFindings: lastReview?.findings, revision: revisionContext }), this.deps.config.loop.adversary_timeout_seconds, "adversary_timeout_seconds");
                    rawReview = {
                        ...report,
