...
export class OrchestratorLoop {
        ...
        shipAttempts: for (;;) {
           ...
           WHERE id = ?`).run(result.status, result.costUsd, JSON.stringify(result.filesChanged), result.commitSha ?? null, result.sdkSessionId ?? null, result.reason ?? null, Date.now(), Date.now(), subTaskId);
                    ...
                    if (contract.length > 0 && this.deps.buildVerifyProbes) {
                        ...
                        while ((observeReportIsNarration(result.finalMessage) || observeHasNoEvidence()) &&
    ...
    async cancelSession(sessionId, opts = {}) {
        ...
        this.deps.state.audit("loop.cancel_requested", { sessionId, reason, requester: opts.requester ?? null, status: row.status, loopRunning }, sessionId);
        if (loopRunning) {
        ...
        try {
            ...
            this.deps.state.audit("loop.cancel_terminated", { sessionId, reason, requester: opts.requester ?? null, fromStatus: row.status, finalStatus: outcome.status }, sessionId);
            this.deps.interactionLog?.log(sessionId, {
