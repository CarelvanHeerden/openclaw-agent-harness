...
function parseAffirmativeFragment(text, oldPath) {
    ...
    if (match) {
        ...
        if (!destinationPaths || !artifactReferenceIsComplete(match[2], oldPath))
            return undefined;
        return { kind: "substitution", sourcePath: oldPath, destinationPaths };
    }
    ...
    match = clause.match(/^document\s+(?:placeholders?|placeholder\s+examples|(?:all\s+)?(?:new|required)\s+variables\s+and\s+placeholder\s+examples)\s+in\s+(.+?)(?:\s+instead)?$/i);
    if (match) {
        const destinationPaths = parsePathList(match[1]);
        if (!destinationPaths)
            return undefined;
...
function provenancePrefixMatches(text) {
...
function unquotedHistoricalStatementIsComplete(text, oldPath) {
    const clause = withoutTerminalPunctuation(text);
    if (parseAffirmativeFragment(clause, oldPath))
        return true;
    const update = clause.match(/^update\s+(.+)$/i);
    return !!update && artifactReferenceIsComplete(update[1], oldPath);
}
function parseProvenanceFragment(text, oldPath) {
    const clause = text.trim();
    const prefix = clause.match(/^(?:the\s+)?(?:previous|prior|historical|original)\s+(?:plan|answer|proposal|instruction|contract)\s+(?:said|stated|quoted)(?:\s+that)?\s+/i);
    if (!prefix)
        return undefined;
...
function supportedNeutralFragment(text) {
...
function standaloneGlobalGateIsComplete(text) {
    const clause = withoutTerminalPunctuation(text).replace(/^however\s+/i, "");
    return (/^(?:keep|leave)\s+(?:the\s+)?(?:session|run|task|work)\s+(?:paused|stopped|on\s+hold)$/i.test(clause) ||
        ...
        /^(?:do\s+not|don't|must\s+not|may\s+not|shall\s+not)\s+(?:proceed|execute|apply|activate|implement|start|continue)(?:\s+it)?(?:\s+until\s+I\s+(?:approve|confirm))?$/i.test(clause) ||
...
function affirmativePartBeforeAttachedGate(text) {
    const clause = withoutTerminalPunctuation(text);
    const match = clause.match(/^(.*?),\s*(?:and|but|however|yet)\s+(.+)$/i);
    if (!match || !standaloneGlobalGateIsComplete(match[2]))
        return undefined;
    return match[1].trim();
}
function parseAuthorizationAnswer(fragments, oldPath) {
    const sourcePaths = [];
    ...
    while (queue.length > 0) {
        ...
        if (polarity === "affirmative") {
            ...
            if (parsed.sourcePath)
                sourcePaths.push(parsed.sourcePath);
            if (parsed.kind === "substitution")
                substitutionSets.push(parsed.destinationPaths);
            if (parsed.kind === "documentation")
                documentationSets.push(parsed.destinationPaths);
            if (parsed.kind === "contract_update")
                contractUpdateReferencesDocumentation = true;
            if (gate)
                globalGate ??= gate;
            continue;
        }
        ...
        const gate = globalAuthorizationGate(fragment);
        if (gate) {
            if (!standaloneGlobalGateIsComplete(fragment))
                unresolved.push(fragment);
            else
                globalGate ??= gate;
            continue;
        }
        if (!supportedNeutralFragment(fragment))
            unresolved.push(fragment);
    }
    const canonicalSet = (paths) => JSON.stringify([...new Set(paths)].sort());
    ...
    if (destinationPaths.length > 0) {
        ...
        if (extraDocumentation.length > 0) {
            unresolved.push(`documentation destinations are not authorized replacement outputs: ${extraDocumentation.join(", ")}`);
        }
    }
...
function taskProhibitionFields(task, path) {
    const fields = [];
    const visit = (value, field) => {
        if (typeof value === "string") {
            if (isProhibition(value, path))
                fields.push(field);
            return;
        }
...
function requiredMentions(task, path) {
    ...
    const visit = (value, field) => {
        if (typeof value === "string") {
            if (!artifactMentioned(value, path))
                return;
...
export function buildArtifactSubstitutionAmendment(input) {
    ...
    const rejectedOperands = parsedAnswer.destinationPaths.filter((path) => path === oldPath || path.startsWith(".env") || blocked.includes(path));
    ...
    const inheritedProhibitions = newPaths.flatMap((path) => taskProhibitionFields(input.task, path).map((field) => `${path} (${field})`));
    ...
    if (revised.workerContext) {
        ...
        revised.workerContext.gotchas = revised.workerContext.gotchas?.map((gotcha, index) => {
            ...
            if (next.changed)
                changed.add(`workerContext.gotchas[${index}]`);
            return next.value;
        });
        revised.workerContext.relatedSymbols = revised.workerContext.relatedSymbols?.map((symbol, index) => {
            ...
            if (next.changed)
                changed.add(`workerContext.relatedSymbols[${index}]`);
            return next.value;
        });
