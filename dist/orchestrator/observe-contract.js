...
export function validateObserveResult(input) {
    ...
    if (!raw || typeof raw !== "object")
        return { ok: false, reason: "observe result must be an object" };
    const result = raw;
    ...
    for (const spec of input.contract.bindings) {
        ...
        if (!binding) {
            if (spec.required !== false)
                return { ok: false, reason: `required binding ${spec.name} is missing` };
            continue;
        }
        ...
        switch (spec.type) {
            ...
            case "contract_patch":
                if (!path)
                    return { ok: false, reason: `proposed path binding ${spec.name} has no path` };
                if (pathExists(repoFiles, path)) {
            ...
            for (const field of target.fields) {
                switch (field) {
                    ...
                    case "workerContext.changeSpec":
                        ...
                        consumer.workerContext.changeSpec = appendBinding(consumer.workerContext.changeSpec ?? "", spec.name, value, target.placeholder);
                        break;
                }
            }
