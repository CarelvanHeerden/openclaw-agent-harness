...
function requiredFindingId(spec) {
    return spec.split(":", 1)[0].trim();
}
export function parseObserveEnvelope(text) {
    const trimmed = text.trim();
    if (trimmed.startsWith("{"))
        return JSON.parse(trimmed);
    const canonical = /^OBSERVE_RESULT\s*:?\s*```(?:json)?\s*([\s\S]*?)```\s*$/i.exec(trimmed);
    if (canonical)
        return JSON.parse(canonical[1]);
...
export function renderObserveContractInstructions(contract) {
    return [
...
function bindingPath(binding) {
    if (typeof binding.value === "string")
        return binding.value.trim() || undefined;
...
function contractPatch(binding) {
    ...
    if (Object.keys(value).some((key) => !allowed.has(key)))
        return undefined;
    if (value.filesLikelyTouched !== undefined &&
    ...
    if (value.verify !== undefined &&
        (!Array.isArray(value.verify) || !value.verify.every((probe) => probe && typeof probe === "object" && typeof probe.kind === "string")))
        return undefined;
    ...
    if (value.successCriteria !== undefined &&
        ...
        return undefined;
    if (value.workerContextChangeSpec !== undefined && typeof value.workerContextChangeSpec !== "string")
        return undefined;
    if (value.requiredBehaviorChecks !== undefined &&
...
export function validateObserveResult(input) {
    ...
    if (!raw || typeof raw !== "object")
        return { ok: false, kind: "invalid_schema", reason: "observe result must be an object" };
    const result = raw;
    ...
    const validEvidencePath = (ref) => {
        ...
        if (!repoEntryExists(repoFiles, path))
            return false;
    ...
    for (const spec of input.contract.bindings) {
        ...
        if (!binding) {
            if (spec.required !== false)
                return { ok: false, kind: "invalid_schema", reason: `required binding ${spec.name} is missing` };
            continue;
        }
        ...
        switch (spec.type) {
            ...
            case "proposed_output_path":
                if (!path)
                    return { ok: false, kind: "invalid_schema", reason: `proposed path binding ${spec.name} has no path` };
...
function appendBinding(text, name, value, placeholder) {
    if (placeholder) {
        if (!text.includes(placeholder))
            throw new Error(`binding placeholder ${placeholder} is missing`);
...
export function applyObserveBindings(input) {
    ...
    if (!contract)
        return { plan: structuredClone(input.plan), changedConsumers: [], bindingsHash: hash([]) };
    ...
    for (const spec of contract.bindings) {
        ...
        if (!patch && value === undefined)
            throw new Error(`observe binding ${spec.name} has no scalar value`);
        for (const target of spec.applyTo ?? []) {
            const consumer = revised.subTasks.find((task) => task.seq === target.consumerSeq);
            if (!consumer)
                throw new Error(`observe binding ${spec.name} targets missing sub-task ${target.consumerSeq}`);
            if (consumer.seq === input.producer.seq)
                throw new Error("observe binding cannot rewrite its producer");
            for (const field of target.fields) {
                switch (field) {
                    case "filesLikelyTouched":
                        if (patch) {
                            if (!patch.filesLikelyTouched)
                                throw new Error(`contract patch ${spec.name} has no filesLikelyTouched`);
                    ...
                    case "verify":
                        ...
                        if (patch) {
                            if (!patch.verify)
                                throw new Error(`contract patch ${spec.name} has no verify probes`);
                        ...
                        break;
                    case "intent":
                        if (patch && patch.intent === undefined)
                            throw new Error(`contract patch ${spec.name} has no intent`);
                        consumer.intent = appendBinding(consumer.intent, spec.name, patch?.intent ?? value, target.placeholder);
                        break;
                    case "successCriteria":
                        if (patch) {
                            if (!patch.successCriteria)
                                throw new Error(`contract patch ${spec.name} has no successCriteria`);
                        ...
                        if (target.placeholder) {
                            ...
                            consumer.successCriteria = consumer.successCriteria.map((criterion) => {
                                if (!criterion.includes(target.placeholder))
                                    return criterion;
                                hit = true;
                                return criterion.split(target.placeholder).join(value);
                            });
                            if (!hit)
                                throw new Error(`binding placeholder ${target.placeholder} is missing from success criteria`);
                        }
                        else {
                    ...
                    case "workerContext.changeSpec":
                        if (!consumer.workerContext)
                            throw new Error(`consumer ${consumer.seq} has no workerContext`);
                        if (patch && patch.workerContextChangeSpec === undefined) {
                            throw new Error(`contract patch ${spec.name} has no workerContextChangeSpec`);
                        }
                        consumer.workerContext.changeSpec = appendBinding(consumer.workerContext.changeSpec ?? "", spec.name, patch?.workerContextChangeSpec ?? value, target.placeholder);
                        break;
                    case "requiredBehaviorChecks":
                        ...
                        for (const check of patch.requiredBehaviorChecks) {
                            ...
                            if (at >= 0)
                                consumer.requiredBehaviorChecks[at] = structuredClone(check);
                            else
                                consumer.requiredBehaviorChecks.push(structuredClone(check));
                        }
                        break;
                }
            }
...
export function loadBearingObserveContractErrors(plan) {
...
//# sourceMappingURL=observe-contract.js.map