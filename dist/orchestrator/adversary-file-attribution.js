...
export function requiresFile(f) {
    return (DIFF_ADDRESSABLE_DIMENSIONS.has(normaliseDimension(f.dimension)) &&
        AT_LEAST_MEDIUM.has((f.severity ?? "").toLowerCase()));
}
/** True when the finding is one that requires a file but has none. */
export function isUnfiledDiffAddressable(f) {
    if (!requiresFile(f))
        return false;
...
export function findingsMissingFile(findings) {
 ...
 */
export function buildFileAttributionRetryNudge(missing) {
    const lines = [
        "",
...
//# sourceMappingURL=adversary-file-attribution.js.map