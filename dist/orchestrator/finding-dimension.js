 ...
 * Pure/deterministic.
 */
const CANONICAL = new Set(["spec", "fit", "quality", "security", "runtime"]);
/**
 ...
 */
export function normaliseDimension(raw) {
    const s = String(raw ?? "")
        .trim()
        .toLowerCase()
        .replace(/[\s_]+/g, "-")
    ...
    for (const key of ["security", "runtime", "quality", "spec", "fit"]) {
        if (s.includes(key))
            return key;
    }
    return "";
}
const hasFile = (f) => String(f.file ?? "").trim().length > 0;
/**
 ...
 */
export function isBroadcastOnly(f) {
    return !isRoutable(f);
}
/**
 ...
 */
export function isMetaDimension(f) {
    const d = normaliseDimension(f.dimension);
    return d === "fit" || d === "runtime";
}
//# sourceMappingURL=finding-dimension.js.map