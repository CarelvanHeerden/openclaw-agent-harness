...
export function fileScopesOverlap(a, b) {
    const sa = normSet(a);
    const sb = normSet(b);
    for (const x of sa)
        if (sb.has(x))
 ...
 *  - otherwise -> true.
 */
export function canDispatchConcurrently(candidate, inFlight) {
    if (inFlight.length === 0)
        return true; // first one always dispatches
    const cFiles = (candidate.filesLikelyTouched ?? []).filter(Boolean);
    if (cFiles.length === 0)
        return false; // unknown candidate scope
    for (const other of inFlight) {
        const oFiles = (other.filesLikelyTouched ?? []).filter(Boolean);
        if (oFiles.length === 0)
            return false; // unknown in-flight scope
        if (fileScopesOverlap(cFiles, oFiles))
            return false;
    }
    return true;
}
...
//# sourceMappingURL=parallel-safety.js.map