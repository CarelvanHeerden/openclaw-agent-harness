...
export function rederiveContractPath(contract, realFiles, opts = {}) {
    ...
    if (opts.repoFiles) {
        for (const f of opts.repoFiles) {
            if (typeof f === "string" && normalisePath(f) === c)
                return { path: contract, remapped: false };
        }
    }
    const segs = c.split("/");
    ...
    for (const rm of sorted) {
        ...
        if (rm.tail.split("/").filter(Boolean).length < 2)
            continue;
        ...
        if (!isTestFilePath(c) && isTestFilePath(corrected)) {
            return {
                ...
                suggestion: {
                    ...
                    via: rm,
                    confidence: "low",
                    reason: `the only evidence is a shared '${rm.tail}' directory suffix, and applying it would move a ` +
 ...
 */
function declineOrPass(contract, c, staleDir, base, contractIsTest, crossKind) {
