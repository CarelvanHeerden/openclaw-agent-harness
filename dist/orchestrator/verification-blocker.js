 ...
 */
const MISSING_BINARY_RE = /\b(tsc|typescript|eslint|prettier|jest|vitest|playwright|cypress|pytest|tsx|ts-node|node|npm|pnpm|yarn|bun|go|cargo|python3?|ruby|docker|psql|prisma)\b[^.\n]{0,8}?\b(is |was |are |were )?(not (installed|available|found|present|on the path)|unavailable|missing|cannot be (found|located|resolved)|could not be (found|located|resolved)|does not exist|: not found|command not found)/i;
const MISSING_BINARY_REVERSED_RE = /\b(cannot find|could not find|unable to (find|locate|run|execute)|no such (file or directory|binary|executable)|command not found|exit(ed)? (with )?(code )?12[67])\b[^.\n]{0,40}?\b(tsc|typescript|eslint|prettier|jest|vitest|playwright|cypress|pytest|tsx|ts-node|npm|pnpm|yarn|bun|cargo|python3?|docker|prisma)\b/i;
const MISSING_DEPENDENCY_RE = /\b(cannot find module|MODULE_NOT_FOUND|ERR_MODULE_NOT_FOUND|node_modules (is |are )?(missing|absent|not installed)|dependencies (are )?not installed|npm (ci|install) (failed|did not run|was never run))\b/i;
const RUNTIME_EVIDENCE_RE = /\b(browser (is |was )?(not available|unavailable)|no (browser|headless) (binary|runtime)|playwright browsers? (are |is )?not installed|screenshot could not be (taken|captured)|preview (deploy|environment) (is )?unavailable|runtime (evidence|verification) (is )?(unavailable|impossible))\b/i;
const NETWORK_RE = /\b(ENOTFOUND|ECONNREFUSED|ETIMEDOUT|EAI_AGAIN|getaddrinfo|network (is )?unreachable|registry\.npmjs\.org.*(unreachable|failed)|DNS (lookup )?fail)/i;
/**
 ...
 */
export function detectVerificationBlocker(f) {
    ...
    const titleIsAboutVerification = VERIFICATION_TITLE_RE.test(title) || anyBlockerPattern(title);
    if (!titleIsAboutVerification)
        return null;
    ...
    if (runtime) {
        return {
            ...
            humanAction: "Provide the runtime evidence the check needs -- a preview deployment, an uploaded log, or an installed " +
    ...
    if (network) {
        return {
            ...
            humanAction: "Restore network access from the harness environment (registry, DNS or proxy) and re-run. " +
    ...
    if (dependency) {
        return {
            ...
            humanAction: "Install the worktree's dependencies (for example `npm ci`) in the harness environment and re-run the review. " +
    ...
    if (binary) {
        ...
        return {
            kind: "missing_binary",
            subject,
            humanAction: `Make ${subject ?? "the missing tool"} available on the harness PATH (install it, or add it to the repo's ` +
    ...
    if (worktree) {
        return {
            ...
            humanAction: "Repair or re-create the harness worktree (re-clone and re-install) and re-run. The branch's contents are " +
                "not what is broken here.",
        };
    }
    return null;
}
function anyBlockerPattern(text) {
...
const SUBJECT_RE = /\b(tsc|typescript|eslint|prettier|jest|vitest|playwright|cypress|pytest|tsx|ts-node|npm|pnpm|yarn|bun|cargo|python3?|docker|prisma|node_modules)\b/i;
 ...
 */
export function describeVerificationBlocker(f, b) {
    return [
...
//# sourceMappingURL=verification-blocker.js.map