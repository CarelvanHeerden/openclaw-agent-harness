 ...
 */
export function findPlanPolicyConflicts(subTasks, denylist, exceptions = []) {
    if (!subTasks || subTasks.length === 0 || denylist.length === 0)
        return [];
    const out = [];
    const seen = new Set();
 ...
 */
export function describePlanPolicyConflicts(conflicts) {
    ...
    for (const [seq, list] of [...bySeq.entries()].sort((a, b) => a[0] - b[0])) {
        ...
        for (const c of list)
            lines.push(`  - \`${c.path}\` is blocked by the denylist rule \`${c.rule}\``);
    }
    lines.push("", "Nothing has been dispatched for the affected sub-task, so no budget has been spent on it yet.", "", "Your options: authorise the exact path(s) via `safety.path_denylist_exceptions` if they are genuinely " +
        "tracked templates and a deployment change is approved separately, restate the sub-task so it achieves " +
        'its goal without writing the blocked path, answer "skip" to drop the sub-task, or "abort".', "", "Note that authorising a template path does not authorise putting a real credential in it: content is " +
        "scanned independently, and a secret in an authorised template is still refused.");
    return lines.join("\n");
}
//# sourceMappingURL=plan-policy-conflict.js.map