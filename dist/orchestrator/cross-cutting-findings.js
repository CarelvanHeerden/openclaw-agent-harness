 ...
 * substantially.
 */
export function isSameFinding(a, b) {
    if ((a.dimension ?? "").toLowerCase().trim() !== (b.dimension ?? "").toLowerCase().trim())
        return false;
    if ((a.file ?? "").trim() !== (b.file ?? "").trim())
        return false;
 ...
 * survived.
 */
export function detectStuckFindings(history, current) {
    if (history.length === 0)
        return [];
    const previous = history[history.length - 1] ?? [];
    const out = [];
    for (const f of current) {
        if (!previous.some((p) => isSameFinding(p, f)))
            continue;
        // Count back through history for the run of consecutive cycles.
        let occurrences = 1;
        for (let i = history.length - 1; i >= 0; i--) {
            if ((history[i] ?? []).some((p) => isSameFinding(p, f)))
                occurrences += 1;
            else
                break;
        }
 ...
 */
export function describeUnresolvable(items) {
    if (items.length === 0)
        return "";
...
//# sourceMappingURL=cross-cutting-findings.js.map