 ...
 */
function classifyAnswerDecision(answerText) {
    if (/^(abort|cancel)\b/i.test(answerText))
        return "abort";
    if (/^skip\b/i.test(answerText))
        return "skip";
    if (/^accept\b/i.test(answerText) || /^keep(-|\s)?commit\b/i.test(answerText))
        return "accept";
    return "guidance";
}
/**
...
export function registerHarnessTools(api, runtime) {
    ...
    disposers.push(toDispose(api.registerTool({
        name: "harness_cancel",
        description: "Cancel a harness session from any state, including one paused awaiting clarification. A session with no running loop is terminated immediately; one with a running loop stops at its next checkpoint. Safe to call more than once.",
        parameters: {
            type: "object",
            properties: {
        ...
        execute: async (_callId, input) => {
            const { sessionId, reason, classification, invokedBy } = input;
            ...
            liveState().audit("tool.cancel", { sessionId, reason: reason ?? "tool-invoked", classification: classification ?? "operator_cancelled", invokedBy: invokedBy ?? null }, sessionId);
    ...
    disposers.push(toDispose(api.registerTool({
        ...
        description: "Recover the association between a FAILED harness session and the pull request it already produced, so harness_revise can update that same PR instead of rebuilding the feature. " +
        ...
        execute: async (_callId, input) => {
            const { sessionId, repo, prNumber, invokedBy, apply, expectedHeadSha } = input;
            ...
            if (res.awaitingConfirmation === true) {
                return {
                    ...
                    details: {
                        ...
                        feedback: {
                            instruction: "STOP and show the question verbatim. Wait for the authenticated requester to reply naturally, then call harness_answer with answeredBy:'human', invokedBy matching the host requester, and the current clarification identity. Do not approve on their behalf.",
            ...
            if (res.awaitingConfirmation === true) {
                return {
                    ...
                    details: {
                        ...
                        effectiveBudget: res.effectiveBudget,
                        feedback: {
                            instruction: "STOP and show the question verbatim. Wait for the authenticated requester to reply naturally, then call harness_answer with answeredBy:'human', invokedBy matching the host requester, and the current clarification identity. Do not approve on their behalf or start another run.",
                            answerWith: "natural_language_reply",
                        },
                    },
                };
        ...
        execute: async (_callId, input) => {
            ...
            if (row.status === "accounting_incomplete") {
                return {
                    content: [{
                            type: "text",
                            text: `Cannot resume ${sessionId}: a provider call has unknown or unpersisted cost/result state. ` +
    ...
    const buildAnswer = (toolContext) => ({
        name: "harness_answer",
        description: "Answer a paused harness clarification after interpreting the authenticated requester's natural-language reply, " +
        ...
        parameters: {
            ...
            properties: {
                ...
                clarificationId: {
                    type: "string",
                    minLength: 1,
                    description: "Stable identity from harness_progress for the open mid-run clarification. Required for newly-created rc.11 clarifications; prevents an answer for an older question with the same seq from landing on a newer one.",
                },
                answeredBy: {
                    type: "string",
                    enum: ["human", "automation"],
                    description: "Use 'human' only when interpreting the current host-authenticated requester's natural-language answer. " +
                ...
                evidence: {
                    type: "string",
                    description: "Optional for a human answer, REQUIRED when answeredBy is 'automation': the evidence the decision rests on -- the changed-file list, the worker commit sha, the check results, and why the deviation is safe. Recorded verbatim (bounded) in the audit trail. An automatic answer with no evidence is refused, because an unevidenced automatic answer cannot be reviewed afterwards.",
                },
            },
        ...
        execute: async (_callId, input) => {
            const { sessionId, answer, invokedBy, clarificationSeq, clarificationId, answeredBy, evidence } = input;
            ...
            if (!trustedSender ||
                ...
                return {
                    content: [{
                            type: "text",
                            text: `Authenticated requester ${trustedSender || "(missing)"} is not authorised for harness_answer ` +
            ...
            let confirmedStoredAmendment = null;
            try {
            ...
            const rejectAutomatic = (reason) => {
                if (!automated)
                    return;
                liveState().audit("tool.clarification_auto_accept_rejected", { ...answerFacts, reason, evidence: evidenceText.slice(0, 2000) }, sessionId);
            };
            if (automated) {
                liveState().audit("tool.clarification_auto_accept_attempted", { ...answerFacts, evidence: evidenceText.slice(0, 2000), hasEvidence: evidenceText.length > 0 }, sessionId);
            }
            ...
            if (typeof clarificationSeq === "number" && Number.isFinite(clarificationSeq) && clarificationSeq !== seq) {
                liveState().audit("tool.answer_stale_seq", { ...answerFacts, suppliedSeq: clarificationSeq, openSeq: seq }, sessionId);
            ...
            if (row.clarification_id) {
                if (!clarificationId || clarificationId !== row.clarification_id) {
                    liveState().audit("tool.answer_stale_id", { ...answerFacts, suppliedId: clarificationId ?? null, openId: row.clarification_id }, sessionId);
                    rejectAutomatic("stale_clarification_id");
                    return {
                        content: [{
                                type: "text",
                                text: `Not answering: this rc.11 clarification requires its current clarificationId. ` +
            ...
            if (automated && evidenceText.length === 0) {
                ...
                return {
                    content: [{
                            type: "text",
                            text: `Not answering: an automatic answer must carry its evidence. Pass \`evidence\` with the changed-file ` +
            ...
            liveState().audit("loop.clarification_answered", { ...answerFacts, answerLen: trimmed.length, evidence: evidenceText.slice(0, 2000) }, sessionId);
            if (automated) {
                liveState().audit("tool.clarification_auto_accept_succeeded", { ...answerFacts, answerLen: trimmed.length, evidence: evidenceText.slice(0, 2000) }, sessionId);
            }
            ...
            if (isTimeExtensionPause(row.clarification_subtask) || budgetPause) {
                ...
                if (alive) {
                    ...
                    liveState().audit(budgetPause ? "tool.answer_budget_extension" : "tool.answer_time_extension", { sessionId, answerLen: trimmed.length, invokedBy: invokedBy ?? null, waitUntilMs, windowOpen }, sessionId);
                ...
                liveState().audit(budgetPause ? "tool.answer_budget_extension_listener_lost" : "tool.answer_time_extension_listener_lost", {
                ...
                if (prUrl) {
                    try {
                        closeActiveDeadline(liveDb(), sessionId);
                    }
                    catch { /* legacy/unmigrated test row */ }
                    liveDb()
            ...
            if (/^(abort|cancel)\b/i.test(trimmed)) {
                ...
                liveDb().prepare(`UPDATE sessions SET status = 'aborted', terminal_cause = 'user_cancel',
                                 terminal_classification = 'operator_cancelled', updated_at = ? WHERE id = ?`).run(Date.now(), sessionId);
            ...
            if (isBriefConfirmationPause(row.clarification_subtask)) {
                ...
                if (parsed.ambiguities.length > 0) {
                    ...
                    liveState().audit("tool.answer_brief_control_ambiguous", {
                        sessionId,
            ...
            try {
                resumeActiveDeadline(liveDb(), sessionId, liveConfig().loop?.session_hard_timeout_seconds ?? 7200);
            }
            catch (err) {
                releaseClaim();
                return {
                ...
                if (row.lead_plan_json && row.clarification_subtask) {
                    ...
                    if (paused.task) {
                            ...
                            if (!amendment.ok) {
                                ...
                                liveDb().prepare(`UPDATE sessions SET clarification_question = ?, clarification_id = ?,
                                    ...
                                    (amendment.proposedDiff
                                        ...
                                        : `Please restate the exact old artifact and exact replacement path(s), or answer "abort".`), replacementId, nextSubtask, Date.now(), sessionId);
                        ...
                        WHERE id = ? AND status = 'pending'`).run("revised contract still conflicts with safety policy", amendment.amendment.id);
                            ...
                            try {
                                liveDb().exec("BEGIN IMMEDIATE");
                                const current = liveDb().prepare(`SELECT status, clarification_id, lead_plan_json FROM sessions WHERE id = ?`).get(sessionId);
                                if (!current ||
                      ...
                      WHERE id = ?`).run(JSON.stringify(revisedPlan), JSON.stringify(brief), now, sessionId);
                            ...
                            liveState().audit("tool.answer_contract_amendment_activated", {
                                sessionId,
                                seq,
            ...
            if (canResumeInPlace) {
                if (/^skip\b/i.test(trimmed)) {
                  ...
                  SET status = 'completed_no_change',
                      ...
                      updated_at = ?
                WHERE session_id = ? AND seq = ?
                  AND cycle = (SELECT MAX(cycle) FROM sub_tasks WHERE session_id = ? AND seq = ?)`).run(`skipped by operator: ${trimmed.slice(0, 300)}`, Date.now(), Date.now(), sessionId, seq, sessionId, seq);
                }
                brief.resumeExistingPlan = true;
                liveState().audit("tool.answer_resume_in_place", { sessionId, seq, skip: /^skip\b/i.test(trimmed) }, sessionId);
            }
    ...
    function listRevisableRows() {
        return liveDb()
            .prepare(`SELECT ${REVISABLE_COLUMNS}
           FROM sessions
     ...
     */
    function hasNoReviewContext(sessionId) {
        const r = liveDb()
     ...
     */
    function rootFeatureBrief(sessionId, fallbackJson) {
        ...
        for (let hop = 0; hop < 8; hop += 1) {
            ...
            if (row.original_feature_brief)
                return row.original_feature_brief;
            currentJson = row.crystallised_prompt ?? currentJson;
            let parent = "";
            try {
                const parsed = JSON.parse(row.crystallised_prompt ?? "{}");
                if (typeof parsed.reviseOfSessionId === "string")
                    parent = parsed.reviseOfSessionId.trim();
            }
            catch {
                return currentJson;
            }
    ...
    disposers.push(toDispose(api.registerTool({
        ...
        parameters: {
            ...
            properties: {
                ...
                repo: {
                    type: "string",
                    minLength: 3,
                    description: "Repository as 'owner/name'. Disambiguates `prNumber`, which is only unique within a repository. Required when the same PR number exists in more than one repo the harness has run against.",
                },
        ...
        execute: async (_callId, input) => {
            const { requester, prNumber, sessionId, repo, budgetUsd, dropFindings, guidance } = input;
            if (!liveConfig().slack.authorised_users.includes(requester)) {
                return { content: [{ type: "text", text: `Requester ${requester} is not in slack.authorised_users` }], details: { ok: false, unauthorised: true } };
            }
            ...
            if (sessionId) {
                ...
                const candidates = liveDb()
                    .prepare(`SELECT ${REVISABLE_COLUMNS} FROM sessions WHERE pr_number = ?${repo ? ` AND lower(repo) = lower(?)` : ``} ORDER BY created_at DESC`)
                    .all(...(repo ? [prNumber, repo] : [prNumber]));
                ...
                if (repos.length > 1) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: `PR #${prNumber} is ambiguous: it matches sessions in ${repos.length} repositories (${repos.join(", ")}). ` +
