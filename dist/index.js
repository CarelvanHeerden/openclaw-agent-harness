...
function bootstrapHarnessSync(api) {
    ...
    runtime.controlPlane = new ControlPlaneService({
        ...
        executeEngine: async (change) => {
            ...
            const row = state.db.prepare(`SELECT pr_number,final_pr_url,published_sha,published_at,cost_usd,created_at,updated_at,merge_recommendation,deploy_status FROM sessions WHERE id=?`).get(change.changeId);
        ...
        FROM sub_task_attempts WHERE session_id=?
            ...
            const securityReceipt = state.db.prepare(`INSERT INTO control_security_receipts (run_id,sha,complete,detected,observed_at) VALUES (?,?,?,?,CAST(unixepoch('subsec')*1000 AS INTEGER)) RETURNING sha,complete,detected,observed_at`).get(change.changeId, pr.headSha, secretScanComplete ? 1 : 0, secretScan.found ? 1 : 0);
...
async function bootstrapHarnessAsync(runtime, api) {
    ...
    try {
        ...
        const configuredModels = [...new Set(anthropicRoutes.map((r) => r.model).filter((m) => Boolean(m)))];
...
async function teardown(runtime, api) {
    ...
    const ownedRunning = () => runtime.ownedRunningSessionIds?.() ?? runningSessionIds();
