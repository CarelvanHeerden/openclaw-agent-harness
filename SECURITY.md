...
- **Interaction log redaction.** Token-shaped strings and known-sensitive keys are redacted from the durable interaction log. Redaction is pattern-based and cannot be assumed complete for a custom credential format. The same pattern set backs the template content scan below and the redaction of checkpoint manifests and operator-facing diagnostics, so a shape added in one place is recognised everywhere — and a shape nobody has added is recognised nowhere.
- **Authorised template exceptions.** `safety.path_denylist_exceptions` lets a deployment authorise EXACT repo-relative paths the denylist covers, for the tracked-template case (`.env.example`). It is empty by default. It takes no globs and no prefixes, it applies to every resolved form of a path rather than to the string as written, and it does **not** turn off content scanning: an authorised template that would receive token-shaped or high-entropy material is refused anyway. Authorising a file to be edited is not authorising a credential to be put in it.
- **Path canonicalisation before policy.** Denylist decisions are made on canonicalised paths, so traversal, absolute and relative spellings, and symlinks all reach the same verdict. `apply_patch` targets are parsed from the patch body rather than trusted from a caller-supplied `locations` array, every target in a multi-file patch is checked, and one forbidden target rejects the whole operation regardless of ordering. A string that ambiguously encodes several paths is refused rather than resolved to whichever reading happens to be permitted — rc.8 allowed a patch touching `.env.production` because the comma-joined list it arrived in did not match the denylist as a single string.
